...
public class TellerService {
	...
	public synchronized static TellerService getInstance() {
		/*
		 * if (null == single) { single = new TellerService(); } return single;
		 */
	 ...
	 * @param tlrno
	 * @param bizClass
	 *            审批类型 0-不限，1-贷款业务，2-合作项目
	 * @param bizSubClass
	 *            贷款品种，000000表示不限；合作项目类型，0表示不限
	 * @return 返回null表示没有权限
	 * @throws CommonException
	 */
	public LimitParam getTellerApproveLimit(String tlrno, String bizClass, String bizSubClass) throws CommonException {
		LimitParamDAO dao = DAOUtils.getLimitParamDAO();
		LimitParam limitParam = dao.query4status(tlrno, bizClass, bizSubClass);
		if (limitParam == null || limitParam.getStatus().equals(SystemConstant.VALID_FLAG_INVALID)) {
	...
	public List getTellerRoleList(String tlrno) throws CommonException {
		return DAOUtils.getTlrRoleRelDAO().queryByCondition("tlrno = '" + tlrno + "'");
	}
	 ...
	 * @throws CommonException
	 */
	public boolean checkTellerFuncRight(String tlrno, String funcCode) throws CommonException {
		if (funcCode.equals("0"))
			return true;
		...
		if (funcInfo.getFuncClass().equals("2")) {
			...
			List list = DAOUtils.getFunctionInfoDAO().queryByCondition(
					"po.upFuncCode = " + funcCode + " and po.status = '" + SystemConstant.VALID_FLAG_VALID + "'");
			for (int i = 0; i < list.size(); i++) {
	 ...
	 * @throws CommonException
	 */
	public List getTellerRoleList(String tlrno, String funcCode) throws CommonException {
		FunctionInfo funcInfo = DAOUtils.getFunctionInfoDAO().query(Integer.parseInt(funcCode));
		if (funcInfo.getStatus().equals(SystemConstant.FLAG_OFF))
			funcCode = funcInfo.getUpFuncCode();
	...
	public List getRoleIdByType(String roleType) throws CommonException {
		try {
			...
			if (result.isEmpty()) {
				ExceptionUtil.throwCommonException("没有指定的岗位信息.", ErrorCode.ERROR_CODE_ROLE_INFO_SELECT);
			}
			return result;
		} catch (CommonException commEx) {
			throw commEx;
		} catch (Exception ex) {
			ExceptionUtil.throwCommonException("获取岗位信息出错.", ErrorCode.ERROR_CODE_ROLE_INFO_SELECT, ex);
		}
		return null;
	...
	// int roleId =
	// Integer.parseInt(globalInfo.getUserRoles().get(i).toString());
