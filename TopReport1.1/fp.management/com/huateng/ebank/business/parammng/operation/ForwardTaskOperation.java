...
public class ForwardTaskOperation extends BaseOperation {
	...
	public void execute(OperationContext context) throws CommonException {
		...
		for (int i = 0; i < taskList.size(); i++) {
			...
			// DAOUtils.getWorkflowTaskInfoDAO().queryByTaskId(workFlowForwardTaskBean.getTaskId());
			// String roleTypeO = WorkflowTaskInfo.getRoleType();
			//
			// if(!tlrno.equals(SystemConstant.ROLE_DEFINFE_TLRNO)){
			// //检查被移交操作员岗位类型
			// List roleInfoList =
			// TellerService.getInstance().getTellerRoleInfoList(forwardTlrno);
			...
			// Applydtl applydtltmp = (Applydtl)
			// DAOUtils.getApplydtlDAO().queryByAppno(appno).get(0);
