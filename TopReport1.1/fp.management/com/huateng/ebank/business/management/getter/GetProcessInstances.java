...
/**
 * Title: GetProcessInstances Description: Copyright: Copyright (c) 2007
...
public class GetProcessInstances {
	...
	/*
	 * public String getProcessInstances(String contractNo,String procNo,String
	 * doTlrno,String taskId,HttpServletRequest req,HttpServletResponse rsp){
	 * ctx = req.getContextPath(); boolean suc =
	 * initialize(contractNo,procNo,doTlrno,taskId); if( suc ){
	 * retrieveByteArrays(); try{ if (gpdBytes != null && imageBytes != null) {
	 * writeTable(); return sb.toString(); }else{ return null; }
	 * }catch(Exception ex){ ex.printStackTrace(); return null; } }else{ return
	 * null; } }
	 */
	...
	/*
	 * private boolean initialize(String contractNo,String procNo,String
	 * doTlrno,String taskId2) { // WorkflowCallerNew workflowCallerNew = new
	 * WorkflowCallerNew(); long taskid1 = Long.valueOf(taskId2);
	 * ProcessInstance processInstance = this.GetProcessInstrance(procNo,
	 * contractNo,doTlrno,taskid1); if( processInstance == null ){ return false;
	 * } this.taskInstanceId = -1; tokenInstanceId = processInstance.getId(); if
	 * (this.taskInstanceId > 0) { TaskInstance taskInstance =
	 * this.getContext().getTaskMgmtSession().loadTaskInstance(taskInstanceId);
	 * currentToken = taskInstance.getToken(); } else { if (this.tokenInstanceId
	 * > 0) currentToken =
	 * this.getContext().getGraphSession().loadToken(this.tokenInstanceId); }
	 * processDefinition =
	 * currentToken.getProcessInstance().getProcessDefinition(); return true; }
	 */
	...
	public ProcessInstance GetProcessInstrance(String piid) {
		...
		/*
		 * while(iter.hasNext()){ taskid = (TaskInstance)iter.next();
		 * processInstance = taskid.getContextInstance().get;
		 * processInstance.getId(); // ProcNameTmp =
		 * (String)processInstance.getProcessDefinition().getName(); //
