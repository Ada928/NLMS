...
public class RouteService {
	...
	public PageQueryResult queryRouteBindingRouteInfo(WorkflowRouteBinding WorkflowRouteBinding, int pageSize,
		...
		// WorkflowRouteDef workflowRouteDef = (WorkflowRouteDef)
		// routedeflist.get(i);
		// List routeparamlist =
		// BaseDAOUtils.getWorkflowRouteParamDAO().queryByCondition(" po.routeId
		// = " + workflowRouteDef.getId() );
		// for(int j =0;j<routeparamlist.size();j++){
		// WorkflowRouteParam workflowRouteParam = (WorkflowRouteParam)
		// routeparamlist.get(j);
		...
		for (int i = 0; i < routedeflist.size(); i++) {
			...
			for (int j = 0; j < wrpList.size(); j++) {
				...
				/*
				 * if(brhClass.compareTo(fBrhClass)==0){ flags = true; }
				 */
	...
	public PageQueryResult queryApproveNodeRouteBindingInfo(RouteBindingView inview, int pageSize, int pageIndex)
		...
		if (!DataFormat.isEmpty(inview.getBussType())) {
			/** mod by abudula at 20101118 start STL-62 */
			// sb1.append(" and substr(bind.bussType,0,4) = '"+
			// inview.getBussType()+"'");
