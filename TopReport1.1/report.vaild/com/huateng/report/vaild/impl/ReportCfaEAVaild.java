...
public class ReportCfaEAVaild extends AbsReportDataVaild {
	...
	public String executeDataVaild(Object obj) {
		...
		// 删除原因
		result.append(this.isDelRemarkVaild(ea.getActiontype(), ea.getActiondesc()));

		// EXPLRMBLONO 外汇质押人民币贷款编号
		result.append(this.valLenByNotNull(ea.getExplrmblono(), 28, "外汇质押人民币贷款编号"));
		result.append(this.exbuiSeNumVaild(ea.getApptype(), ea.getCurrentfile(), ea.getFiller2(), ea.getBrNo()));
		...
		// 贷款起息日 VALUEDATE
		result.append(this.checkDateFormat(ea.getValuedate(), "yyyyMMdd", "贷款起息日"));
		...
		// MATURITY 贷款到期日
		result.append(this.dateLessCheckDateOrEqual(ea.getValuedate(), ea.getMaturity(), "贷款起息日", "贷款到期日"));
		if (ea.getExplbalainfos() != null) {
			for (int i = 0; i < ea.getExplbalainfos().size(); i++) {
				result.append(this.isEmpty(ea.getExplbalainfos().get(i).getExplcurr(), "质押外汇币种"));
				result.append(this.isAmountAndZero22_2(ea.getExplbalainfos().get(i).getExplamount(), "质押外汇金额"));
			}
		}
