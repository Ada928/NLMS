...
public class ReportCfaEBVaild extends AbsReportDataVaild {
	...
	public String executeDataVaild(Object obj) {
		...
		// 删除原因
		result.append(this.isDelRemarkVaild(eb.getActiontype(), eb.getActiondesc()));

		// EXPLRMBLONO 外汇质押人民币贷款编号
		result.append(this.valLenByNotNull(eb.getExplrmblono(), 28, "外汇质押人民币贷款编号"));
		result.append(this.exbuiSeNumVaild(eb.getApptype(), eb.getCurrentfile(), eb.getFiller2(), eb.getBrNo()));
		...
		result.append(this.isAmountAndZero22_2(eb.getCredrepayamount(), "本月还本金额"));
		...
		if (eb.getExplbalainfos() != null) {
			...
			for (int i = 0; i < eb.getExplbalainfos().size(); i++) {
				...
				result.append(this.isAmountAndZero22_2(eb.getExplbalainfos().get(i).getExplbala(), "质押外汇余额"));

				// EXPLPERAMOUNT 质押外汇履约金额 必填项，大于等于0
				result.append(this.isAmountAndZero22_2(eb.getExplbalainfos().get(i).getExplperamount(), "质押外汇履约金额"));

				// PLCOSEAMOUNT 质押履约结汇金额 必填项，大于等于0
				result.append(this.isAmountAndZero22_2(eb.getExplbalainfos().get(i).getPlcoseamount(), "质押履约结汇金额"));
			}
		}
