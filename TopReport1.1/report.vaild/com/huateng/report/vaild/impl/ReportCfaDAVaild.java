...
public class ReportCfaDAVaild extends AbsReportDataVaild {
	...
	public String executeDataVaild(Object obj) {
		...
		// 删除原因
		result.append(this.isDelRemarkVaild(da.getActiontype(), da.getActiondesc()));
		...
		if (da.getDebtortype().equals("1011")) {
			...
			result.append(this.isEmpty(da.getCfeogudad(), "债务人类型为“中资企业”时,中资企业境外担保项下贷款业务批准文件号"));

		}
		// 中资企业境外担保项下境内贷款额度币种 ,非必填项，债务人类型为“中资企业”时，为必填项
		if (da.getDebtortype().equals("1011")) {
			result.append(this.isEmpty(da.getCfeogudcurr(), "债务人类型为“中资企业”时,中资企业境外担保项下境内贷款额度币种"));
		}
		// 中资企业境外担保项下境内贷款额度金额 非必填项，大于等于0。债务人类型为“中资企业”时，为必填项。
		if (da.getDebtortype().equals("1011")) {
			result.append(this.isEmpty(da.getCfeogudamount(), "债务人类型为“中资企业”时,中资企业境外担保项下境内贷款额度金额"));
			result.append(this.isAmountAndZero22_2(da.getCfeogudamount(), "中资企业境外担保项下境内贷款额度金额"));
		...
		result.append(this.dateLessCheckDateOrEqual(da.getValuedate(), da.getMaturity(), "起息日", "到期日"));
		...
		result.append(this.exbuiSeNumVaild(da.getApptype(), da.getCurrentfile(), da.getFiller2(), da.getBrNo()));

		if (da.getFogucodeinfos() != null) {
			for (int i = 0; i < da.getFogucodeinfos().size(); i++) {
				result.append(this.TypeConfirmCode(da.getFogucodeinfos().get(i).getFogucode(), "境外担保人代码"));
				result.append(this.optionOne(da.getFogucodeinfos().get(i).getFoguname(),
						da.getFogucodeinfos().get(i).getFogunamen(), "境外担保人中英文名称"));
				result.append(this.isEmpty(da.getFogucodeinfos().get(i).getFogurecode(), "境外担保人注册地国家/地区代码"));
				result.append(this.isEmpty(da.getFogucodeinfos().get(i).getGuaranteetype(), "担保方式"));
