...
public class ReportBopDVaild extends AbsReportDataVaild {
	...
	public String executeDataVaild(Object obj) {
		...
		// if(ds.getLcyamt()==null||ds.getLcyamt().compareTo(new BigDecimal(0))
		// ==0){
		...
		// if(ds.getLcyamt()!=null&&ds.getLcyamt().compareTo(new
		// BigDecimal(0))>0&&StringUtils.isEmpty(ds.getLcyacc())){
		...
		// }
		// }
		// LCYACC人民币帐号/银行卡号 OTHAMT 结汇汇率 EXRATE LCYAMT 人民币帐号/银行卡号,结汇汇率,结汇金额 要么全为空
		// ,要么全有值
		...
		if (ds.getFcyamt() != null && ds.getFcyamt().compareTo(new BigDecimal(0)) > 0
				&& StringUtils.isEmpty(ds.getFcyacc())) {
		...
		if (ds.getOthamt() != null && ds.getOthamt().compareTo(new BigDecimal(0)) > 0
				&& StringUtils.isEmpty(ds.getOthacc())) {
		...
		// 国内银行扣费币种 INCHARGECCY 国内银行扣费金额 INCHARGEAMT
		// 若币种不为空则对应金额必须>0；若金额>0，则对应币种不能为空。
