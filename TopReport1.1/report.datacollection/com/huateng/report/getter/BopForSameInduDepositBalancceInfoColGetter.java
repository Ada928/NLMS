...
public class BopForSameInduDepositBalancceInfoColGetter extends BaseGetter {
	...
	private List getList() throws CommonException {
		...
		if ("newBalance".equalsIgnoreCase(op)) {
			list.add(bop);
		} else if ("modBalance".equalsIgnoreCase(op) || "delBalance".equalsIgnoreCase(op)
				|| "detailBalance".equalsIgnoreCase(op)) {
			...
			hql.append(
					"        bds.contractcurr,bds.floatrate,bds.anninrate,bds.spapfeboindex,bds.remark, bds.work_Date,");
			...
			hql.append(
					"       bcd.creditor_id,bcd.creditorcode, bcd.creditorname, bcd.creditornamen,bcd.creditortype,");
