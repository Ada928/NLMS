...
public class BOPForDebtUsanceLeOfCreditColGetter extends BaseGetter {
	...
	private List getData() throws CommonException {
		...
		if (NEW_CMD.equals(op)) {
			...
			bop.setExdebtcode(ReportUtils.getBussinessNo(TopReportConstants.REPORT_FILE_TYPE_CFA_AF));
			ReportUtils.setObjectPro(bop, TopReportConstants.REPORT_FILE_TYPE_CFA_AF);
			...
			if (null != exdebtds) {
				...
				bop.setInpriamount(exdebtds.getInpriamount());
				...
				bop.setInltcabuscode(exdebtds.getInltcabuscode());
				bop.setAppcode(exdebtds.getAppcode());
				bop.setAppname(exdebtds.getAppname());
