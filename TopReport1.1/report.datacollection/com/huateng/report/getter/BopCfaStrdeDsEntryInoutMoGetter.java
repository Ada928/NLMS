...
public class BopCfaStrdeDsEntryInoutMoGetter extends BaseGetter {

	public Result call() throws AppException {
		try {
			PageQueryResult pageResult = getData();
			setValue2DataBus(ReportConstant.QUERY_LOG_BUSI_NAME, "商业银行人民币结构性存款补录资金流出入和结购汇信息查询");
	...
	private PageQueryResult getData() throws CommonException {
		...
		if (StringUtils.isNotBlank(brNo)) {
			buff.append(" AND strde.brNo = ? ");
			paramentList.add(brNo);
		}

		buff.append(" AND strde.recStatus IN (? , ?) ");
		...
		paramentList.add(TopReportConstants.REPORT_FILE_TYPE_CFA_FD);
