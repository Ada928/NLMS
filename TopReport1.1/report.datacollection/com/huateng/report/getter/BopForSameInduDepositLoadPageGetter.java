...
public class BopForSameInduDepositLoadPageGetter extends BaseGetter {
	...
	private PageQueryResult getData() throws CommonException {
		...
		if (StringUtils.isNotBlank(qworkDate)) {
			hql.append(" and bds.work_Date ='").append(qworkDate).append("'");
		}
		if (StringUtils.isNotBlank(qactiontype)) {
			hql.append(" and bds.actiontype ='").append(qactiontype).append("'");
		}
		if (StringUtils.isNotBlank(qrecStatus)) {
			hql.append(" and bds.rec_Status ='").append(qrecStatus).append("'");
		}
		if (StringUtils.isNotBlank(qapproveStatus)) {
			hql.append(" and bds.approve_Status ='").append(qapproveStatus).append("'");
		}
		if (StringUtils.isNotBlank(qrepStatus)) {
			hql.append(" and bds.rep_Status ='").append(qrepStatus).append("'");
		}
		if (StringUtils.isNotBlank(qfiller2)) {
			hql.append(" and bds.filler2 like '%").append(qfiller2).append("%'");
		}

		hql.append(" and  (bds.rec_Status ='" + TopReportConstants.REPORT_RECSTATUS_01 + "' or  bds.rec_Status='"
				+ TopReportConstants.REPORT_RECSTATUS_02 + "')");
		hql.append(" and (bds.actiontype ='" + TopReportConstants.REPORT_ACTIONTYPE_A + "' or bds.actiontype ='"
				+ TopReportConstants.REPORT_ACTIONTYPE_C + "' )");
