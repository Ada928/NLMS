...
/**
 *
 * 外债信息 - 贵金属拆借 - 签约信息 Operation, 用于update delete insert
 * 
 * @author wenhao.chen
 * @version 1.0 2012-8-30
 *
 */
public class BOPForDebtNobleMetalLendingOperation extends BaseOperation {

	public static final String ID = "BOPForDebtNobleMetalLendingOperation";
	// 外债信息表cmd
	...
	private static final HtLog htLog = HtLogFactory.getLogger(BOPForDebtNobleMetalLendingOperation.class);
	...
	public void execute(OperationContext context) throws CommonException {
		...
		// 贵金属拆借-签约信息需要对 外债信息表 ：BOP_CFA_EXDEBT_DS,债权人信息表 :BOP_CFA_CREDITOR_DS
		// 进行处理
		// 贵金属拆借-签约信息 - 外债信息表 ,债权人信息表 update insert delete
		if (CMD_INSERT.equalsIgnoreCase(cmd)) {
			...
			reportCommonService.saveBiDataProcessLog(bpExdebt.getApptype(), bpExdebt.getCurrentfile(), bpExdebt.getId(),
					...
					"创建外债信息贵金属拆借签约信息        签约信息ID ：" + bpExdebt.getId());

			gi.addBizLog("Updater.log", new String[] { gi.getTlrno(), gi.getBrno(), "执行创建外债信息贵金属拆借签约信息" });
			htLog.info("Updater.log", new String[] { gi.getTlrno(), gi.getBrno(), "执行创建外债信息贵金属拆借签约信息" });
			...
			reportCommonService.saveBiDataProcessLog(bpExdebt.getApptype(), bpExdebt.getCurrentfile(), bpExdebt.getId(),
					...
					"修改外债信息贵金属拆借签约信息       签约信息ID ：" + bpExdebt.getId());

			gi.addBizLog("Updater.log", new String[] { gi.getTlrno(), gi.getBrno(), "执行修改外债信息贵金属拆借签约信息" });
			htLog.info("Updater.log", new String[] { gi.getTlrno(), gi.getBrno(), "执行修改外债信息贵金属拆借签约信息" });
			...
			reportCommonService.saveBiDataProcessLog(bpExdebt.getApptype(), bpExdebt.getCurrentfile(), bpExdebt.getId(),
					...
					"删除外债信息贵金属拆借签约信息      签约信息ID ：" + bpExdebt.getId());

			gi.addBizLog("Updater.log", new String[] { gi.getTlrno(), gi.getBrno(), "执行删除外债信息贵金属拆借签约信息" });
			htLog.info("Updater.log", new String[] { gi.getTlrno(), gi.getBrno(), "执行删除外债信息贵金属拆借签约信息" });
			...
			gi.addBizLog("Updater.log", new String[] { gi.getTlrno(), gi.getBrcode(), "执行外债信息贵金属拆借签约信息审核" });
			htLog.info("Updater.log", new String[] { gi.getBrcode(), gi.getTlrno(), "执行外债信息贵金属拆借签约信息审核" });
