...
public class BopCfaDofoexloDsService {
	...
	public synchronized static BopCfaDofoexloDsService getInstance() {
		return (BopCfaDofoexloDsService) ApplicationContextUtils.getBean(BopCfaDofoexloDsService.class.getName());
	}
	 ...
	 */
	public PageQueryResult pageQueryByEdit(int pageIndex, int pageSize, String currentfile, String qstartDate,
			String qendDate, String actiontype, String recStatus, String approveStatus, String repStatus,
	 ...
	 * @throws CommonException
	 */
	public PageQueryResult pageQueryByAudit(int pageIndex, int pageSize, String currentfile, String qstartDate,
			String qendDate, String actiontype, String recStatus, String approveStatus, String repStatus,
			String filler2, String brno) throws CommonException {
	...
	public PageQueryResult pageQueryByAlreadyAudit(int pageIndex, int pageSize, String currentfile, String workDate,
			String actiontype, String recStatus, String approveStatus, String repStatus, String qfiller2, String brno)
					throws CommonException {

		StringBuilder queryhql = new StringBuilder(" FROM BopCfaDofoexloDs WHERE currentfile = ? AND recStatus = ? ");
	...
	private PageQueryResult pageQueryByHql(int pageIndex, int pageSize, String workDate, String actiontype,
			...
			StringBuilder queryhql, List<Object> paraList, String startDate, String endDate) throws CommonException {
	...
	public PageQueryResult pageQueryByHql(int pageIndex, int pageSize, String currentfile, String workDate,
		...
		StringBuilder queryhql = new StringBuilder(" FROM BopCfaDofoexloDs WHERE currentfile = ? ");
	...
	public BopCfaDofoexloDs load(String uuid) throws CommonException {
		...
		List list = rootdao.queryByQL2List(" FROM BopCfaDofoexloDs WHERE id = ? ", new Object[] { uuid }, null);
		if (!list.isEmpty()) {
	...
	public void updateChangeInfo(BopCfaDofoexloDs bopcfa) throws CommonException {
		...
		List list = rootdao.queryByQL2List(
				...
				new Object[] { TopReportConstants.REPORT_FILE_TYPE_CFA_CB, bopcfa.getId(), bopcfa.getFiller1(),
						TopReportConstants.REPORT_ACTIONTYPE_D, bopcfa.getFiller1(),
						TopReportConstants.REPORT_ACTIONTYPE_D },
				null);
		if (!list.isEmpty()) {
			BopCfaDofoexloDs lastreportdata = (BopCfaDofoexloDs) list.get(0);
			if (bopcfa.getLoanopenbalan().compareTo(lastreportdata.getEndbalan()) != 0) {
				ExceptionUtil.throwCommonException("[期初余额]必须等于[上期末余额]");
			}
		}
		list = rootdao.queryByQL2List(" FROM BopCfaDofoexloDs WHERE currentfile = ? AND id = ? ",
				new Object[] { TopReportConstants.REPORT_FILE_TYPE_CFA_CA, bopcfa.getFiller1() }, null);
		if (!list.isEmpty()) {
			BopCfaDofoexloDs lastreportdata = (BopCfaDofoexloDs) list.get(0);
		...
		StringBuilder hql = new StringBuilder(" SELECT COUNT(*) FROM BopCfaDofoexloDs WHERE currentfile = '")
				.append(bopcfa.getCurrentfile()).append("' AND buscode = '").append(bopcfa.getBuscode()).append("' ")
				.append(" AND id <> '").append(bopcfa.getId()).append("' AND actiontype <> '")
	...
	public void delete(BopCfaDofoexloDs bopcfa) throws CommonException {
		ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
		int count = rootdao.queryByHqlToCount(" SELECT COUNT(*) FROM BopCfaDofoexloDs WHERE filler1 = '"
				+ bopcfa.getId() + "' AND currentfile = '" + TopReportConstants.REPORT_FILE_TYPE_CFA_CB
				+ "' AND recStatus <> '" + TopReportConstants.REPORT_RECSTATUS_07 + "' ");
		if (0 < count) {
	...
	public void saveChangeInfo(BopCfaDofoexloDs bopcfa) throws CommonException {
		...
		List list = rootdao.queryByQL2List(
				...
				new Object[] { TopReportConstants.REPORT_FILE_TYPE_CFA_CB, bopcfa.getFiller1(),
						TopReportConstants.REPORT_ACTIONTYPE_D, bopcfa.getFiller1(),
						TopReportConstants.REPORT_ACTIONTYPE_D },
				null);
		if (!list.isEmpty()) {
	...
	public void approved(List<BopCfaDofoexloDs> bopcfaList) throws CommonException {
		ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
