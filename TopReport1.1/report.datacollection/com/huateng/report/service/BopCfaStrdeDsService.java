...
public class BopCfaStrdeDsService {
	...
	public void bopCfaStrdeAd_add(BopCfaStrdeDs bopCfaStrdeDs) throws CommonException {
		...
		// 状态变化等信息
		/*
		 * 操作状态=A-创建 记录状态=02-编辑待确认 审核状态=00-未审核 回执状态=00-未返回 是否已成功上报=0-否
		 */
		...
		// 默认工作日期当天
		saveBopCfaStrdeDs
				.setWorkDate(DateUtil.dateToNumber(GlobalInfo.getCurrentInstanceWithoutException().getTxdate()));
		// 默认未上报
		saveBopCfaStrdeDs.setSubSuccess(TopReportConstants.REPORT_IS_SUB_SUCCESS_NO);
		 ...
		 * 保存前后台验证 add
		 */
		ReportDataVaildUtil.executeVaild(saveBopCfaStrdeDs.getApptype(), saveBopCfaStrdeDs.getCurrentfile(),
				saveBopCfaStrdeDs);
		...
		ReportCommonService.getInstance().saveBiDataProcessLog(appType, currentFile, recId, busiNo, execType,
				execResult, execRemark);
	...
	public void bopCfaStrdeAd_mod(BopCfaStrdeDs bopCfaStrdeDs) throws CommonException {
		...
		if (!TopReportConstants.REPORT_RECSTATUS_01.equals(recStatus)
				&& !TopReportConstants.REPORT_RECSTATUS_02.equals(recStatus)) {
			// 不是01或者02不能删除
		 ...
		 * 保存前后台验证 mod
		 */
		ReportDataVaildUtil.executeVaild(modBopCfaStrdeDs.getApptype(), modBopCfaStrdeDs.getCurrentfile(),
				modBopCfaStrdeDs);
	...
	public void bopCfaStrdeAd_del(BopCfaStrdeDs bopCfaStrdeDs) throws CommonException {
		...
		// 记录系统信息
		/*
		 * 操作状态=D-删除 记录状态=02-编辑待确认 审核状态=00-未审核 回执状态=00-未返回 是否已成功上报=不变化
		 */
	...
	public void bopCfaStrdeTerminate_add(BopCfaStrdeDs bopCfaStrdeDs) throws CommonException {
		...
		// 记录到数据处理记录表
		ReportCommonService.getInstance().saveBiDataProcessLog(TopReportConstants.REPORT_APP_TYPE_CFA,
				TopReportConstants.REPORT_FILE_TYPE_CFA_FB, uuid, bopCfaStrdeDs.getStrdecode(),
				TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_EDIT, "新增", "终止信息新增");
	...
	public void bopCfaStrdeTerminate_mod(BopCfaStrdeDs bopCfaStrdeDsTerminate) throws CommonException {
		...
		// BopCfaStrdeDs ds = dao.query(BopCfaStrdeDs.class,
		// bopCfaStrdeDsTerminate.getId());
		// String recStat = ds.getRecStatus();
		// if(!TopReportConstants.REPORT_RECSTATUS_01.equals(recStat) &&
		// !TopReportConstants.REPORT_RECSTATUS_02.equals(recStat)) {
		// //不是01或者02不能修改
		// ExceptionUtil.throwCommonException("该数据记录状态不是可编辑或者编辑待确认，无法修改");
		// }
		// 根据是否上报成功填写系统信息
		 ...
		 * 保存前后台验证 mod
		 */
		ReportDataVaildUtil.executeVaild(bopCfaStrdeDsTerminate.getApptype(), bopCfaStrdeDsTerminate.getCurrentfile(),
				bopCfaStrdeDsTerminate);

		dao.update(bopCfaStrdeDsTerminate);
		// 记录到数据处理记录表
		ReportCommonService.getInstance().saveBiDataProcessLog(TopReportConstants.REPORT_APP_TYPE_CFA,
				TopReportConstants.REPORT_FILE_TYPE_CFA_FB, bopCfaStrdeDsTerminate.getId(),
				bopCfaStrdeDsTerminate.getStrdecode(), TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_EDIT, "修改",
				"终止信息修改");
	}
	...
	public void bopCfaStrdeTerminate_del(BopCfaStrdeDs bopCfaStrdeDsTerminate) throws CommonException {
		...
		ReportCommonService.getInstance().saveBiDataProcessLog(TopReportConstants.REPORT_APP_TYPE_CFA,
				TopReportConstants.REPORT_FILE_TYPE_CFA_FB, bopCfaStrdeDsTerminate.getId(),
				bopCfaStrdeDsTerminate.getStrdecode(), TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_DEL, "删除",
				"终止信息删除");
	}
	...
	public void bopCfaStrdeInpay_add(BopCfaStrdeDs bopCfaStrdeDs) throws CommonException {
		...
		// 记录到数据处理记录表
		ReportCommonService.getInstance().saveBiDataProcessLog(TopReportConstants.REPORT_APP_TYPE_CFA,
				TopReportConstants.REPORT_FILE_TYPE_CFA_FC, uuid, bopCfaStrdeDs.getStrdecode(),
				TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_EDIT, "新增", "利息给付信息新增");
	}
	...
	public void bopCfaStrdeInpay_mod(BopCfaStrdeDs bopCfaStrdeDsInpay) throws CommonException {
		 ...
		 * 保存前后台验证 add
		 */
		ReportDataVaildUtil.executeVaild(bopCfaStrdeDsInpay.getApptype(), bopCfaStrdeDsInpay.getCurrentfile(),
				bopCfaStrdeDsInpay);

		dao.update(bopCfaStrdeDsInpay);
		// 记录到数据处理记录表
		ReportCommonService.getInstance().saveBiDataProcessLog(TopReportConstants.REPORT_APP_TYPE_CFA,
				TopReportConstants.REPORT_FILE_TYPE_CFA_FC, bopCfaStrdeDsInpay.getId(),
				bopCfaStrdeDsInpay.getStrdecode(), TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_EDIT, "修改",
				"利息给付信息修改");
	...
	public void bopCfaStrdeInpay_del(BopCfaStrdeDs bopCfaStrdeDsInpay) throws CommonException {
		...
		ReportCommonService.getInstance().saveBiDataProcessLog(TopReportConstants.REPORT_APP_TYPE_CFA,
				TopReportConstants.REPORT_FILE_TYPE_CFA_FC, bopCfaStrdeDsInpay.getId(),
				bopCfaStrdeDsInpay.getStrdecode(), TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_DEL, "删除",
				"利息给付信息删除");
	}
	...
	public void bopCfaStrdeInoutMo_add(BopCfaStrdeDs bopCfaStrdeDs) throws CommonException {
		...
		// 记录到数据处理记录表
		ReportCommonService.getInstance().saveBiDataProcessLog(TopReportConstants.REPORT_APP_TYPE_CFA,
				TopReportConstants.REPORT_FILE_TYPE_CFA_FD, uuid, null,
				TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_EDIT, "新增", "资金流出入和结购汇信息新增");
	}
	...
	public void bopCfaStrdeInoutMo_mod(BopCfaStrdeDs bopCfaStrdeDs) throws CommonException {
		...
		// 记录到数据处理记录表
		ReportCommonService.getInstance().saveBiDataProcessLog(TopReportConstants.REPORT_APP_TYPE_CFA,
				TopReportConstants.REPORT_FILE_TYPE_CFA_FD, bopCfaStrdeDs.getId(), null,
				TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_EDIT, "修改", "资金流出入和结购汇信息修改");
	}
	...
	public void bopCfaStrdeInoutMo_del(BopCfaStrdeDs bopCfaStrdeDs) throws CommonException {
		...
		ReportCommonService.getInstance().saveBiDataProcessLog(TopReportConstants.REPORT_APP_TYPE_CFA,
				TopReportConstants.REPORT_FILE_TYPE_CFA_FD, bopCfaStrdeDs.getId(), null,
				TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_DEL, "删除", "资金流出入和结购汇信息删除");
	}
}