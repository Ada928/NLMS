...
import com.huateng.report.operation.BOPForDebtUsanceLeOfCreditOperation;
 ...
 */
public class BOPForDebtUsanceLeOfCreditUpdate extends BaseUpdate {

	private static final String DATASET_ID = "BOPForDebtUsanceLeOfCreditCol";
		...
		if (updateResultBean.hasNext()) {
			...
			if (!StringUtils.isEmpty(op)) {
				if (RECORD_ADD.equalsIgnoreCase(op)) {
					...
					bpExdebt.setAppcode(bpExdebtTemp.getAppcode());
					bpExdebt.setAppname(bpExdebtTemp.getAppname());
					...
					bpExdebt.setCurrentfile(TopReportConstants.REPORT_FILE_TYPE_CFA_AF);
					...
					oc.setAttribute(BOPForDebtUsanceLeOfCreditOperation.CMD,
							BOPForDebtUsanceLeOfCreditOperation.CMD_INSERT);
					...
					oc.setAttribute(BOPForDebtUsanceLeOfCreditOperation.CMD,
							BOPForDebtUsanceLeOfCreditOperation.CMD_UPDATE);
					...
					oc.setAttribute(BOPForDebtUsanceLeOfCreditOperation.CMD,
							BOPForDebtUsanceLeOfCreditOperation.CMD_DELETE);

				}
			}
		}
		oc.setAttribute(BOPForDebtUsanceLeOfCreditOperation.IN_PARAM_EXDEBT, bpExdebt);
		oc.setAttribute(BOPForDebtUsanceLeOfCreditOperation.IN_PARAM_BCC, bcc);
		OPCaller.call(BOPForDebtUsanceLeOfCreditOperation.ID, oc);
