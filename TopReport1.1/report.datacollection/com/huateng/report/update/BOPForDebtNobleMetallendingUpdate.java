 ...
 */
public class BOPForDebtNobleMetallendingUpdate extends BaseUpdate {

	private static final String DATASET_ID = "BOPForDebtNobleMetalLendingCol";
		...
		if (updateResultBean.hasNext()) {
			...
			if (!StringUtils.isEmpty(op)) {
				if (RECORD_ADD.equalsIgnoreCase(op)) {
					...
					bpExdebt.setFiller2(bpExdebtTemp.getFiller2());// 增加 业务流水号
					...
					bpExdebt.setCurrentfile(TopReportConstants.REPORT_FILE_TYPE_CFA_AH);
					...
					oc.setAttribute(BOPForDebtNobleMetalLendingOperation.CMD,
							BOPForDebtNobleMetalLendingOperation.CMD_INSERT);
					...
					bpExdebt.setSpapfeboindex(bpExdebtTemp.getSpapfeboindex());
					bpExdebt.setRemark(bpExdebtTemp.getRemark());
					bpExdebt.setFiller2(bpExdebtTemp.getFiller2());// add 业务流水号
					bpExdebt.setWorkDate(gInfo.getTxdate().toString().replaceAll("-", ""));
					...
					oc.setAttribute(BOPForDebtNobleMetalLendingOperation.CMD,
							BOPForDebtNobleMetalLendingOperation.CMD_UPDATE);
					...
					oc.setAttribute(BOPForDebtNobleMetalLendingOperation.CMD,
							BOPForDebtNobleMetalLendingOperation.CMD_DELETE);
				}
			}
		}
		oc.setAttribute(BOPForDebtNobleMetalLendingOperation.IN_PARAM_EXDEBT, bpExdebt);
		oc.setAttribute(BOPForDebtNobleMetalLendingOperation.IN_PARAM_BCC, bcc);
