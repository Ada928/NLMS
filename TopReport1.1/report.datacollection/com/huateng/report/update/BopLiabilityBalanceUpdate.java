...
public class BopLiabilityBalanceUpdate extends BaseUpdate {
	...
	public UpdateReturnBean saveOrUpdate(MultiUpdateResultBean arg0, HttpServletRequest arg1, HttpServletResponse arg2)
		...
		// 返回结果对象
		UpdateResultBean updateResultBeanExguAdd = multiUpdateResultBean.getUpdateResultBeanByID(DATASET2_ID);
		...
		if (updateResultBeanExguAdd.hasNext()) {
			...
			if (!StringUtils.isEmpty(op)) {
				...
				if (RECORD_ADD.equalsIgnoreCase(op)) {
					...
					bopCfaExguDs.setCurrentfile(TopReportConstants.REPORT_FILE_TYPE_CFA_BB);
					...
					oc.setAttribute(BopLiabilityBalanceOperation.CMD, BopLiabilityBalanceOperation.CMD_INSERT);
					...
					oc.setAttribute(BopLiabilityBalanceOperation.CMD, BopLiabilityBalanceOperation.CMD_UPDATE);
					oc.setAttribute(BopExguDsOperation.IN_PARAM_TOR, bopExguTorDs);
					...
					oc.setAttribute(BopLiabilityBalanceOperation.CMD, BopLiabilityBalanceOperation.CMD_DELETE);
		...
		oc.setAttribute(BopLiabilityBalanceOperation.IN_PARAM_EXGU, bopCfaExguDs);
