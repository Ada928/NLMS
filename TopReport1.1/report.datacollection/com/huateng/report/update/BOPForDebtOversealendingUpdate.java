...
import com.huateng.report.operation.BOPForDebtOverseaLendingOperation;
 ...
 */
public class BOPForDebtOversealendingUpdate extends BaseUpdate {

	private static final String DATASET_ID = "BOPForDebtOverseaLendingCol";
		...
		if (updateResultBean.hasNext()) {
			...
			if (!StringUtils.isEmpty(op)) {
				if (RECORD_ADD.equalsIgnoreCase(op)) {
					...
					bpExdebt.setCurrentfile(TopReportConstants.REPORT_FILE_TYPE_CFA_AC);
					...
					oc.setAttribute(BOPForDebtOverseaLendingOperation.CMD,
							BOPForDebtOverseaLendingOperation.CMD_INSERT);
					...
					oc.setAttribute(BOPForDebtOverseaLendingOperation.CMD,
							BOPForDebtOverseaLendingOperation.CMD_UPDATE);
					...
					oc.setAttribute(BOPForDebtOverseaLendingOperation.CMD,
							BOPForDebtOverseaLendingOperation.CMD_DELETE);
				}
			}
		}
		oc.setAttribute(BOPForDebtOverseaLendingOperation.IN_PARAM_EXDEBT, bpExdebt);
		oc.setAttribute(BOPForDebtOverseaLendingOperation.IN_PARAM_BCC, bcc);
		OPCaller.call(BOPForDebtOverseaLendingOperation.ID, oc);
