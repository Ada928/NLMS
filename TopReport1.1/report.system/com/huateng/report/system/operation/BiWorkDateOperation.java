	...
	public void execute(OperationContext context) throws CommonException {
		...
		if (OP_SAVE.equals(cmd)) {
			...
			if (!tls.isNeedApprove(ReportEnum.REPORT_TASK_FUNCID.TASK_100599.value)) {
				...
				try {
					SysTaskInfo syst = ReportTaskUtil.getSysTaskInfoBean(REPORT_TASK_FUNCID.TASK_100599.value, updCd,
							biWorkdateConf, year, oldst);
			...
			/*
			 * BiWorkDateService workDateService =
			 * BiWorkDateService.getInstance();
			 * workDateService.updateWorkDate(year, workDateStrs);
			 * gi.addBizLog("Updater.log", new
			 * String[]{gi.getTlrno(),gi.getBrcode(),"修改工作日期维护"});
			 * htlog.info("Updater.log", new
			 * String[]{gi.getBrcode(),gi.getTlrno(),"修改工作日期维护"});
			 */
			...
			CutoverWorkDateBean cutoverWorkDateBean = (CutoverWorkDateBean) context
					.getAttribute(IN_CUTOVERWORKDATEBEAN);
