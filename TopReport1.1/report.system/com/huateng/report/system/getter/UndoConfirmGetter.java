...
public class UndoConfirmGetter extends BaseGetter {
	...
	public Result call() throws AppException {
		try {
			this.setValue2DataBus(ReportConstant.QUERY_LOG_BUSI_NAME, "系统维护-主管确认查询");
	...
	private PageQueryResult getData(List codesList) throws AppException {
		...
		if (StringUtils.isNotBlank(insCd)) {
			hql.append(" and dd.insCd='").append(insCd).append("'");
		}
		if (StringUtils.isNotBlank(intInsId)) {
			hql.append(" and dd.intInsId='").append(intInsId).append("'");
		}
		if (StringUtils.isNotBlank(updTransCd)) {
			hql.append(" and dd.updTransCd='").append(updTransCd).append("'");
		}
		hql.append(" order by dd.id  desc");
		return UndoConfirmService.getInstance().list(pageIndex, pageSize, hql.toString());
	}
}
