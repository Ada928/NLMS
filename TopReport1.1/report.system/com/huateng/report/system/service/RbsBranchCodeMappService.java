...
public class RbsBranchCodeMappService {
	...
	public synchronized static RbsBranchCodeMappService getInstance() {
		return (RbsBranchCodeMappService) ApplicationContextUtils.getBean(RbsBranchCodeMappService.class.getName());
	}
	 ...
	 */
	public PageQueryResult pageQueryByHql(int pageIndex, int maxRows, String qrbsbranchcode, String qbusitype,
			String qbranchcode) throws CommonException {
	...
	public void rbs_mod(RbsBranchCodeMapp bean) throws CommonException {
		...
		// boolean flag = hasExistsThisRbsBranchcode(bean.getBusitype(),
		// bean.getRbsbranchcode());
		RbsBranchCodeMapp beanInTable = dao.query(RbsBranchCodeMapp.class, bean.getId());
		if (!bean.getBusitype().equalsIgnoreCase(beanInTable.getBusitype())
				|| !bean.getRbsbranchcode().equalsIgnoreCase(beanInTable.getRbsbranchcode())) {
			// 被改动过,判断是否在表中存在
			boolean flag = hasExistsThisRbsBranchcode(bean.getBusitype(), bean.getRbsbranchcode());
			if (flag) {
				// 已经存在，不让修改
				String key = "要保存的RBS机构号[" + bean.getRbsbranchcode() + "]," + "业务类型：[";
				String bop = TopReportConstants.REPORT_BUSITYPE_BOP.equalsIgnoreCase(bean.getBusitype()) ? "BOP"
						: bean.getBusitype();
