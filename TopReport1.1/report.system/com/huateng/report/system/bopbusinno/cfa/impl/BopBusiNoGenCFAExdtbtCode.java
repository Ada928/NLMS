...
public class BopBusiNoGenCFAExdtbtCode implements IGenBopBusinessNo {
	...
	public void updateBopBusiNo(Map<String, Object> paramMap) throws CommonException {
		...
		List list = rootdao.queryByQL2List(
				" from BopCfaExdebtDs where filler1='" + recId + "' and exdebtcode<>'" + exdebt.getExdebtcode() + "'");
		...
		Object maxObj = rootdao.queryByHqlMax("select max(changeno) from BopCfaExdebtDs where  filler1='" + recId
		...
		List seqList = rootdao.queryByQL2List(
				" from BopCfaExdebtDs where filler1='" + recId + "' and changeno='" + seqTemp + "' order by crtTm");
