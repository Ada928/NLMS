...
public class BopBusiNoGenCFAStrdeCode implements IGenBopBusinessNo {
	...
	public void updateBopBusiNo(Map<String, Object> paramMap) throws CommonException {
		...
		List list = rootdao.queryByQL2List(
				" from BopCfaStrdeDs where filler1='" + recId + "' and strdecode<>'" + strde.getStrdecode() + "'");
		...
		Object maxObjter = rootdao.queryByHqlMax("select max(terpaycode) from BopCfaStrdeDs where  filler1='" + recId
				+ "' and  terpaycode<>'" + seqTemp + "'");
		if (maxObjter != null) {
			terpay = Integer.parseInt(maxObjter.toString()) + 1;
		}
		List terpaySeqList = rootdao.queryByQL2List(
				" from BopCfaStrdeDs where filler1='" + recId + "' and terpaycode='" + seqTemp + "' order by crtTm");
		...
		// 查询付息序号
		int inpay = 1;
		Object maxObj = rootdao.queryByHqlMax("select max(inpaycode) from BopCfaStrdeDs where  filler1='" + recId
				+ "' and  inpaycode<>'" + seqTemp + "'");
		if (maxObj != null) {
			inpay = Integer.parseInt(maxObj.toString()) + 1;
		}
		List inpayList = rootdao.queryByQL2List(
				" from BopCfaStrdeDs where filler1='" + recId + "' and inpaycode='" + seqTemp + "' order by crtTm");
