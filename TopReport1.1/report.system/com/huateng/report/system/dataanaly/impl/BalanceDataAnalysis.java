...
public class BalanceDataAnalysis extends DataAnalysisTool {
	/**
	 * 查询从上个工作日期后当当天的所有余额数据, 得到这段时间内的 累积 收入和支出的总额。 按 branchcode, partynumber,
	 * currencycode 分组 后 将 totaldtbal， totalcrbal 两列分别 进行 加总
	 */
	...
	/**
	 * 查询 从上个工作日期后当当天的所有余额记录的总条数 如果 branchcode, partynumber, currencycode
	 * 相同则为同一条
	 */
	...
	public Map<String, Object> executeAnalysis(Map<String, Object> paramMap) throws CommonException {
		...
		/** 将上一工作日期格式化成字符串 */
		String lastworkingdate = DateFormatUtils.format(com.huateng.ebank.framework.util.DateUtil.getBhDate(),
				DATE_FORMAT);
		/** 设置查询条件 */
		...
		for (int i = 1; i <= pagenumber; i++) {
			...
			/** 获取当前工作日期的余额 */
			Iterator<RbsDsBiBalance> balanceiter = rootdao.queryByQL(SEARCH_TODAY_BALANCE + condition,
					new Object[] { CNY_CURRENCY, workdate }, null);
			String pk = null;
	 ...
	 * @return
	 */
	private BopAccDs getBalanceData(String workdate, AccountDataAnalysis analysis, Map<String, String> branchCodeMap,
	 ...
	 */
	private int getPageNumber(ROOTDAO rootdao, Object[] paraments) throws CommonException {
		/** 查询当天账户余额记录的总记录数 */
		int totalrecords = Integer
				.parseInt(String.valueOf(rootdao.queryBySQL(SEARCH_COUNT_TODAY_DR_CR, paraments, null).next()));
	 ...
	 */
	@SuppressWarnings("unchecked")
	private Map<String, String> getAccountDataByAccountno(ROOTDAO rootdao, Collection<String> accountList)
			throws CommonException {

		String condition = ReportUtils.toInString(accountList);
		Iterator<BopAccDs> iter = rootdao.queryByQL(SEARCH_ACCOUNT_BY_BALANCE + condition,
				new Object[] { TopReportConstants.REPORT_FILE_TYPE_CFA_CA }, null);
		Map<String, String> accountMap = new HashMap<String, String>();
