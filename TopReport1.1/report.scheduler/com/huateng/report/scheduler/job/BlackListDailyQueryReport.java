 ...
 * 生成定时任务
 */
public class BlackListDailyQueryReport implements Job {
		...
		if ("ON".equalsIgnoreCase(onOff)) {
			...
			try {
				...
				if (isdo) {
					...
					log.info("==定时生成查询统计数据(每日)begin==");
					BlackListQueryReportTask blackListQueryReportTaskTask = new BlackListQueryReportTask();
					blackListQueryReportTaskTask.dailyBlackListQueryCount();
					log.info("==定时生成查询统计数据(每日)end==");
					endTm = new Date();
				...
				e.printStackTrace();
				log.error("定时生成查询统计数据(每日)异常" + e.getMessage());
				serContext.setAttribute(BOP_IMPORT_ISFIN, true);
				result = ReportEnum.REPORT_RESULT.FAILD.value;
				remark += ":定时生成查询统计数据(每日)异常," + e.getMessage();
			}
