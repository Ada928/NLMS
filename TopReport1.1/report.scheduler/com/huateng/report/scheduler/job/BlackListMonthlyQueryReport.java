 ...
 * 生成定时任务
 */
public class BlackListMonthlyQueryReport implements Job {
	...
	public void execute(JobExecutionContext context)
		...
		if ("ON".equalsIgnoreCase(onOff)) {
			...
			String remark = "";
			try {
				serContext = (ServletContext) context.getScheduler()
						.getContext().get("serContext");
				jobId = context.getJobDetail().getJobDataMap()
						.getString("jobId");// 定时器主键
				ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
				ReportJobConfig jobConfig = rootdao.query(
						ReportJobConfig.class, jobId);
				jobName = jobConfig.getJobName();
				boolean isdo = ReportJobUtil.isDoTaskJob(serContext,
						BOP_IMPORT_ISFIN, jobConfig);
				remark = jobConfig.getJobRemark();
				if (isdo) {
					startTm = new Date();
					serContext.setAttribute(BOP_IMPORT_ISFIN, false);
					log.info("==定时生成查询统计数据(每月)begin==");
					BlackListQueryReportTask blackListQueryReportTaskTask = new BlackListQueryReportTask();
					blackListQueryReportTaskTask.monthlyBlackListQueryCount();
					log.info("==定时生成查询统计数据(每月)end==");
					endTm = new Date();
					result = ReportEnum.REPORT_RESULT.SUCCESS.value;
					serContext.setAttribute(BOP_IMPORT_ISFIN, true);

					// 切换globalinfo系统日期
					GlobalInfoService globalService = GlobalInfoService
							.getInstance();
				...
				e.printStackTrace();
				log.error("定时生成查询统计数据(每月)异常" + e.getMessage());
				serContext.setAttribute(BOP_IMPORT_ISFIN, true);
				result = ReportEnum.REPORT_RESULT.FAILD.value;
				remark += ":定时生成查询统计数据(每月)异常," + e.getMessage();
			}
