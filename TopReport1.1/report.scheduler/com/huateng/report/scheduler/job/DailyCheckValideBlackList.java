...
import com.cibfintech.blacklist.Scheduler.BankBlackListAutoValidChange;
 ...
 * 生成定时任务
 */
public class DailyCheckValideBlackList implements Job {
		...
		if ("ON".equalsIgnoreCase(onOff)) {
			...
			try {
				...
				if (isdo) {
					...
					log.info("==定时检查黑名单是否到期(每日)begin==");
					BankBlackListAutoValidChange task = new BankBlackListAutoValidChange();
					task.dailyCheckValidBlackList();
					log.info("==定时检查黑名单是否到期(每日)end==");
					endTm = new Date();
				...
				e.printStackTrace();
				log.error("定时检查黑名单是否到期(每日)异常" + e.getMessage());
				serContext.setAttribute(BOP_IMPORT_ISFIN, true);
				result = ReportEnum.REPORT_RESULT.FAILD.value;
				remark += ":定时检查黑名单是否到期(每日)异常," + e.getMessage();
			}
