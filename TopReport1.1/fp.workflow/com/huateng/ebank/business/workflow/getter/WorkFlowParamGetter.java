...
public class WorkFlowParamGetter extends BaseGetter {
	...
	protected PageQueryResult getData() throws Exception {
		...
		List workFlowParamList = (List) OperationContext.getValue(oc, WorkFlowParamOperation.OUT_WORKFLOWPARAM_LIST);
		...
		if (workFlowParamList != null && workFlowParamList.size() > 0) {
			for (int i = 0; i < workFlowParamList.size(); i++) {
				...
				listbrclass = DAOUtils.getDataDicDAO().queryByCondition(
						"po.dataTypeNo=" + SystemConstant.DATADIC_TYPE_BRCLASS + " and po.dataNo=" + brclass);
				...
				listbizclass = DAOUtils.getDataDicDAO().queryByCondition("po.dataTypeNo="
						+ SystemConstant.DATADIC_TYPE_LIMIT_TYPE + "and po.dataNo=" + wfw.getBizClass());
				...
				if (!DataFormat.isEmpty(roleId)) {
					wfw.setWorkflowRoleName(
							roleId + "-" + DAOUtils.getRoleInfoDAO().query(new Integer(roleId)).getRoleName());
				} else {
					wfw.setWorkflowRoleName("");
				}
