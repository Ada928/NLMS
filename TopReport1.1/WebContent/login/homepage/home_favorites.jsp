...
<body bgcolor="white" style="margin: 0px;">
	<center>
		<div style="padding: 3px; text-align: center;">
			...
			%>
			<div class="menudiv" id="div_<%=fun.getId().trim()%>" style="cursor: pointer;"
				onclick="doIndexWork('<%=fun.getId().trim()%>','<%=fun.getFuncname()%>','<%=fun.getPagepath()%>')"
				onmouseover="this.style.backgroundColor='#cce6f9'" onmouseout="this.style.backgroundColor=''"
				title="<%=fun.getFuncname()%>"><%=fun.getFuncname()%></div>
			<%
				}
				} else {
			%>
			<div
				...
				onclick="window.parent.maxFrm(0)">请点击设置收藏夹</div>
			<%
				}
			%>
