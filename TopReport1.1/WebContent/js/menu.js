/*----------------------------------------------------------------------------\
|                             DHTML Menu 4.30     by Liu Shengbin             |
|-----------------------------------------------------------------------------|
...
// menuCache
//
/* add by zhaozhiguo �Ƿ���post��ʽ��funcid������̨ begin*/
var Menu_postFuncId = true;
/* add by zhaozhiguo �Ƿ���post��ʽ��funcid������̨ end*/
var menuCache = {
	_count:		0,
...
// BuildMenu
//
function Menu_buildMenu(node,bar)//init treeView from array
{
	...
	while( index <= nLen )
	{
		...
		switch( node[index] )
		{
			...
			case 1:	
				...
				break;
			case 2:		
				var menutemp = menuarray[count];
				menutemp.add(tmpItem =new MenuItem(node[index+1] ,node[index+7], null, null, node[index+2]));
				tmpItem.target="main";	
				break;
			case 0:	
				count++;			
				menuarray[count] = new Menu();	
				if(count == 1)			
				{
					var menutemp = menuarray[count-1];
					menutemp.add(new MenuButton( node[index+1],menuarray[count] ));
				}
				else
				{
					var menutemp = menuarray[count-1];
					menutemp.add(new MenuItem( node[index+1],null,null,menuarray[count], node[index+2] ));
				}	
				break;
		}
		index ++;
...
//Menu.prototype.cssFile = "css/menu.css";
//Menu.prototype.cssText = null;
/*Menu.prototype.cssText = ".menu-body {background-color:	#0257CC;color:white;margin:0;padding:0;	overflow:hidden;border:0;cursor:default;}"+
".menu-body .outer-border {border:1px solid;border-color:ThreeDLightShadow ThreeDDarkShadow ThreeDDarkShadow ThreeDLightShadow;}"+
".menu-body .inner-border {border:1px solid;border-color:#636563 #636563 #636563 #636563; padding:1px;width:100%;height:100%;}"+
".menu-body td {font-family: \"??\", Arial;font-size: 12px;}"+
".menu-body .hover {background-color:blue;color:white;}"+
".menu-body td.empty-icon-cell {padding:	0px 2px 0px 2px;border:0;}"+
".menu-body td.empty-icon-cell span {width:5px;}"+
".menu-body td.icon-cell {padding:0px 2px 0px 2px;border:0;}"+
".menu-body td.icon-cell img {width:16px;height:16px;margin:0;}"+
".menu-body td.label-cell {width:100%;padding:1px 5px 0px 0px;font-size:	12px;}"+
".menu-body td.shortcut-cell {padding:0px 5px 0px 5px;}"+
".menu-body td.arrow-cell {width:20px;padding:0px 2px 0px 0px;font-family:Webdings;font-size:80%;}"+
".menu-body .disabled .disabled-container {color:GrayText;}"+
".menu-body .disabled .icon-cell .disabled-container,.menu-body .disabled-hover .icon-cell .disabled-container {background:GrayText;filter:progid:DXImageTransform.Microsoft.Chroma(Color=#010101) DropShadow(color=ButtonHighlight, offx=1, offy=1);width:100%;height:100%;}"+
".menu-body .disabled-hover .icon-cell .disabled-container {filter:progid:DXImageTransform.Microsoft.Chroma(Color=#010101);}"+
".menu-body .disabled .icon-cell .disabled-container .disabled-container,.menu-body .disabled-hover .icon-cell .disabled-container .disabled-container {background:Transparent;filter:Mask(Color=#010101);}"+
".menu-body .disabled-hover td {background-color:Highlight;color:GrayText;}"+
".menu-body td.separator {font-size:0.001mm;padding:4px 10px 4px 10px;}"+
".menu-body .separator-line {overflow:hidden;border-top:1px solid ThreeDShadow;border-bottom:1px solid ThreeDHighlight;height:2px;}"+
".menu-body #scroll-up-item td,.menu-body #scroll-down-item td {font-family:Webdings;text-align:center;padding:10px;}"+
".menu-body #scroll-up-item,.menu-body #scroll-down-item {width:100%;}"+
".menu-body #scroll-up-item td,.menu-body #scroll-down-item td {font-family:Webdings;text-align:center;padding:0px 5px 0px 5px;font-size:10px;}"+
".menu-bar {background:#0257CC;cursor:default;padding:0px 1px;}"+
".menu-bar .menu-button {padding-right: 7px;padding-left: 7px;padding-bottom: 2px;padding-top: 3px;background-color: #0257CC;border-top: 1px solid #d6d3ce;border-right: 1px solid #d6d3ce;border-bottom: 1px solid #d6d3ce;border-left: 1px solid #d6d3ce;display:inline-block;font-size:12px;color:#FFFFFF;font-family:\"??\", Arial;margin:1px;white-space:nowrap;cursor:default;}"+
".menu-bar .menu-button.active {padding:3px 7px 2px;border:1px solid;border-right: #636563 1px solid;border-top: #636563 1px solid;border-left: #636563 1px solid;cursor: default;border-bottom: #636563 1px solid;background-color: #0257CC;}"+
".menu-bar .menu-button.hover {background:blue;padding:3px 7px 2px;border:1px solid;border-right: #08246b 1px solid;border-top: #08246b 1px solid;border-left: #08246b 1px solid;border-bottom: #08246b 1px solid;background-color: #0000FF;}"+
".menu-bar .menu-button.disabled,.menu-bar .menu-button.disabled-hover {color:GrayText;border:0 ;padding:3px 7px 2px;}";
*/
...
function MenuItem( sLabelText, fAction, sIconSrc, oSubMenu , funcid) {
	// public
	this.icon = sIconSrc || "";
...
//						if (menuMain) {
//							form["__FuncId"].value = this.funcid;
//						}
//						form.submit();
						window.parent.parent.doWork(this.funcid,this.text,this.action);
					} catch (e) {
						alert("menu.js:"+e);
					}
				} else {
...
//
function MenuButton( sLabelText, oSubMenu ,faction) {
	this.MenuItem = MenuItem;
...
////////////////////////////////////////////////////////////////////////////////////
// MenuButtonUrl extends MenuItem
//
function MenuButtonUrl( sLabelText, fAction ) {
	this.MenuItem = MenuItem;
	this.MenuItem( sLabelText, fAction, null, null );
...
MenuButtonUrl.prototype = new MenuItem;

MenuButtonUrl.prototype.scrollIntoView = function () {};
MenuButtonUrl.prototype.toHtml = function () {
	var cssClass = this.getCssClass();
	var toolTip = this.getToolTip();
...
MenuButtonUrl.prototype.getCssClass = function () {
	if ( this.disabled && this._selected )
		return "menu-button disabled-hover";
	else if ( this.disabled )
		return "menu-button disabled";
...
var eventListeners = {
	menu: {
		onkeydown:	function ( id ) {
			/*var oThis = menuCache[id];   //delete key event by liu-----------------------------------
		...
		onmouseup:	function ( id ) {
			...
			top.document.title = srcEl.outerText;   //++++++++++ title 
		},
