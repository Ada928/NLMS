...
<img src="<%=request.getContextPath()%>/images/tile.jpg" align="middle" width="984" height="83" />
 ...
 -->
	<script language="javascript">
		...
		<%StringBuffer m = (StringBuffer) request.getSession().getAttribute(
					"menu");%>
	...
	<div align="right" class="welcome">
		<div align="right">
			操作员号：【<%=request.getSession().getAttribute("tlrno")%>】 操作号名：【<%=request.getSession().getAttribute("tlrname") + ""%>】
			交易日期：【<%=request.getSession().getAttribute("busidate")%>】 最后登陆时间：【<%=request.getSession().getAttribute("lastlogintime")%>】
		</div>
	</div>
