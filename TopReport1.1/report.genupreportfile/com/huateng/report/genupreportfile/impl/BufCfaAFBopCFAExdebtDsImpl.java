...
public class BufCfaAFBopCFAExdebtDsImpl implements IGetSubFileList {
	...
	public List getSubFileResultList(Map<String, Object> paramMap) throws CommonException {
		...
		for (int i = 0; i < ob.size(); i++) {
			String torHql = "from BopCfaCreditorDs model where model.recId = '" + ob.get(i).getId() + "'";
			BopCfaCreditorDs bopCfaCreditorDs = (BopCfaCreditorDs) dao.queryByQL2List(torHql).get(0);
