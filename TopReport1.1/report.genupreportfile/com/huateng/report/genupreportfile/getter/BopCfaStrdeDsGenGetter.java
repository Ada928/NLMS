...
/**
 * @author zhuhongyong 商业银行人民币结构性存款生成上报文件
 */
public class BopCfaStrdeDsGenGetter extends BaseGetter {
	...
	public Result call() throws AppException {
		...
		try {
			...
			if (GET_TYPE_CONTRACT.equalsIgnoreCase(getType))
				logValue = "商业银行人民币结构性存款签约信息上报文件页面查询";
			if (GET_TYPE_TERMINATE.equalsIgnoreCase(getType))
				logValue = "商业银行人民币结构性存款终止信息上报文件页面查询";
			if (GET_TYPE_INPAY.equalsIgnoreCase(getType))
				logValue = "商业银行人民币结构性存款利息给付信息上报文件页面查询";
			if (GET_TYPE_INOUTMO.equalsIgnoreCase(getType))
				logValue = "商业银行人民币结构性存款资金流出入和结购汇信息信息上报文件页面查询";
	...
	private PageQueryResult getData() throws CommonException {
		...
		return BopCfaStrdeDsGenService.getInstance().pageQueryByHql(getType, qactiontype, qbrNo, qfiller2, pageSize,
				pageIndex);
	}
}
