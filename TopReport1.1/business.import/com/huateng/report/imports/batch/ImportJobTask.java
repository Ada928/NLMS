...
public class ImportJobTask {
	...
	/**
	 * 文件导入及分析导入步骤： 1：检查后台导数是否执行 2：检查后台导数是否正确完成 3：前台导数 4：检查前台导数是否正确完成 5：执行数据分析
	 * 
	...
	public void doBiz() throws CommonException {
		...
		if (errDir.isDirectory() && errDir.exists()) {
			if (errDir.listFiles().length > 0) {
				// ExceptionUtil.throwCommonException("后台执行batch.sh出错，请检查对应" +
				// filePath + "文件夹下的文件！");
			}
		} else {
			...
			try {
				...
				for (File file : dir.listFiles()) {
					if (file.isFile()) {
						...
						list = ROOTDAOUtils.getROOTDAO().queryByQL2List("from BiImportFileConfig where fileName = '"
								+ bean.getFileName() + "' and status = '" + Constants.YES + "'");
						if (list.isEmpty()) {
						...
						for (Object obj : list) {
							try {
								...
								bean.setImpStatus(FileImportService.getInstance()
										.getImportLogStatus(bean.getFileNameFull(), bean.getTableName(), workdate).getImportStatus());
							} catch (Exception e) {
