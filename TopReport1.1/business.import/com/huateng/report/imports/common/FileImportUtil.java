...
public class FileImportUtil {
	...
	public static String getWorkDate(String workdate) throws CommonException {
		if (DataFormat.isEmpty(workdate)) {
			workdate = DataFormat.dateToNumber(GlobalInfo.getCurrentInstance().getTxdate());
		}
		return workdate;
	}
	...
	public static String getFileNameFull(String workdate, String filename) {
		...
		if (ind == -1) {
			...
			if (filename.endsWith("xls")) {
				filepath = filename.substring(0, ind) + "_" + workdate + filename.substring(ind);
			} else {
				filepath = filename.substring(0, ind - 4) + "." + workdate + filename.substring(ind - 4);
			}
	...
	public static String getFileNameFullWithout(String workdate, String filename) {
		...
		if (ind == -1) {
			...
			filepath = filename.substring(0, ind) + workdate + filename.substring(ind);
		}
		return filepath;
	}
	 ...
	 * @throws CommonException
	 */
	public static boolean isExist(String workdate, String filename) throws CommonException {
		File f = new File(FileImportService.getInstance().getDefaultImportPath() + File.separator + workdate
				+ File.separator + filename);
		return f.isDirectory() ? false : f.exists();
	}

	// 工作日期的格式为 121030
	public static boolean isExistWithout(String workdate, String filename) throws CommonException {
		if (null != workdate && !"".equals(workdate)) {
			workdate = workdate.substring(2, 8);
		}
		File f = new File(FileImportService.getInstance().getDefaultImportPath() + File.separator + workdate
	...
	public static String getFilePath(String workdate) throws CommonException {
		return FileImportService.getInstance().getDefaultImportPath() + File.separator + workdate + File.separator;
	}
