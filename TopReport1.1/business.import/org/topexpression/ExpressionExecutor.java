...
public class ExpressionExecutor {
	...
	/**
	 * An expression of grammatical analysis, converts it into Token object
	 * queue
	 * 
	 * @param expression
	 * @return
	...
	public Constant execute(List<ExpressionToken> _RPNExpList) throws IllegalExpressionException {
		...
		for (ExpressionToken expToken : _RPNExpList) {
			...
			if (ExpressionToken.ETokenType.ETOKEN_TYPE_CONSTANT == expToken.getTokenType()) {
				...
				// Reads a variable
				// From the context of practical value, obtain variables can
				// translate into constants Token, pressed into the stack
				Variable varWithValue = VariableContainer.getVariable(expToken.getVariable().getVariableName());
				if (varWithValue != null) {
					// Generate a value constants，varWithValue.getDataValue
					// There may be null
					ExpressionToken constantToken = ExpressionToken.createConstantToken(varWithValue.getDataType(),
				...
				for (int i = 0; i < opType; i++) {
					if (!compileStack.empty()) {
						...
						if (ExpressionToken.ETokenType.ETOKEN_TYPE_CONSTANT == argToken.getTokenType()) {
							...
							// If remove the Token is not constant, then throw
							// mistake
							throw new IllegalExpressionException(
									"操作符" + operator.getToken() + "找不到相应的参数，或参数个数不足;位置：" + expToken.getStartPosition(),
									expToken.getStartPosition());
						}
					} else {
						// Stack has been playing an empty, without via
						// operators corresponding operands
						throw new IllegalExpressionException(
								"操作符" + operator.getToken() + "找不到相应的参数，或参数个数不足;位置：" + expToken.getStartPosition(),
								expToken.getStartPosition());
					}
				...
				if (!compileStack.empty()) {
					...
					// Check to meet a function word yuan, execution in a stack
					// of the popup first word yuan whether for “）”
					if (")".equals(onTop.getSplitor())) {
						boolean doPop = true;
						List<Constant> argsList = new ArrayList<Constant>();
						ExpressionToken parameter = null;
						// Popup the parameters of the function, until encounter
						// "(" to end
						while (doPop && !compileStack.empty()) {
							...
							if (ExpressionToken.ETokenType.ETOKEN_TYPE_CONSTANT == parameter.getTokenType()) {
								...
								// In a function meet neither constants, also
								// not "(", the error occurs
								throw new IllegalExpressionException(
										"函数" + expToken.getFunctionName() + "执行时遇到非法参数" + parameter.toString(),
										expToken.getStartPosition());
							}
						}

						if (doPop && compileStack.empty()) {
							// Operation with empty, find no stack () function
							// left brace (
							throw new IllegalExpressionException("函数" + expToken.getFunctionName() + "执行时没有找到应有的\"(\"",
									expToken.getStartPosition());
						}
				...
				// Reading a separator, pressed into the stack, usually are "("
				// and")"
				compileStack.push(expToken);
		...
		// Expression compiler completion, it is compiled within only a compiler
		// should stack results
	...
	/**
	 * Will express distributed-group management.then channeling (formatting
	 * good), convert word yuan list
	 * 
		...
		for (int i = 0; i < expChars.length; i++) {
			...
			if (' ' == expChars[i]) {
				...
				if (status == 0) {
					...
					status = 2; // Escapes "date, leave escapes into string
								// state
					tokenBuffer.append(expChars[i]);
				}
				...
				if (status == 0) {
					...
					status = 2; // Escapes \ number, leave escapes into string
								// state
					tokenBuffer.append(expChars[i]);
				}
	...
	private void addToken(String tokenString, List<ExpressionToken> tokens) throws IllegalExpressionException {
		...
		if (ExpressionTokenHelper.isInteger(tokenString)) {
			token = ExpressionToken.createConstantToken(BaseDataMeta.DataType.DATATYPE_INT,
					Integer.valueOf(tokenString));
			tokens.add(token);

		} else
		// long
		if (ExpressionTokenHelper.isLong(tokenString)) {
			token = ExpressionToken.createConstantToken(BaseDataMeta.DataType.DATATYPE_LONG,
					Long.valueOf(tokenString.substring(0, tokenString.length() - 1)));
			tokens.add(token);

		} else
		// float
		if (ExpressionTokenHelper.isFloat(tokenString)) {
			token = ExpressionToken.createConstantToken(BaseDataMeta.DataType.DATATYPE_FLOAT,
					Float.valueOf(tokenString.substring(0, tokenString.length() - 1)));
			tokens.add(token);

		} else
		// double
		if (ExpressionTokenHelper.isDouble(tokenString)) {
			token = ExpressionToken.createConstantToken(BaseDataMeta.DataType.DATATYPE_DOUBLE,
					Double.valueOf(tokenString));
			tokens.add(token);

		} else
		// String
		if (ExpressionTokenHelper.isString(tokenString)) {
			token = ExpressionToken.createConstantToken(BaseDataMeta.DataType.DATATYPE_STRING,
		...
		if (ExpressionTokenHelper.isOperator(tokenString)) {
			...
			// The rest should be variable, this judgment depends on generated
			// RPN is correct premise
			// Variables, in Boolean type and null type discriminant after, as
			// long as it is not begin, behind the letters "(" is variable
	...
	public List<ExpressionToken> compile(List<ExpressionToken> expTokens) throws IllegalExpressionException {
		...
		for (ExpressionToken expToken : expTokens) {
			...
			if (ExpressionToken.ETokenType.ETOKEN_TYPE_CONSTANT == expToken.getTokenType()) {
				...
				if (var == null) {
					...
					throw new IllegalExpressionException(
							"表达式不合法，变量\"" + expToken.toString() + "\"缺少定义;位置:" + expToken.getStartPosition(),
							expToken.toString(), expToken.getStartPosition());
				} else {
				...
				if (opStack.empty()) {// If the operation stack is empty
					...
					while (!opStack.empty() && doPeek) {
						// If the stack not null, however, have a stack of
						// operator precedence
						ExpressionToken onTopOp = opStack.peek();

						// If the stack elements are functions, direct will
						// operators pressed into the stack
						if (ExpressionToken.ETokenType.ETOKEN_TYPE_FUNCTION == onTopOp.getTokenType()) {
							...
							doPeek = false;
								...
								&& "(".equals(onTopOp.getSplitor())) {// If the
																		// stack
																		// element
																		// is
																		// (for
							...
							if (expToken.getOperator().getPiority() > onTopOp.getOperator().getPiority()) {
								...
								// The current operator precedence > stack
								// operator precedence, will the current
								// operator into the station
								opStack.push(expToken);
								doPeek = false;

							} else if (expToken.getOperator().getPiority() == onTopOp.getOperator().getPiority()) {
								// The current operator precedence = stack
								// operator precedence, and executing order from
								// left to right, will stack operators popup
								// Execute operators calibration
								ExpressionToken result = verifyOperator(onTopOp, verifyStack);
								// The calibration results inspection push-up
								// stack
								verifyStack.push(result);
								// Calibration through, pop-up stack operators,
								// joining the inverse Poland type queue
								opStack.pop();
								_RPNExpList.add(onTopOp);

							} else {
								// The current operator precedence < stack
								// operator precedence, will stack operators
								// popup
								// Execute operators calibration
								ExpressionToken result = verifyOperator(onTopOp, verifyStack);
								// The calibration results inspection push-up
					...
					// The current operator precedence < = stack in all the
					// operator precedence
				...
				// Meet the function name, then use a temporary variable
				// temporary down and wait (comes
				_function = expToken;

			} else if (ExpressionToken.ETokenType.ETOKEN_TYPE_SPLITOR == expToken.getTokenType()) {
				// Handle the reader "("
				if ("(".equals(expToken.getSplitor())) {
					// If the _function! = null, the specification is function
					// of left brace
					if (_function != null) {
						...
						// Will "(" and temporary buffer function pressed into
						// the operator stacks, in parentheses before
						opStack.push(expToken);
					...
					while (doPop && !opStack.empty()) {
						...
						if (ExpressionToken.ETokenType.ETOKEN_TYPE_OPERATOR == onTopOp.getTokenType()) {
							...
							// If the stack element is ordinary operators,
							// executive operators calibration
							ExpressionToken result = verifyOperator(onTopOp, verifyStack);
							// The calibration results inspection push-up stack
							verifyStack.push(result);

							// Calibration through, then added to the inverse
							// Poland type of columns
							_RPNExpList.add(onTopOp);

						} else if (ExpressionToken.ETokenType.ETOKEN_TYPE_FUNCTION == onTopOp.getTokenType()) {
							// If meet the function, then explaining ") "is the
							// function of right brace
							// Executive function calibration
							ExpressionToken result = verifyFunction(onTopOp, verifyStack);
							// The calibration results inspection push-up stack
							verifyStack.push(result);

							// Calibration through, adding ") "to the inverse
							// Poland type
							_RPNExpList.add(expToken);
					...
					if (doPop && opStack.empty()) {
						throw new IllegalExpressionException("在读入\")\"时，操作栈中找不到对应的\"(\" ", expToken.getSplitor(),
								expToken.getStartPosition());
					}

					// Read the ", "handle
				} else if (",".equals(expToken.getSplitor())) {
					// Ordinal popup operators stack of all operators, pressed
					// into the inverse Poland type queue, until I met function
					// word yuan
					boolean doPeek = true;
					...
					while (!opStack.empty() && doPeek) {
						...
						if (ExpressionToken.ETokenType.ETOKEN_TYPE_OPERATOR == onTopOp.getTokenType()) {
							...
							// The calibration results inspection push-up stack
							verifyStack.push(result);
							// Calibration through, and pressed into the inverse
							// Poland type queue
							_RPNExpList.add(onTopOp);
							...
							// In reads ",", operators stack for"(", the error
							// occurs
							throw new IllegalExpressionException(
									"在读入\",\"时，操作符栈顶为\"(\",,(函数丢失) 位置：" + onTopOp.getStartPosition(),
									expToken.getSplitor(), expToken.getStartPosition());
						}
					}
					// Stack all pop-up, but not meet function word yuan
					if (doPeek && opStack.empty()) {
						throw new IllegalExpressionException("在读入\",\"时，操作符栈弹空，没有找到相应的函数词元 ", expToken.getSplitor(),
								expToken.getStartPosition());
					}
				}
			}
		...
		// Will operate within the rest of the operator - accessor to one popup,
		// and pressed into the inverse Poland type queue
		while (!opStack.empty()) {
			ExpressionToken onTopOp = opStack.pop();
			...
			if (ExpressionToken.ETokenType.ETOKEN_TYPE_OPERATOR == onTopOp.getTokenType()) {
				...
				throw new IllegalExpressionException("函数" + onTopOp.getFunctionName() + "缺少\")\"",
						onTopOp.getFunctionName(), onTopOp.getStartPosition());

			} else if ("(".equals(onTopOp.getSplitor())) {
				// The rest of the only have "(", then explaining expression
				// formula lack right brace")"
				throw new IllegalExpressionException("左括号\"(\"缺少配套的右括号\")\"", onTopOp.getFunctionName(),
						onTopOp.getStartPosition());
			}
		}

		// Expression calibration completion, it is only a stack should be
		// within calibration results, otherwise as expressions don't finish
		...
		for (int i = 0; i < opType; i++) {
			if (!verifyStack.empty()) {
				...
				if (ExpressionToken.ETokenType.ETOKEN_TYPE_CONSTANT == argToken.getTokenType()) {
					...
					// If you take the Token is not constant, nor variables,
					// then throw mistake
					throw new IllegalExpressionException(
							"表达式不合法，操作符\"" + op.getToken() + "\"参数错误;位置：" + argToken.getStartPosition(),
							opToken.toString(), opToken.getStartPosition());
				}

			} else {
				// Stack has been playing an empty, without via operators
		...
		if (!verifyStack.empty()) {
			...
			while (doPop && !verifyStack.empty()) {
				...
				if (ExpressionToken.ETokenType.ETOKEN_TYPE_CONSTANT == parameter.getTokenType()) {
					...
					// Find no should exist right brace
					throw new IllegalExpressionException(
							"表达式不合法，函数\"" + funtionToken.getFunctionName() + "\"遇到非法参数" + parameter.toString() + ";位置:"
									+ parameter.getStartPosition(),
							funtionToken.toString(), funtionToken.getStartPosition());
				}
			}
			...
			if (doPop && verifyStack.empty()) {
				...
				throw new IllegalExpressionException(
						"表达式不合法，函数\"" + funtionToken.getFunctionName() + "\"缺少\"(\"；位置:"
								+ (funtionToken.getStartPosition() + funtionToken.toString().length()),
						funtionToken.toString(), funtionToken.getStartPosition());
			}
			...
			Constant result = FunctionExecution.varify(funtionToken.getFunctionName(), funtionToken.getStartPosition(),
					arguments);
			return ExpressionToken.createConstantToken(result);
