...
public class MethodImpl extends BaseMethodFactory {
	...
	public void initMap_ID_MapData() {// 将Id_MapData初始化到mapList中
		// IMyBatisSessionTemplate
		// myBatisSessionTemplate=(IMyBatisSessionTemplate)SpringContextUtils.getBean("myBatisSessionTemplate");
		// List<Map> resultList =
		// myBatisSessionTemplate.selectList("com.huateng.report.imports.sqlmap.Import.select6");
		// int maptype = -1;
	...
	public void initMap_SYS_WorkDate() {
		...
		// myBatisSessionTemplate=(IMyBatisSessionTemplate)SpringContextUtils.getBean("myBatisSessionTemplate");
		// List<Map> resultList =
		// myBatisSessionTemplate.selectList("com.huateng.report.imports.sqlmap.Import.getWorkDate",inputMap);
		// int programId = 0;
		// List lcTempList = new ArrayList();
		// for (Map idmp : resultList) {
		// lcTempList = new ArrayList();
	...
	private void initMenthod() {
		...
		super.add(new BaseMethod("getTxt") {
			public Object call(Object[] arg) throws IllegalExpressionException {
				...
				if (ImportFileVar.FILEDATA_FORMATTYPE_FIXED.equals(curImpFileInfo.getFormatType())) {
					if ((iiBgPointer >= 0) && (iiEdPointer >= 0) && (iiBgPointer <= iiEdPointer
							&& (curImpFileInfo.getFRowValue().get(0).toString().getBytes().length) >= iiEdPointer)) {
						try {
						...
						return txuSubStringTxt(curImpFileInfo.getFRowValue().get(0).toString(), iiBgPointer,
								iiEdPointer).trim();
					} else {
						return "";
					}
		...
		/**
		 * 输入的arg[1]，以arg[0]为map_type读通用ID_MapData表，如果匹配不到，arg[2] = 0
		 * 时，则返回空值，arg[2] = 1 时，返回原值
		 * 
		 * @param arg[0]
		 *            数字参数1
		 * @param arg[1]
		 ...
		 *            返回值类型
		 */
		super.add(new BaseMethod("getMapValue") {// (getMapValue(n1,subString(),1或0)
													// 原case3和case15
			public Object call(Object[] arg) throws IllegalExpressionException {
		...
		/**
		 * 取得行项目中的特定项目内容 - 获取主键ID BRANCHCODE+ACCOUNTNUMBER+CURRENCYCODE 或者
		 * BRANCHCODE+PARTYNUMBER
		 * 
		 * @param arg[0]
		 *            行数据内容
		 * @param arg[1]
		...
		super.add(new BaseMethod("getNoneColumn") {// 取得行项目中的特定项目内容
			public Object call(Object[] arg) throws IllegalExpressionException {
				...
				if (arg[1] != null && "localextention".equalsIgnoreCase(arg[1].toString())
						&& Integer.valueOf(arg[3].toString()).intValue() != 0) {
					int num = Integer.valueOf(arg[2].toString()).intValue()
							+ (Integer.valueOf(arg[3].toString()).intValue() - 11);
					txtContent = curImpFileInfo.getFRowValue().get(num - 1).toString().trim();
				} else if (arg[1] != null && "localextention".equalsIgnoreCase(arg[1].toString())
						&& Integer.valueOf(arg[3].toString()).intValue() == 0) {
		...
		super.add(new BaseMethod("inAlphabets") {// case9
			public Object call(Object[] arg) throws IllegalExpressionException {
				...
				if ((Double.valueOf(txtContent) >= Double
						.valueOf(String.valueOf(mv_cp1).substring(0, Integer.valueOf(String.valueOf(mv_np1)) - 1)))
						&& (Double.valueOf(txtContent) <= Double
								.valueOf(String.valueOf(mv_cp1).substring(Integer.valueOf(String.valueOf(mv_np1)),
										2 * Integer.valueOf(String.valueOf(mv_np1)) - 1)))) {
		...
		/**
		 * 输入参数取n1起始位，n2个字符（n2为0，则返回原字符串），C1存放原日期格式,，C2存放新日期格式（日期格式必须是由yyyy、mm、
		 * dd组成）， 如："yyyy-mm-dd"等如果输入参数不是日期,C1存放对应类型说明: 当前有: excelType
		 * 
		 * @param arg[0]
		 *            数据内容
		 * @param arg[1]
		...
		super.add(new BaseMethod("toUpperCase") {// case26，case27
			public Object call(Object[] arg) throws IllegalExpressionException {
				...
				int mv_np1 = Integer.valueOf(arg[1].toString()).intValue(); // 数字参数1
				int mv_np2 = Integer.valueOf(arg[2].toString()).intValue(); // 数字参数2
				int changeable = Integer.valueOf(arg[3].toString()).intValue(); // 数字参数3:1
																				// 不做变换
																				// 2
																				// 字符转换为大写
				String txtContent = ""; // 返回字段
				...
				try {
					if (txtContent != null) {
						if (txtContent.length() > mv_np2 + mv_np1) {
							txtContent = txtContent.substring(txtContent.length() - mv_np2 - mv_np1 + 1,
									txtContent.length() - mv_np1 + 1);
						} else {
							txtContent = "";
						}
					}
		...
		super.add(new BaseMethod("getResult") {// 字段转换函数2，得到最终字段值case2
			public Object call(Object[] arg) throws IllegalExpressionException {
				...
				try {
					...
					case 4: { // 如果：字段1=c1[1,n2]且字段2=c1[n2+1,2*n2]，返回'1'
								// ，否则返回'0'
					...
					case 5: { // 如果：字段1=c1[1,n2]或字段2=c1[n2+1,2*n2]，返回'1'
								// ，否则返回'0'
						if (field1.equals(subString(mv_cp1, 0, mv_np2))
								|| field2.equals(subString(mv_cp1, mv_np2, 2 * mv_np2))) {
							txtContent = "1";
		...
		/**
		 * 字段1=c1，且字段2不等于c2，满足条件的返回字段3，否则返回n1 其中c2的值支持穷举，如果多值的时候，值之间以’|”分割,
		 * 如aa|bb|cc|
		 * 
		 * @param arg[0]
		 *            取得字段1
		...
		/**
		 * 字段1转化为number后 >、< 、>=、<= n1, (<,>,>=.<=存放在c1中) 且字段2=c2, 返回字段3, 否则返回空
		 * 
		 * @param arg[0]
		 *            取得字段1
		 * @param arg[1]
				...
				try {
					...
					if (mv_cp1.trim().equals("01")) {
						...
						if (txtContent.length() >= Integer.valueOf(field3)) {
							...
							txtContent = String.valueOf(Math.round(lexTmp * mv_np1));
							int j = txtContent.length();
							for (int i = 0; i < Integer.valueOf(field3) - j; i++) {
						...
						txtContent = String.valueOf(Math.round(lexTmp * mv_np1));
						if (txtContent.length() >= Integer.valueOf(field3)) {
							txtContent = subString(txtContent, 0, (int) Integer.valueOf(field3));
						} else {
							int j = txtContent.length();
							for (int i = 0; i < Integer.valueOf(field3) - j; i++) {
								txtContent = " " + txtContent;
		...
		super.add(new BaseMethod("sameOperator") {// 字段转换函数2，得到最终字段值case12
			public Object call(Object[] arg) throws IllegalExpressionException {
				...
				Double lexTmp = 0.0;
				try {
					if ((!mv_cp1.trim().equals("+")) && (!mv_cp1.trim().equals("-")) && (!mv_cp1.trim().equals("*"))
							&& (!mv_cp1.trim().equals("/"))) {
						return false;
					}
					double lexTmp2 = 0.0;
		...
		/**
		 * 如果字段1不为空则返回字段1；如果字段1为空、字段2不等于c1，则返回空；
		 * 如果字段1为空、字段2等于c1、字段3为空，则返回空；如果字段1为空、字段2等于c1、字段3不为空，则返回c2
		 * 
		 * @param arg[0]
		 *            取得字段1
		 * @param arg[1]
