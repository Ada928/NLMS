...
public class ExpressionParser {
	...
	private Stack<String> parenthesis = new Stack<String>();// Matching
															// parentheses stack
	...
	public List<ExpressionToken> getExpressionTokens(String expression) throws IllegalExpressionException {
		...
		ExpressionToken expressionToken = null;// The last time the
												// ExpressionToken read
		...
		try {
			while ((ele = eReader.readToken()) != null) {
				...
				pushParenthesis(ele);
				if ((tempExpressionToken != null && expressionToken != null
						&& "(".equals(tempExpressionToken.getSplitor()) && ",".equals(expressionToken.getSplitor()))
						|| (tempExpressionToken != null && expressionToken != null
								&& ",".equals(tempExpressionToken.getSplitor())
								&& ",".equals(expressionToken.getSplitor()))
						|| (tempExpressionToken != null && expressionToken != null
								&& ",".equals(tempExpressionToken.getSplitor())
								&& ")".equals(expressionToken.getSplitor()))) {
	...
	/**
	 * Will ExpressionToken syncopated elements into, and validation minus or
	 * minus
	 * 
	 * @param previousToken
	 * @param ele
	...
	public ExpressionToken changeToToken(ExpressionToken previousToken, Element ele)
			throws ParseException, IllegalExpressionException {
		if (ele == null) {
		...
		if (ElementType.NULL == ele.getType()) {
			...
			if (ele.getText().equals("-") && (previousToken == null // With "-"
																	// beginning
																	// affirmation
																	// is a
																	// minus
					|| previousToken.getTokenType() == ETokenType.ETOKEN_TYPE_OPERATOR // Operators
																						// behind
																						// affirmation
																						// is
																						// a
																						// minus
					|| previousToken.getTokenType() == ETokenType.ETOKEN_TYPE_SPLITOR // "("
																						// or",
																						// "behind
																						// affirmation
																						// is
