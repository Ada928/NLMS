...
public class OperatorTypeReader implements ElementReader {
	 ...
	 *             Not a legal operators type thrown when
	 * @throws IOException
	 */
	public Element read(ExpressionReader sr) throws IllegalExpressionException, IOException {
		...
		if (isOperatorWord(sb.toString())) {
			if (sb.length() == 1) {// Two symbols operators of priority, such as
									// < =, should not be considered < operator
				sr.mark(0);
				b = sr.read();
				if (b != -1) {
					if (isOperatorWord(sb.toString() + (char) b)) {
						return new Element(sb.toString() + (char) b, index, ElementType.OPERATOR);
					}
				}
				sr.reset();
			}
			return new Element(sb.toString(), index, ElementType.OPERATOR);
		}
		...
		while ((b = sr.read()) != -1) {
			...
			if (VariableTypeReader.STOP_CHAR.indexOf(c) >= 0) {// Word stop
																// operator
