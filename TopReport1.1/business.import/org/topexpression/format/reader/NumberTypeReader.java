...
public class NumberTypeReader implements ElementReader {
	public static final String NUMBER_CHARS = "01234567890.";// Represent the
																// value of
																// characters
	 ...
	 *             Not a legal thrown when numeric types
	 */
	public Element read(ExpressionReader sr) throws IOException, IllegalExpressionException {
		...
		while ((b = sr.read()) != -1) {
			...
			if (NUMBER_CHARS.indexOf(c) == -1) {
				if (LONG_MARKS.indexOf(c) >= 0) {
					...
					sr.reset();
					if (sb.indexOf(".") >= 0) {// No end logo, have a decimal
												// amount, for double
						checkDecimal(sb, sr);
						return new Element(sb.toString(), index, ElementType.DOUBLE);
		...
		// Read to the end
		if (sb.indexOf(".") >= 0) {// No end logo, have a decimal amount, for
									// double
			checkDecimal(sb, sr);
			return new Element(sb.toString(), index, ElementType.DOUBLE);
		} else {// No sign, no decimal point, the end for int
