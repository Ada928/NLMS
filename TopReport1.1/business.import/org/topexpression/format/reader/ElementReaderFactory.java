...
public class ElementReaderFactory {
	...
	/**
	 * According to flow beginning texturally different words yuan read Flow
	 * should not Spaces beginning
	 * 
	 * @param reader
	 ...
	 */
	public static ElementReader createElementReader(ExpressionReader reader)
		...
		if (b != -1) {
			char c = (char) b;
			try {
				if (c == StringTypeReader.START_MARK) {// "Beginning, tectonic
														// string read device
					return StringTypeReader.class.newInstance();
				} else if (SplitorTypeReader.SPLITOR_CHAR.indexOf(c) >= 0) {// If
																			// is
																			// separators,
																			// tectonic
																			// separator
				...
				} else if (NumberTypeReader.NUMBER_CHARS.indexOf(c) >= 0) {// In
																			// digital
																			// beginning,
																			// tectonic
																			// numeric
				...
				} else if (OperatorTypeReader.isOperatorStart(reader)) {// If
																		// the
																		// prefix
																		// is
																		// operator,
	...
	public static ElementReader valInstance(ExpressionReader reader)
