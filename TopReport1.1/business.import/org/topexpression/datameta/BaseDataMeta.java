...
public abstract class BaseDataMeta {
	...
	public Float getFloatValue() {
		...
		if (DataType.DATATYPE_INT != this.dataType && DataType.DATATYPE_FLOAT != this.dataType
	...
	public Double getDoubleValue() {
		if (DataType.DATATYPE_INT != this.dataType && DataType.DATATYPE_LONG != this.dataType
				&& DataType.DATATYPE_FLOAT != this.dataType && DataType.DATATYPE_DOUBLE != this.dataType) {
			throw new UnsupportedOperationException("当前常量类型不支持此操作");
		}
		if (dataValue == null) {
			return null;
	...
	/**
	 * Check data types of compatibility Same as the type must be compatible
	 * Type is different, compatible data types including int ，long ，float ，
	 * double null Type and all types of compatible
	 * 
	 * @param another
	 * @return
	 */
	private boolean isCompatibleType(BaseDataMeta another) {
		...
		} else if (DataType.DATATYPE_INT != this.getDataType() && DataType.DATATYPE_LONG != this.getDataType()
				&& DataType.DATATYPE_FLOAT != this.getDataType() && DataType.DATATYPE_DOUBLE != this.getDataType()) {
			return false;

		} else if (DataType.DATATYPE_INT != another.getDataType() && DataType.DATATYPE_LONG != another.getDataType()
	...
	/**
	 * Obtain two Numbers of compatible type If two data type can't compatible,
	 * returns null
	 * 
	 * @param another
	 * @return
