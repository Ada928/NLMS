...
public class BankInfoService {
	 ...
	 */
	public void removeEntity(Bctl bean) {
		BlackListDAO rootDAO = BlackListDAOUtils.getBlackListDAO();
		try {
			rootDAO.delete(bean);
	 ...
	 * @param biNationregion
	 */
	public void modOrAddEntity(Bctl bean) {
		BlackListDAO rootDAO = BlackListDAOUtils.getBlackListDAO();
		try {
	...
	public void addEntity(Bctl bean) throws CommonException {
		BlackListDAO rootDAO = BlackListDAOUtils.getBlackListDAO();
	...
	public boolean isExists(String id) {
		BlackListDAO rootDAO = BlackListDAOUtils.getBlackListDAO();
		try {
			Bctl bean = (Bctl) rootDAO.query(Bctl.class, id);
			if (bean == null) {
				return false;
			}
		} catch (CommonException e) {
	...
	public void modEntity(Bctl bean) {
		BlackListDAO rootDAO = BlackListDAOUtils.getBlackListDAO();
		try {
			rootDAO.update(bean);
			System.out.println(this.getClass().getName() + " 已插入或更新实体");
	...
	// 通过id来获取实体类
	public Bctl selectById(String id) {
		BlackListDAO rootdao = BlackListDAOUtils.getBlackListDAO();
		Bctl bean = null;
		try {
			bean = (Bctl) rootdao.query(Bctl.class, id);
		} catch (CommonException e) {
			e.printStackTrace();
		}
		return bean;
