...
public class BankOperateLogService {
	...
	public static BankOperateLogService getInstance() {
		return (BankOperateLogService) ApplicationContextUtils.getBean(BankOperateLogService.class.getName());
	}
	 ...
	 * @param paramgroupId 参数段编号
	 */
	public List getAllBankOperateLog() throws CommonException {
		BlackListDAO rootDAO = BlackListDAOUtils.getBlackListDAO();
		List list = rootDAO.queryByQL2List("1=1");
		for (int i = 0; i < list.size(); i++) {
			BctlOperateLog bean = (BctlOperateLog) list.get(i);
			list.set(i, bean);
		}
		return list;
	}
	 ...
	 */
	public void removeEntity(BctlOperateLog bean) {
		BlackListDAO rootDAO = BlackListDAOUtils.getBlackListDAO();
		try {
			rootDAO.delete(bean);
	 ...
	 * @param biNationregion
	 */
	public void modOrAddEntity(BctlOperateLog bean) {
		BlackListDAO rootDAO = BlackListDAOUtils.getBlackListDAO();
		try {
	...
	public void addEntity(BctlOperateLog bean) throws CommonException {
		BlackListDAO rootDAO = BlackListDAOUtils.getBlackListDAO();
	...
	public boolean isExists(String id) {
		BlackListDAO rootDAO = BlackListDAOUtils.getBlackListDAO();
		try {
			BctlOperateLog bean = (BctlOperateLog) rootDAO.query(BctlOperateLog.class, id);
			if (bean == null) {
				return false;
			}
		} catch (CommonException e) {
	...
	public void modEntity(BctlOperateLog bean) {
		BlackListDAO rootDAO = BlackListDAOUtils.getBlackListDAO();
		try {
			rootDAO.update(bean);
			System.out.println(this.getClass().getName() + " 已插入或更新实体");
	...
	// 通过id来获取实体类
	public BctlOperateLog selectById(String id) {
		BlackListDAO rootdao = BlackListDAOUtils.getBlackListDAO();
		BctlOperateLog bean = null;
		try {
			bean = (BctlOperateLog) rootdao.query(BctlOperateLog.class, id);
		} catch (CommonException e) {
			e.printStackTrace();
		}
		return bean;
