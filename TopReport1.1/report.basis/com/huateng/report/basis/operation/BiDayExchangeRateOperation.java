...
public class BiDayExchangeRateOperation extends BaseOperation {
	...
	public void execute(OperationContext context) throws CommonException {
		...
		if (CMD_DEL.equals(cmd)) {
			...
			try {
				taskInfo = ReportTaskUtil.getSysTaskInfoBean(ReportEnum.REPORT_TASK_FUNCID.TASK_110699.value,
						ReportEnum.REPORT_TASK_TRANS_CD.DEL.value, biDayexchangerate, biDayexchangerate.getId(),
						biDayexchangerate.getSt());
			...
			try {
				taskInfo = ReportTaskUtil.getSysTaskInfoBean(ReportEnum.REPORT_TASK_FUNCID.TASK_110699.value,
						ReportEnum.REPORT_TASK_TRANS_CD.NEW.value, biDayexchangerate, biDayexchangerate.getId(),
						biDayexchangerate.getSt());
				service.addTosystaskinfo(taskInfo);
			} catch (IOException e) {
				// TODO Auto-generated catch block
			...
			try {
				taskInfo = ReportTaskUtil.getSysTaskInfoBean(ReportEnum.REPORT_TASK_FUNCID.TASK_110699.value,
						ReportEnum.REPORT_TASK_TRANS_CD.EDIT.value, biDayexchangerate, biDayexchangerate.getId(),
						biDayexchangerate.getSt());
				service.addTosystaskinfo(taskInfo);
			} catch (IOException e) {
				// TODO Auto-generated catch block
