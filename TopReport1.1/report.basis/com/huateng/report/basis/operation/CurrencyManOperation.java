...
public class CurrencyManOperation extends BaseOperation {
	...
	public void execute(OperationContext context) throws CommonException {
		...
		if (CMD_INSERT.equalsIgnoreCase(cmd)) {
			...
			try {
				taskInfo = ReportTaskUtil.getSysTaskInfoBean(ReportEnum.REPORT_TASK_FUNCID.TASK_110199.value,
						ReportEnum.REPORT_TASK_TRANS_CD.NEW.value, sysCurrency, sysCurrency.getId(),
						sysCurrency.getSt());
			...
			try {
				taskInfo = ReportTaskUtil.getSysTaskInfoBean(ReportEnum.REPORT_TASK_FUNCID.TASK_110199.value,
						ReportEnum.REPORT_TASK_TRANS_CD.EDIT.value, sysCurrency, sysCurrency.getId(),
						sysCurrency.getSt());
				sysCurService.addTosystaskinfo(taskInfo);
			} catch (IOException e) {
				// TODO Auto-generated catch block
			...
			try {
				taskInfo = ReportTaskUtil.getSysTaskInfoBean(ReportEnum.REPORT_TASK_FUNCID.TASK_110199.value,
						ReportEnum.REPORT_TASK_TRANS_CD.DEL.value, sysCurrency, sysCurrency.getId(),
						sysCurrency.getSt());
				sysCurService.addTosystaskinfo(taskInfo);
			} catch (IOException e) {
				// TODO Auto-generated catch block
