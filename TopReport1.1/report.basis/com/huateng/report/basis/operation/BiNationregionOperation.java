...
public class BiNationregionOperation extends BaseOperation {
	...
	public void execute(OperationContext context) throws CommonException {
		...
		if (CMD_DEL.equals(cmd)) {
			...
			try {
				taskInfo = ReportTaskUtil.getSysTaskInfoBean(ReportEnum.REPORT_TASK_FUNCID.TASK_110499.value,
						ReportEnum.REPORT_TASK_TRANS_CD.DEL.value, biNationregion, biNationregion.getId(),
						biNationregion.getSt());
		...
		// service.removeEntity(biNationregion);
			...
			try {
				taskInfo = ReportTaskUtil.getSysTaskInfoBean(ReportEnum.REPORT_TASK_FUNCID.TASK_110499.value,
						ReportEnum.REPORT_TASK_TRANS_CD.NEW.value, biNationregion, biNationregion.getId(),
						biNationregion.getSt());
				service.addTosystaskinfo(taskInfo);
			} catch (IOException e) {
				// TODO Auto-generated catch block
			...
			try {
				taskInfo = ReportTaskUtil.getSysTaskInfoBean(ReportEnum.REPORT_TASK_FUNCID.TASK_110499.value,
						ReportEnum.REPORT_TASK_TRANS_CD.EDIT.value, biNationregion, biNationregion.getId(),
						biNationregion.getSt());
				service.addTosystaskinfo(taskInfo);
			} catch (IOException e) {
				// TODO Auto-generated catch block
