...
public class BiDayExchangeRateGetter extends BaseGetter {
	...
	private PageQueryResult getData() {
		...
		String ratecut = (String) getCommQueryServletRequest().getParameterMap().get("ratecut");
		String rateDate = (String) getCommQueryServletRequest().getParameterMap().get("rateDate");
		String qst = getCommQueryServletRequest().getParameter("st");
		int pageSize = this.getResult().getPage().getEveryPage();
		int pageIndex = this.getResult().getPage().getCurrentPage();
		PageQueryResult pqr = BiDayExchangeRateService.getInstance().pageQueryByHql(pageIndex, pageSize, ratecut,
				rateDate, qst);
		return pqr;
	}
}
