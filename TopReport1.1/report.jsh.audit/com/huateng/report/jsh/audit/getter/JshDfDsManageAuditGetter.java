...
public class JshDfDsManageAuditGetter extends BaseGetter {
	...
	public Result call() throws AppException {
		...
		try {
			...
			this.setValue2DataBus(ReportConstant.QUERY_LOG_BUSI_NAME, "外汇账目内结汇管理信息审核查询");
	...
	private PageQueryResult getData() throws CommonException {
		...
		hql.append(
				" from MtsJshDefgDs model where model.apptype = ? and model.currentfile = ? and model.recStatus in (?,?)");
		hqlObj.add(TopReportConstants.REPORT_APP_TYPE_JSH);
		hqlObj.add(TopReportConstants.REPORT_FILE_TYPE_JSH_F);
