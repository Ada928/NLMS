...
public class UpdateAndQuery {
	...
	public void updateDfzh1() {
		...
		StringBuffer updateSql = new StringBuffer();
		querySql.append(
				"select distinct zhdh ,cpzxh ,cpznxh ,yefx from cpwj  where kmdh in (select k.kmdh from kmdhb k where k.kmcc='0') and BCZBZ='0' and cbbz='0'");
		updateSql.append("update  cpwj set flbz=? where CPZXH=? and yefx=? and zhdh<>?");
		PreparedStatement pstmtQ = null;
		PreparedStatement pstmtU = null;
		ResultSet rs = null;
	...
	public void updateDqzwjLldh() {
		...
		try {
			...
			while (rs.next()) {
				...
				qx = (Integer.parseInt(date.substring(0, 4)) - Integer.parseInt(jyrq.substring(0, 4))) * 12
						+ (Integer.parseInt(date.substring(4, 6)) - Integer.parseInt(jyrq.substring(4, 6)));
				if (qx > 3 && qx < 6) {
					lldh = "01102M03";
				} else if (qx > 6 && qx < 12) {
					lldh = "01102M06";
	...
	public void updateDfzh2() {
		...
		StringBuffer updateSql = new StringBuffer();
		updateSql.append(
				"update  cpwj set flbz=zhdh where kmdh  not in (select k.kmdh from kmdhb k where k.kmcc='0') and BCZBZ='0' and cbbz='0'");
		PreparedStatement pstmtU = null;
		ResultSet rs = null;
		try {
			pstmtU = conn.prepareStatement(updateSql.toString());
	...
	public void shieldData() {
		...
		try {
			...
			while (rs.next()) {
				...
				switch (flg) {
				case '0':// 公贷账号
					conn.prepareStatement("delete hqdgzwj  where zhdh in (select contents from ex_data where flg='0')")
							.executeUpdate();
					conn.prepareStatement("delete cpwj  where zhdh in (select contents from ex_data where flg='0')")
							.executeUpdate();
					break;
				case '1':// 公贷客户号
					conn.prepareStatement("delete dgkhxxwj  where khdh in (select contents from ex_data where flg='1')")
							.executeUpdate();
					conn.prepareStatement(
							"delete customer  where trim(externalcusid) in (select contents from ex_data where flg='1')")
							.executeUpdate();
					break;
				case '2':// 个人客户账号
					conn.prepareStatement("delete dshqzwj where  zhdh in (select contents from ex_data where flg='2')")
							.executeUpdate();
					conn.prepareStatement("delete hqdgzwj where  zhdh in (select contents from ex_data where flg='2')")
							.executeUpdate();
					break;
				case '3':// 个人客户号
					conn.prepareStatement(
							"delete coustomer_info  where custcd in (select contents from ex_data where flg='3')")
							.executeUpdate();
					conn.prepareStatement(
							"delete customer_relation_info  where custno in (select contents from ex_data where flg='3') or custcd in (select contents from ex_data where flg='2')")
							.executeUpdate();
					conn.prepareStatement(
							"delete assureinfo where custcd in (select contents from ex_data where flg='3')")
							.executeUpdate();
					conn.prepareStatement("delete dkhzwj where zhdh in (select contents from ex_data where flg='3')")
							.executeUpdate();
					conn.prepareStatement(
							"delete dskhb where dk.zjzl||dk.zjhm  in (select contents from ex_data where flg='3')")
							.executeUpdate();
					break;
				case '4':// 内部账号
					conn.prepareStatement("delete   nbzzwj where zhdh in (select contents from ex_data where flg='4')")
							.executeUpdate();
					conn.prepareStatement("delete   cpwj where zhdh in (select contents from ex_data where flg='4')")
							.executeUpdate();
					break;
				case '5':// 对公贷款
					conn.prepareStatement("delete   dkhzwj where htbh in (select contents from ex_data where flg='5')")
							.executeUpdate();
					conn.prepareStatement(
							"delete   acceptancedraft where appid in (select contents from ex_data where flg='5')")
							.executeUpdate();
					conn.prepareStatement(
							"delete   loan where contractno in (select contents from ex_data where flg='5')")
							.executeUpdate();
					break;
				case '6':// 个人贷款
					conn.prepareStatement("delete   dkhzwj where htbh in (select contents from ex_data where flg='6')")
							.executeUpdate();
					conn.prepareStatement(
							"delete   loan where contractno in (select contents from ex_data where flg='6')")
							.executeUpdate();
					break;
				...
				/**
				 * 个人账号需要删除 cpwj; dshqzwj ;dqzwj; dskhxxwj ; hqdgzwj ; dkhzwj
				 * customer_relation_info ; tla_lncino_base
				 * ;assureinfo;monitordtl;lnhtr;customer_info;credit_info,dskhb
				 * dgkhxxwj ;acceptancedraft;loan;customer; security
				 * ;dbextension;loansort2list; LoanCheck ;dgkhxxwj
				 */
