...
public class BaseDao {
	...
	public static int queryAndWriteFile(String tableName, String cjrq, Map<String, String> sqlMap,
			Map<String, List<String>> tableInfoMap, BufferedWriter bw, DefautValueVO defautValue) throws Exception {
		int ret = 0;
		int count = 0;
		Connection conn = DBUtil.getConnection();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
				...
				while (rs.next()) {
					...
					if (count == fieldInfoList.size()) {
						for (int i = 0; i < count; i++) {
							...
							// 变形字段标记
							// '不变形-0,名称变形-1,身份证变形-2,名称暂时不取-3,身份证暂时不取-4,其他暂不取-5,其他暂时不变形-6'
							...
							try {
								if ((fieldName.equals("YXJGDM") || fieldName.equals("YWBLX"))
										&& (null == fieldValue || "".equals(fieldValue.trim()))) {
									fieldValue = defautValue.getYXJGDM();
								} else if (fieldName.equals("NBJGH")
										&& (null == fieldValue || "".equals(fieldValue.trim()))) {
									fieldValue = defautValue.getNBJGH();
								} else if (fieldName.equals("JRXKZH")
										&& (null == fieldValue || "".equals(fieldValue.trim()))) {
									fieldValue = defautValue.getJRXKZH();
								} else if (fieldName.equals("JBJGMC")
										&& (null == fieldValue || "".equals(fieldValue.trim()))) {
									fieldValue = defautValue.getYXJGMC();
								} else if (fieldName.equals("YXJGMC")
										&& (null == fieldValue || "".equals(fieldValue.trim()))) {
									fieldValue = defautValue.getYXJGMC();
								} else if (fieldName.equals("ZXJGDM")
										&& (null == fieldValue || "".equals(fieldValue.trim()))) {
									fieldValue = defautValue.getNBJGH();
								} else if (fieldName.equals("FKXDM")
										&& (null == fieldValue || "".equals(fieldValue.trim()))) {
									fieldValue = defautValue.getYXJGDM();
								} else if (fieldName.equals("FKXMC")
										&& (null == fieldValue || "".equals(fieldValue.trim()))) {
									fieldValue = defautValue.getYXJGMC();
								} else if (fieldName.equals("JYJGMC")
										&& (null == fieldValue || "".equals(fieldValue.trim()))) {
									fieldValue = defautValue.getYXJGMC();
								} else if (fieldName.equals("DJJG")
										&& (null == fieldValue || "".equals(fieldValue.trim()))) {
									fieldValue = defautValue.getYXJGDM();
								} else if (fieldName.equals("CZJG")
										&& (null == fieldValue || "".equals(fieldValue.trim()))) {
									fieldValue = defautValue.getYXJGDM();
								}
								...
								// 非定长用逗号分割
								value = ToolUtils.formatString(fieldType, fieldValue, Integer.parseInt(fieldLength),
										specflag, Integer.parseInt(fieldLength2), Integer.parseInt(fieldSetFlag));
							} catch (Exception e) {
								break;
							}
