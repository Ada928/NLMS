...
public class ReportDataBakService {
	private static final HtLog htlog = HtLogFactory.getLogger(ReportDataBakService.class);
	...
	public String createDataBakFile() throws Exception {
		...
		if (beanList.size() > 0) {
			...
			try {
				...
				if (zipFile.exists()) {
					htlog.info(zipFileName + "============zip file exists======");
					zipFile.delete();
				}
				htlog.info(zipFileName + "============zip file start======");
				// 压缩文件
				PackZipUtil ziputil = new PackZipUtil();
				ziputil.createZip(fileList, zipFileName);
				htlog.info(zipFileName + "============zip file end======");
	...
	private String queryDataAndWrFile(Connection conn, String tableName, String querySql, int startnum, int endnum,
		...
		try {
			htlog.info("exp table " + tableName + ",startrow:" + startnum + ",endrow:" + endnum + "=====start======");
			...
			htlog.info("exp table " + tableName + ",startrow:" + startnum + ",endrow:" + endnum + "=====end======");
		...
		if (dataList != null && dataList.size() > 0) {
			...
			try {
				...
				if (file.exists()) {
					...
					htlog.info("write file " + filePath + "=====append=====");
				} else {
					fos = new FileOutputStream(filePath);
					htlog.info("write file " + filePath + "=====new=====");
				}
				...
				csvwrite.flush();
				htlog.info("write file " + filePath + "=====end=====");
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				if (csvwrite != null) {
