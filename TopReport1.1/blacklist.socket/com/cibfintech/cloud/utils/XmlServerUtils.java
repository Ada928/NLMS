...
public class XmlServerUtils {
	...
	public static String tranToXML() {
		 ...
		 * msgRepBody.setRspCode("000000"); msgRepBody.setRspMsg("success");
		 * msgRepBody.setSeqNo("HX2017092900000001");
		 * msgRepBody.setAccountCode("3212017092900000001");
		 * msgRepBody.setAccountType("I"); msgRepBody.setBlacklistType("1");
		 * msgRepBody.setCertificateNumber("3212017092900000001");
		 * msgRepBody.setCertificateType("01"); msgRepBody.setClientName("张三");
		 * msgRepBody.setClientEnglishName("zhangsan");
		 * msgRepBody.setIsValid("T"); msgRepBody.setValidDate("21100908");
		 * msgRepBody.setRemarks("remaker"); list.add(msgRepBody);
		 * 
		 * ReponseHeader msHeader = new ReponseHeader();
		 * msHeader.setSysNo("HX"); msHeader.setTranCode("10000");
		 * msHeader.setTranDate("20170929"); msHeader.setTranTime("132030");
		 * 
		 * Reponse msgReponse = new Reponse();
		 * 
		 * msgReponse.setMsgHeader(msHeader); msgReponse.setReqBody(list);
		 * 
	...
	public static byte[] responseBlackList(List<Map<String, String>> mplist) throws UnsupportedEncodingException {
		...
		if (0 != mplist.size() && null != mplist) {
			...
			for (Map<String, String> li : mplist) {
				...
				sbf.append(
						fillStr(li.get(BlackListConstants.RES_AUTHORITY), BlackListConstants.RES_AUTHORITY_LEN, " "));
				sbf.append(
						fillStr(li.get(BlackListConstants.RES_MODIFYDATE), BlackListConstants.RES_MODIFYDATE_LEN, " "));
	...
	public static String fillStr(String curStr, int len, String addStr) {
		...
		if (curStr != null && !"".equals(curStr)) {
			if (curStr.length() == len) {
				...
				Log.warn("输入字符串超出限定长度------------------" + curStr + "实际长度为： " + curStr.length()); // 此处需要客户端
																									// 自定义(超长输入)异常。
	...
	public static Map<String, String> getRequst(byte[] b) {
		// 请求功能码:trancode
		String trancode = new String(b, BlackListConstants.REQ_TRANCODE_SUBLEN, BlackListConstants.REQ_TRANCODE_LEN,
				charset);
		// 银行代码:brcode
		String brcode = new String(b, BlackListConstants.REQ_BRCODE_SUBLEN, BlackListConstants.REQ_BRCODE_LEN, charset);
		// 信息类别:reqtype
		String reqtype = new String(b, BlackListConstants.REQ_REQTYPE_SUBLEN, BlackListConstants.REQ_REQTYPE_LEN,
				charset);
		// 查询信息req
		String req = new String(b, BlackListConstants.REQ_REQMSG_SUBLEN, BlackListConstants.REQ_REQMSG_LEN, charset);
		Log.info("处理接收信息：" + "trancode:" + trancode + "brcode:" + brcode + "reqtype:" + reqtype + "req:" + req);
		...
		// 0:根据证件查,1:根据账户查,2:根据中文姓名查,3:根据英文姓名查,4.根据卡号、折号查询
		// 只根据一个条件查
		if (reqtype.trim().length() == 1) {
			// 上送：证件类型+证件号码
			if ("0".equals(clientMap.get(BlackListConstants.REQ_REQTYPE))) {
				...
				clientMap.put(BlackListConstants.REQ_CERNUMB,
						req.substring(BlackListConstants.REQ_CERTYPE_LEN).trim());
				// 上送账户
			...
			char[] reqtypes = reqtype.trim().toCharArray();
			for (int i = 0; i < reqtypes.length; i++) {
				// 上送：证件类型+证件号码
				if ('0' == reqtypes[i]) {
					clientMap.put(BlackListConstants.REQ_CERTYPE,
							reqs[i].substring(0, BlackListConstants.REQ_CERTYPE_LEN).trim());

					clientMap.put(BlackListConstants.REQ_CERNUMB, reqs[i]
							.substring(BlackListConstants.REQ_CERTYPE_LEN).trim());
					// 上送账户
				} else if ('1' == reqtypes[i]) {
					// 根据账户
					clientMap.put(BlackListConstants.REQ_ACCNUM, reqs[i].trim());
					// 根据中文名查
				} else if ('2' == reqtypes[i]) {
					clientMap.put(BlackListConstants.REQ_CLIENAME, reqs[i].trim());
					// 根据英文
				} else if ('3' == reqtypes[i]) {
					clientMap.put(BlackListConstants.REQ_ENNAME, reqs[i].trim());
					// 卡、折号
				} else if ('4' == reqtypes[i]) {
					clientMap.put(BlackListConstants.REQ_CARDNO, reqs[i].trim());
				}
		...
		if (tranCode != null && !"".equals(tranCode) && "0001".equals(tranCode)) { // 功能码不为空且为"0001"
			...
			if (brCode != null && !"".equals(brCode)) {//
				...
				if (msType != null && !"".equals(msType)) {
					char[] types = msType.trim().toCharArray();
					String qur = "";
					for (int i = 0; i < types.length; i++) {
						...
						if ('0' == types[i] || '1' == types[i] || '2' == types[i] || '3' == types[i]
								|| '4' == types[i]) {
							if (i == (types.length - 1) && null != qur && !"".equals(qur)) {
					...
					Log.info("截取详细信息：" + "tranCode:" + tranCode + "brCode:" + brCode + "msType:" + msType + "qur:"
							+ qur);
