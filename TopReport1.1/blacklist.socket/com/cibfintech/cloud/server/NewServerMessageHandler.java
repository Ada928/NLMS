...
public class NewServerMessageHandler extends IoHandlerAdapter {
	...
	private void dealRequest(Map<String, String> clientMap, IoSession session, long t1) throws CommonException {
		...
		try {
			// 从商行黑名单查询==
			StringBuffer hql = new StringBuffer(" from NsBankBlackList po where po.del= 'F' and po.approve='T' ");
			// 从国际黑名单查询==
			StringBuffer hql2 = new StringBuffer(" from NsInternationalBlackList po where po.del= 'F' ");
			// 公安部黑名单查询==
			StringBuffer hql3 = new StringBuffer(" from NsPoliceBlackList po where po.del= 'F' ");
			boolean markb = false;
			boolean andor = false;
			boolean flag = true;
			// 银行代码
			...
			if (StringUtils.isNotBlank(clientMap.get(BlackListConstants.REQ_CERTYPE))
					&& StringUtils.isNotBlank(clientMap.get(BlackListConstants.REQ_CERNUMB))) {
				markb = true;
			...
			if (StringUtils.isNotBlank(clientMap.get(BlackListConstants.REQ_CLIENAME))) {
				markb = true;
				if (andor) {
					hql.append(" or po.clientName like '%").append(clientMap.get(BlackListConstants.REQ_CLIENAME))
							.append("%'");
					hql2.append(" or po.clientName like '%").append(clientMap.get(BlackListConstants.REQ_CLIENAME))
							.append("%'");
					hql3.append(" or po.clientName like '%").append(clientMap.get(BlackListConstants.REQ_CLIENAME))
							.append("%'");
				} else {
			...
			if (StringUtils.isNotBlank(clientMap.get(BlackListConstants.REQ_ENNAME))) {
				markb = true;
				if (andor) {
					hql.append(" or po.clientEnglishName like '%").append(clientMap.get(BlackListConstants.REQ_ENNAME))
							.append("%'");
					hql2.append(" or po.clientEnglishName like '%").append(clientMap.get(BlackListConstants.REQ_ENNAME))
							.append("%'");
					hql3.append(" or po.clientEnglishName like '%").append(clientMap.get(BlackListConstants.REQ_ENNAME))
							.append("%'");
				} else {
					hql.append(" and po.clientEnglishName like '%").append(clientMap.get(BlackListConstants.REQ_ENNAME))
							.append("%'");
					hql2.append(" and po.clientEnglishName like '%")
							.append(clientMap.get(BlackListConstants.REQ_ENNAME)).append("%'");
					hql3.append(" and po.clientEnglishName like '%")
							.append(clientMap.get(BlackListConstants.REQ_ENNAME)).append("%'");
					andor = true;
				}
			}
			...
			if (StringUtils.isNotBlank(clientMap.get(BlackListConstants.REQ_ACCNUM))) {
				...
				flag=false;
				if (andor) {
					hql.append(" or po.accountCode = '").append(clientMap.get(BlackListConstants.REQ_ACCNUM))
							.append("'");
					hql3.append(" or po.accountCode = '").append(clientMap.get(BlackListConstants.REQ_ACCNUM))
							.append("'");
				} else {
					hql.append(" and po.accountCode = '").append(clientMap.get(BlackListConstants.REQ_ACCNUM))
							.append("'");
					hql3.append(" and po.accountCode = '").append(clientMap.get(BlackListConstants.REQ_ACCNUM))
							.append("'");
					andor = true;
				}
			}
			...
			if (StringUtils.isNotBlank(clientMap.get(BlackListConstants.REQ_CARDNO))) {
				...
				flag=false;
				if (andor) {
					hql.append(" or po.cardBkBookNo = '").append(clientMap.get(BlackListConstants.REQ_CARDNO))
							.append("'");
					hql3.append(" or po.cardBkBookNo = '").append(clientMap.get(BlackListConstants.REQ_CARDNO))
							.append("'");
				} else {
					hql.append(" and po.cardBkBookNo = '").append(clientMap.get(BlackListConstants.REQ_CARDNO))
							.append("'");
			...
			if (null != bankBlacklists && bankBlacklists.size() != 0) {
				...
				for (NsBankBlackList blackList : bankBlacklists) {
					...
					// 客户类别 公C私I
					mp.put(BlackListConstants.RES_CLITYPE, blackList.getAccountType());
					// 客户名称
					mp.put(BlackListConstants.RES_CLINAME, blackList.getClientName());
					// 客户英文名称
					mp.put(BlackListConstants.RES_ENGNAME, blackList.getClientEnglishName());
					// 证件类型
					mp.put(BlackListConstants.RES_CETYPE, blackList.getCertificateType());
					// 证件号码
					mp.put(BlackListConstants.RES_CERNO, blackList.getCertificateNumber());

					if (BlackListConstants.REQ_CARDNOFLAG.equals(clientMap.get(BlackListConstants.REQ_REQTYPE))) {
						// 卡号、折号
						mp.put(BlackListConstants.RES_ACCNO, blackList.getCardBkBookNo());
					} else {
						// 账号
						mp.put(BlackListConstants.RES_ACCNO, blackList.getAccountCode());
					}
					// 名单种类（00:白 10：黑名单 20：灰名单）
					mp.put(BlackListConstants.RES_BLTYPE, blackList.getBlacklistType() + "0");
					// 名单类型（区别数据库类型 ,即是指B:商行、I:国际、P:公安部黑名单中的任意一个黑名单）
					mp.put(BlackListConstants.RES_AUTHORITY, BlackListConstants.RES_AUTHORITY_B);
					if (null != blackList.getLastModifyDate()) {
						// 变更时间
						mp.put(BlackListConstants.RES_MODIFYDATE, DateUtil.dateToString(blackList.getLastModifyDate()));
					}
			...
			if (null != interBlacklists && interBlacklists.size() != 0) {
				...
				for (NsInternationalBlackList blackList : interBlacklists) {
					...
					// 客户类别 公C私I
					mpi.put(BlackListConstants.RES_CLITYPE, blackList.getAccountType());
					// 客户名称
					mpi.put(BlackListConstants.RES_CLINAME, blackList.getClientName());
					// 客户英文名称
					mpi.put(BlackListConstants.RES_ENGNAME, blackList.getClientEnglishName());
					// 证件类型
					mpi.put(BlackListConstants.RES_CETYPE, blackList.getCertificateType());
					// 证件号码
					mpi.put(BlackListConstants.RES_CERNO, blackList.getCertificateNumber());
					// 名单种类（00:白 10：黑名单 20：灰名单）
					mpi.put(BlackListConstants.RES_BLTYPE, blackList.getBlacklistType() + "0");
					// 名单类型（区别数据库类型 ,即是指B:商行、I:国际、P:公安部黑名单中的任意一个黑名单）
					mpi.put(BlackListConstants.RES_AUTHORITY, BlackListConstants.RES_AUTHORITY_I);
					if (null != blackList.getLastModifyDate()) {
			...
			if (null != policeBlackLists && policeBlackLists.size() != 0) {
				...
				for (NsPoliceBlackList blackList : policeBlackLists) {
					...
					// 客户类别 公C私I
					mpp.put(BlackListConstants.RES_CLITYPE, blackList.getAccountType());
					// 客户名称
					mpp.put(BlackListConstants.RES_CLINAME, blackList.getClientName());
					// 客户英文名称
					mpp.put(BlackListConstants.RES_ENGNAME, blackList.getClientEnglishName());
					// 证件类型
					mpp.put(BlackListConstants.RES_CETYPE, blackList.getCertificateType());
					// 证件号码
					mpp.put(BlackListConstants.RES_CERNO, blackList.getCertificateNumber());
					if (BlackListConstants.REQ_CARDNOFLAG.equals(clientMap.get(BlackListConstants.REQ_REQTYPE))) {
						mpp.put(BlackListConstants.RES_ACCNO, blackList.getCardBkBookNo()); // 卡号、折号
					} else {
						mpp.put(BlackListConstants.RES_ACCNO, blackList.getAccountCode()); // 账号
					}
					// 名单种类（00:白 10：黑名单 20：灰名单）
					mpp.put(BlackListConstants.RES_BLTYPE, blackList.getBlacklistType() + "0");
					// 名单类型（区别数据库类型,即是指B:商行、I:国际、P:公安部黑名单中的任意一个黑名单）
					mpp.put(BlackListConstants.RES_AUTHORITY, BlackListConstants.RES_AUTHORITY_P);
					if (null != blackList.getLastModifyDate()) {
