...
public class BopDrDsService {
	...
	public void delete(MtsBopDrDs mtsBopDrDs, String currentfile) throws CommonException {
		...
		htLog.info("Updater.log",
				new String[] { globalInfo.getTlrno(), globalInfo.getBrno(), "境内收入申报单" + currentfile + "删除" });
	}
	...
	public void update(MtsBopDrDs mbDr, String currentfileMessage) throws CommonException {
		...
		rootDao.saveOrUpdate(mtsBopDrDs);
		htLog.info("Updater.log",
				new String[] { globalInfo.getTlrno(), globalInfo.getBrno(), "境内收入申报单" + currentfileMessage + "修改" });
	}
	...
	private MtsBopDrDs bopDrUpdateSetValue(MtsBopDrDs mbDr, String currentfile) throws CommonException {
		...
		if (!StringUtils.isEmpty(mbDr.getSubSuccess())
			...
			/**
			 * 上报已成功 操作状态=C-修改 记录状态=02-编辑待确认 审核状态=00-未审核 回执状态=00-未返回 是否已成功上报=不变化
			 */
			...
			/**
			 * 上报未成功 操作状态=A-创建 记录状态=02-编辑待确认 审核状态=00-未审核 回执状态=00-未返回 是否已成功上报=不变化
			 */
			mbDr.setActiontype(TopReportConstants.REPORT_ACTIONTYPE_A);
		}
		mbDr.setRecStatus(TopReportConstants.REPORT_RECSTATUS_02);
	 ...
	 *            要审核的实体List
	 */
	public void AuditMtsBopDrDs(String approveStatusChoose, String approveResultChoose, List<MtsBopDrDs> MtsBopDrDsList)
			throws CommonException {
		...
		for (MtsBopDrDs mtsBopDrDs : qMtsBopDrDsList) {
			...
			if (mtsBopDrDs.getActiontype().equals(TopReportConstants.REPORT_ACTIONTYPE_D)
					&& mtsBopDrDs.getSubSuccess().equals(TopReportConstants.REPORT_IS_SUB_SUCCESS_YES)) {
				// 数据处理记录表保存
				commonService.saveBiDataProcessLog(mtsBopDrDs.getApptype(), mtsBopDrDs.getCurrentfile(),
						mtsBopDrDs.getId(), mtsBopDrDs.getRptno(), TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_AUDIT,
						approveStatusChooseName, mtsBopDrDs.getActiondesc());
			} else {
				// 数据处理记录表保存
				commonService.saveBiDataProcessLog(mtsBopDrDs.getApptype(), mtsBopDrDs.getCurrentfile(),
						mtsBopDrDs.getId(), mtsBopDrDs.getRptno(), TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_AUDIT,
