...
public class BopDrDsOperation extends BaseOperation {
	...
	public void execute(OperationContext context) throws CommonException {
		...
		if (CMD_INSERT.equalsIgnoreCase(cmd)) {
			...
			// 记录操作日志
			reportCommonService.saveBiDataProcessLog(mtsBopDrDs.getApptype(), mtsBopDrDs.getCurrentfile(),
					mtsBopDrDs.getId(), mtsBopDrDs.getRptno(), TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_EDIT,
					"A-创建", "境内收入申报单-" + currentfileMessage + "  " + currentfileMessage + "ID ：" + mtsBopDrDs.getId());
		}

		else if (CMD_UPDATE.equalsIgnoreCase(cmd)) {
			// 状态非01 02的不能修改
			if (!(TopReportConstants.REPORT_RECSTATUS_02.equalsIgnoreCase(mtsBopDrDs.getRecStatus())
					|| TopReportConstants.REPORT_RECSTATUS_01.equalsIgnoreCase(mtsBopDrDs.getRecStatus()))) {
				ExceptionUtil.throwCommonException("不能修改当前管理信息记录,当前记录状态为    " + mtsBopDrDs.getRecStatus() + "");
			}
			...
			if (!(TopReportConstants.REPORT_RECSTATUS_02.equalsIgnoreCase(mtsBopDrDs.getRecStatus())
				...
				ExceptionUtil.throwCommonException(
						"不能删除当前" + currentfileMessage + "记录,当前记录状态为    " + mtsBopDrDs.getRecStatus() + "");
			}
			// 状态改为删除状态
			bopDrDsService.delete(mtsBopDrDs, currentfileMessage);
			// 记录日志
			reportCommonService.saveBiDataProcessLog(mtsBopDrDs.getApptype(), mtsBopDrDs.getCurrentfile(),
					mtsBopDrDs.getId(), mtsBopDrDs.getRptno(), TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_DEL,
					"D-删除", "删除境内收入申报单" + currentfileMessage + "  " + currentfileMessage + "ID ：" + mtsBopDrDs.getId());
		}
	}
