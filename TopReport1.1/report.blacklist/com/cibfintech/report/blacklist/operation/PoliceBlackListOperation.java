...
public class PoliceBlackListOperation extends BaseOperation {
	...
	public void execute(OperationContext context) throws CommonException {
		...
		if (CMD_DEL.equals(cmd)) {
			...
			try {
				taskInfo = ReportTaskUtil.getSysTaskInfoBean(ReportEnum.REPORT_TASK_FUNCID.TASK_110599.value,
						ReportEnum.REPORT_TASK_TRANS_CD.DEL.value, policeBlackList, policeBlackList.getId(),
			...
			gi.addBizLog("Updater.log", new String[] { gi.getTlrno(), gi.getBrcode(), "公安部黑名单的删除" });
			htlog.info("Updater.log", new String[] { gi.getBrcode(), gi.getTlrno(), "公安部黑名单的删除" });
			...
			try {
				taskInfo = ReportTaskUtil.getSysTaskInfoBean(ReportEnum.REPORT_TASK_FUNCID.TASK_110599.value,
						ReportEnum.REPORT_TASK_TRANS_CD.NEW.value, policeBlackList, policeBlackList.getId(),
						policeBlackList.getOperateState());
				service.addTosystaskinfo(taskInfo);
			} catch (IOException e) {
				e.printStackTrace();
			...
			GlobalInfo gi = GlobalInfo.getCurrentInstance();
			gi.addBizLog("Updater.log", new String[] { gi.getTlrno(), gi.getBrcode(), "公安部黑名单的增加" });
			htlog.info("Updater.log", new String[] { gi.getBrcode(), gi.getTlrno(), "公安部黑名单的增加" });
			...
			sys1.setIsValid(policeBlackList.getIsValid());
			...
			try {
				taskInfo = ReportTaskUtil.getSysTaskInfoBean(ReportEnum.REPORT_TASK_FUNCID.TASK_110599.value,
						ReportEnum.REPORT_TASK_TRANS_CD.EDIT.value, policeBlackList, policeBlackList.getId(),
						policeBlackList.getOperateState());
				service.addTosystaskinfo(taskInfo);
			} catch (IOException e) {
				e.printStackTrace();
		...
		GlobalInfo gi = GlobalInfo.getCurrentInstance();
		gi.addBizLog("Updater.log", new String[] { gi.getTlrno(), gi.getBrcode(), "公安部黑名单的编辑" });
		htlog.info("Updater.log", new String[] { gi.getBrcode(), gi.getTlrno(), "公安部黑名单的编辑" });
	}
