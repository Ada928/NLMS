...
public class JshEgDsService {
	...
	public void delete(MtsJshDefgDs mtsJshDefgDs, String currentFile) throws CommonException {
		...
		htLog.info("Updater.log",
				new String[] { globalInfo.getTlrno(), globalInfo.getBrno(), "外汇账户内购汇" + currentFile + "删除" });
	}
	...
	public void save(MtsJshDefgDs mtsJshDefgDs, String message) throws CommonException {
		...
		if (null != mdTemp) {
			...
			htLog.info("Updater.log",
					new String[] { globalInfo.getTlrno(), globalInfo.getBrno(), "外汇账户内购汇" + message + "保存" });
		}
	}
	...
	public void update(MtsJshDefgDs mtsJshDefgDs, String message) throws CommonException {
		...
		htLog.info("Updater.log",
				new String[] { globalInfo.getTlrno(), globalInfo.getBrno(), "外汇账户内购汇" + message + "修改" });
	}
	 ...
	 *            审核结果
	 * @param MtsJshDefgDsList
	 *            要审核的实体List
	 */
	public void AuditMtsJshDefgDs(String approveStatusChoose, String approveResultChoose,
			List<MtsJshDefgDs> MtsJshDefgDsList) throws CommonException {
		...
		for (MtsJshDefgDs mtsJshDefgDs : qMtsJshDefgDsList) {
			...
			if (mtsJshDefgDs.getActiontype().equals(TopReportConstants.REPORT_ACTIONTYPE_D)
					&& mtsJshDefgDs.getSubSuccess().equals(TopReportConstants.REPORT_IS_SUB_SUCCESS_YES)) {
				// 数据处理记录表保存
				commonService.saveBiDataProcessLog(mtsJshDefgDs.getApptype(), mtsJshDefgDs.getCurrentfile(),
						mtsJshDefgDs.getId(), mtsJshDefgDs.getRptno(),
						TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_AUDIT, approveStatusChooseName,
						mtsJshDefgDs.getActiondesc());
			} else {
				// 数据处理记录表保存
				commonService.saveBiDataProcessLog(mtsJshDefgDs.getApptype(), mtsJshDefgDs.getCurrentfile(),
						mtsJshDefgDs.getId(), mtsJshDefgDs.getRptno(),
