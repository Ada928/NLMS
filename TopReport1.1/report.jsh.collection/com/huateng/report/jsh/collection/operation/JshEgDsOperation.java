...
public class JshEgDsOperation extends BaseOperation {
	...
	public void execute(OperationContext context) throws CommonException {
		...
		if (CMD_INSERT.equalsIgnoreCase(cmd)) {
			...
			reportCommonService.saveBiDataProcessLog(mtsJshDefgDs.getApptype(), mtsJshDefgDs.getCurrentfile(),
					mtsJshDefgDs.getId(), mtsJshDefgDs.getRptno(), TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_EDIT,
					"A-创建", "外汇账户内购汇-" + message + "  " + message + "ID ：" + mtsJshDefgDs.getId());
		}

		else if (CMD_UPDATE.equalsIgnoreCase(cmd)) {
			if (!(TopReportConstants.REPORT_RECSTATUS_02.equalsIgnoreCase(mtsJshDefgDs.getRecStatus())
					|| TopReportConstants.REPORT_RECSTATUS_01.equalsIgnoreCase(mtsJshDefgDs.getRecStatus()))) {
				ExceptionUtil.throwCommonException("不能修改当前基础信息记录,当前记录状态为    " + mtsJshDefgDs.getRecStatus() + "");
			}
			...
			reportCommonService.saveBiDataProcessLog(mtsJshDefgDs.getApptype(), mtsJshDefgDs.getCurrentfile(),
					mtsJshDefgDs.getId(), mtsJshDefgDs.getRptno(), TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_EDIT,
					"C-修改", "更新外汇账户内购汇-" + message + "  " + message + "ID ：" + mtsJshDefgDs.getId());
		} else if (CMD_DELETE.equalsIgnoreCase(cmd)) {
			// 状态非01 02的不能删除
			if (!(TopReportConstants.REPORT_RECSTATUS_02.equalsIgnoreCase(mtsJshDefgDs.getRecStatus())
					|| TopReportConstants.REPORT_RECSTATUS_01.equalsIgnoreCase(mtsJshDefgDs.getRecStatus()))) {
				ExceptionUtil.throwCommonException("不能删除当前基础信息记录,当前记录状态为    " + mtsJshDefgDs.getRecStatus() + "");
			}
			jshDEgDsService.delete(mtsJshDefgDs, message);

			reportCommonService.saveBiDataProcessLog(mtsJshDefgDs.getApptype(), mtsJshDefgDs.getCurrentfile(),
					mtsJshDefgDs.getId(), mtsJshDefgDs.getRptno(), TopReportConstants.REPORT_DATAPROCESS_EXECTYPE_EDIT,
					"D-删除", "删除外汇账户内购汇-" + message + "  " + message + "ID ：" + mtsJshDefgDs.getId());
		}
	}
