...
public class FieldImpl extends Field {
	...
	public String outputString() throws AppException {
		...
		if (StringUtils.isEmpty(getStatus()) || StringUtils.equalsIgnoreCase(getStatus(), STATUS_M)
			...
			if (StringUtils.isEmpty(getType()) || StringUtils.equalsIgnoreCase(getType(), FIELD_TYPE_STRING)) { // 字符串类型
				...
				// value = StringUtils.leftPad(String.valueOf(lvAbS),
				// getLength()-1, '0');
				if (lv >= 0) {
			...
			if (value.length() > getLength()) {
				...
				throw new AppException(Module.SYSTEM_MODULE, Rescode.PARSE_BUFFER_ERROR,
						"Field[" + getId() + "] length too long , " + value.length() + ">" + getLength());
			}

			if (this.getParent() != null
					&& StringUtils.equalsIgnoreCase(this.getParent().getType(), BUFFER_TYPE_STRUCT)) {
				return value;
			} else {
