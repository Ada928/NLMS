...
public class BopCfaStrdeDsAuditOperation extends BaseOperation {
	...
	public void execute(OperationContext context) throws CommonException {
		...
		if (CONTRACT_AUDIT_CMD.equalsIgnoreCase(cmd)) {
			// 签约信息审核
			List<BopCfaStrdeDs> bopCfaStrdeDsList = (List<BopCfaStrdeDs>) context
					.getAttribute(CONTRACT_AUDIT_LIST_IN_PARAM);
			service.contract_audit(bopCfaStrdeDsList, approveStatusChoose, approveResultChoose);
			globalInfo.addBizLog("Updater.log",
					new String[] { globalInfo.getTlrno(), globalInfo.getBrcode(), "商业银行人名币结构性存款签约信息审核" });
			htlog.info("Updater.log",
					new String[] { globalInfo.getTlrno(), globalInfo.getBrno(), "商业银行人名币结构性存款签约信息审核" });
		} else if (TERMINATE_AUDIT_CMD.equals(cmd)) {
			// 终止信息审核
			List<BopCfaStrdeDs> bopCfaStrdeDsList = (List<BopCfaStrdeDs>) context
					.getAttribute(TERMINATE_AUDIT_LIST_IN_PARAM);
			service.terminate_audit(bopCfaStrdeDsList, approveStatusChoose, approveResultChoose);
			globalInfo.addBizLog("Updater.log",
					new String[] { globalInfo.getTlrno(), globalInfo.getBrcode(), "商业银行人名币结构性存款终止信息审核" });
			htlog.info("Updater.log",
					new String[] { globalInfo.getTlrno(), globalInfo.getBrno(), "商业银行人名币结构性存款终止信息审核" });
		} else if (INPAY_AUDIT_CMD.equals(cmd)) {
			// 利息给付信息审核
			List<BopCfaStrdeDs> bopCfaStrdeDsList = (List<BopCfaStrdeDs>) context
					.getAttribute(INPAY_AUDIT_LIST_IN_PARAM);
			service.inpay_audit(bopCfaStrdeDsList, approveStatusChoose, approveResultChoose);
			globalInfo.addBizLog("Updater.log",
					new String[] { globalInfo.getTlrno(), globalInfo.getBrcode(), "商业银行人名币结构性存款利息给付信息审核" });
			htlog.info("Updater.log",
					new String[] { globalInfo.getTlrno(), globalInfo.getBrno(), "商业银行人名币结构性存款利息给付信息审核" });
		} else if (INOUTMO_AUDIT_CMD.equals(cmd)) {
			// 资金流出入和结购汇信息
			List<BopCfaStrdeDs> bopCfaStrdeDsList = (List<BopCfaStrdeDs>) context
					.getAttribute(INOUTMO_AUDIT_LIST_IN_PARAM);
			service.inoutMo_audit(bopCfaStrdeDsList, approveStatusChoose, approveResultChoose);
			globalInfo.addBizLog("Updater.log",
					new String[] { globalInfo.getTlrno(), globalInfo.getBrcode(), "商业银行人名币结构性存款资金流出入和结购汇信息审核" });
			htlog.info("Updater.log",
					new String[] { globalInfo.getTlrno(), globalInfo.getBrno(), "商业银行人名币结构性存款资金流出入和结购汇信息审核" });
		}
	}
