 ...
 *
 * @hibernate.class table="BI_ANALY_PROCESS"
 */
...
public abstract class BaseBiAnalyProcess implements Serializable {
	...
	public boolean equals(Object obj) {
		if (null == obj)
			return false;
		if (!(obj instanceof resource.bean.report.BiAnalyProcess))
			return false;
		else {
			resource.bean.report.BiAnalyProcess biAnalyProcess = (resource.bean.report.BiAnalyProcess) obj;
			if (null == this.getId() || null == biAnalyProcess.getId())
				return false;
			else
				return (this.getId().equals(biAnalyProcess.getId()));
		}
	}
