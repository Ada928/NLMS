 ...
 *
 * @hibernate.class table="BI_BOPJSH_RET_NO"
 */
...
public abstract class BaseBiBopjshRetNo implements Serializable {
	...
	public boolean equals(Object obj) {
		if (null == obj)
			return false;
		if (!(obj instanceof resource.bean.report.BiBopjshRetNo))
			return false;
		else {
			resource.bean.report.BiBopjshRetNo biBopjshRetNo = (resource.bean.report.BiBopjshRetNo) obj;
			if (null == this.getId() || null == biBopjshRetNo.getId())
				return false;
			else
				return (this.getId().equals(biBopjshRetNo.getId()));
		}
	}
