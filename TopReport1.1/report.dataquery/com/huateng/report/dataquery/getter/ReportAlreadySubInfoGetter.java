...
public class ReportAlreadySubInfoGetter extends BaseGetter {
	...
	private PageQueryResult getData() throws CommonException {
		...
		// 已上报文件信息
		List<AlreadySubInfo> list = rootdao
				.queryByQL2List(" from AlreadySubInfo model where model.fileName='" + subFileInfo.getId()
						+ "' and model.apptype='" + appType + "' and model.currentfile='" + currentfile + "'");
		...
		for (AlreadySubInfo alreadySubInfo : list) {
			// 返回错误消息实体
			ReportBackErrBean backErrBean = ReportUtils.getReportBankMsg(alreadySubInfo.getRecId(),
					subFileInfo.getApptype(), subFileInfo.getCurrentfile());
			if (backErrBean != null) {
