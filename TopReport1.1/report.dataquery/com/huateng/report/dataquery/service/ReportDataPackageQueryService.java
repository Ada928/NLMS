...
public class ReportDataPackageQueryService {
	...
	public synchronized static ReportDataPackageQueryService getInstance() {
		return (ReportDataPackageQueryService) ApplicationContextUtils
				.getBean(ReportDataPackageQueryService.class.getName());
	}

	public AlreadySubInfo getAlreadySubInfo(String recid, String busiType, String fileType) throws CommonException {
		ROOTDAO rootDao = ROOTDAOUtils.getROOTDAO();
		List list = rootDao.queryByQL2List(" from AlreadySubInfo model where model.recId='" + recid
				+ "' and model.apptype='" + busiType + "' and model.currentfile='" + fileType + "'");
		if (list.size() == 1) {
	...
	public List getReportErrDetailList(String busiCode, String repFileName, String busiType, String fileType)
			throws CommonException {
		ROOTDAO rootDao = ROOTDAOUtils.getROOTDAO();
		StringBuffer hql = new StringBuffer();
		hql.append(" from RepFileErrDet model where model.repFileName='" + repFileName + "' and model.apptype='"
				+ busiType + "' and model.currentfile='" + fileType + "'");
