 ...
 * Shanghai Huateng Software Systems Co., Ltd. 查询结果导出处理类,继承自
 * <com.huateng.mvc.struts.action.BaseAction>， 如不满足需求，可继承QryExpAction，
 * 并重写wirtHead、wirtDetail和wirtTail方法
 *
 * @author shen_antonio
...
public class QryExpAction extends com.huateng.mvc.struts.action.BaseAction {
	...
	public ActionForward safeExecute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
		...
		// String els =
		// request.getParameter(cqId+"_"+QueryExportForm.P_COL_SORT_NAME);
	...
	public String proBatch(QueryExportForm form) {
		...
		if (form.getColumnSort() != null) {
			...
			// params.put(QueryExportForm.P_COL_SORT_NAME,
			// form.getColumnSort());
		}
		// 结束页码
		...
		try {
			...
			params.put("perfetch",
					StringUtils.isBlank(perfetch) ? ConfigReader.getProperty("PageQryExp_perfetch") : perfetch);
		...
		// 未设置取描述信息
		if (DataFormat.isEmpty(fileName))
			fileName = desc;
		...
		msg += "<div><span>文件名为:</span><span style='font-weight:bold;color:blue'>" + desc + "."
				+ (form.isZipFlag() ? "zip" : form.getExportType()) + "</span></div></div>";
		if (error != null)
			msg = "批量下载执行失败:" + error;
		StringBuffer rspStr = new StringBuffer();
		rspStr.append("<script language=\"javascript\" src=\"" + form.getRequest().getContextPath()
				+ "/templets/lib/popup_message.js\"></script>");
		...
		rspStr.append(
				"POPUP_MESSAGE.prototype.oncommand=function(){window.top.doWork('999998','批量下载','/fpages/pageqryexp/ftl/mydownload.ftl');}");
		rspStr.append("</script>");
		// return "<script
		// language=\"javascript\">alert(\""+"批量下载已成功执行！"+"\");</script>";
