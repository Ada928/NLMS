...
import com.huateng.ebank.business.common.generator.GetBizLogNoGentator;
...
public abstract class BaseGetter extends _CallGetter {
	...
	private static final Log log = LogFactory.getLog(BaseGetter.class);
	...
	public ResultParamBean process(Map param, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			try {
				...
				if (ginfo != null) {
					// 记录查询日志
					if (param.containsKey(ReportConstant.SAVE_QUERY_LOG_SIGN)) {
						String isSaveLog = (String) param.get(ReportConstant.SAVE_QUERY_LOG_SIGN);
						if (isSaveLog != null && isSaveLog.equals("1") && "1".equals(ginfo.getSaveQueryLog())) {// 记录日志
							String busiName = getValueFromDataBus(ReportConstant.QUERY_LOG_BUSI_NAME);
							if (busiName != null && busiName.trim().length() > 0) {
								ginfo.addBizLog("Updater.log",
										new String[] { ginfo.getTlrno(), ginfo.getBrno(), busiName });
							}
						}
					}
				}
			...
			if (funcLogFlag != null && !funcLogFlag.equals("")) {
				if (txnLogFlag != null && txnLogFlag.equals("true") && funcLogFlag.equals("true")) {
					...
					if (list != null && list.size() != 0) {
						logger.printNoLevel("插入操作员日志信息 ", logger.lever_txnLog, true);
						for (int i = 0; i < list.size(); i++) {
							// 取得操作员的流水号
							try {
								String sequence = ((GetBizLogNoGentator) GeneratorFactory
										.getGenerator("GetBizLogNoGentator")).gen(null);
								// String
								// sequence=getBizLogNoGentator.gen(null);
								// Integer sequence =
								// GeneratorFactory.getDialectSeq().getSeqNo("SEQ_BIZ_LOG_TLSRNO");
								// 将操作员的流水号写入文件日志
								globalInfo.setBaseTlsrno(sequence);
								logger.printNoLevel("操作员的流水号:" + sequence, logger.lever_txnLog, true);
								BizLogThreadPoolExecutor.addTask(globalInfo, globalInfo.getBaseTlsrno(),
										(String) list.get(i), txnLogFlag, uuId, oprTxnCd);
