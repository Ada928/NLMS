	...
	public int doStartTag() throws JspException {
		try {
			...
			if (retCode == null) {
				...
				pageContext.getOut().print(webPath + "images/denglu.gif");
			} else if (retCode.equals(ErrorCode.ERROR_CODE_TLR_INFO_SELECT)
					|| retCode.equals(ErrorCode.ERROR_CODE_USER_NOT_EXIST)) {
			...
			} else if (retCode.equals(ErrorCode.ERROR_CODE_TLRNO_SESSION_BINDED)
					|| retCode.equals(ErrorCode.ERROR_CODE_TLRNO_SESSION_INVALID)) {
				pageContext.getOut().print(webPath + "images/denglu_session_invalidate.gif");
			}
			// else if (retCode.equals(ErrorCode.ERROR_CODE_COMMIT_TRANSACTION))
			// { //add by Farly.yu 2007-04-24 流水重复
			...
			throw new JspException("[pocompl] Exception in LoginBkgroundTag : condition " + e.getMessage());
		}
		return (SKIP_BODY);
	} // --doStartTag
