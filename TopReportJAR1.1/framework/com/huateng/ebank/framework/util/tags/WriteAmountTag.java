...
public class WriteAmountTag extends TagSupport {
	...
	/**
	 * The format string to be used format to convert value to String.
	 */
	 ...
	 *
	 * @param formatKey
	 *            value to use key to search message in bundle
	 * @exception JspException
	 *                if a JSP exception has occurred
	 */
	...
	protected String formatValue(Object valueToFormat) throws JspException {
		...
		if (value instanceof Number) {
			if ((value instanceof Byte) || (value instanceof Short) || (value instanceof BigDecimal)
		...
		if (this.inChinese)
			return toFixedLengthString(DataFormat.currencyToChineseCurrency(moneyLA));
		else
			return moneyLA;
	}
	...
	protected String toFixedLengthString(String inString) {
		...
		if (this.length <= _inString.length()) {
			...
			for (int i = 0; i < this.length - _inString.length(); i++)
				sb.append(" ");
			return sb.toString();
		}
	}
