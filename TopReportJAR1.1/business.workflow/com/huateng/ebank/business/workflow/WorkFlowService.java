...
/**
 * Title: WorkFlowService Description: 工作流服务:负责吊起工作流和任务分配 Copyright: Copyright
...
public class WorkFlowService {
	...
	public synchronized static WorkFlowService getInstance() {
		/*
		 * if (null == single) { single = new WorkFlowService(); } return
		 * single;
		 */
	...
	/**
	 * 启动工作流： 处理流程： 请求TaskAssignService服务，获取分配操作员号 启动工作流程
	 * 请求TaskAssignService服务，记录操作员工作量统计表信息
	 * 
	 * @param wokrFlowStartBean
	 *            启动工作流Bean
	 * @return
	...
	public WIFlowInstance startFlow(WorkFlowStartBean workFlowStartBean) throws CommonException {
		try {
			 ...
			 * TaskAssignService.getInstance(); String tlrno =
			 * taskAssignService.getAssignToTlrno(workFlowStartBean
			...
			WIFlowInstance wiFlowInstance = workFlowDAO.startFlow(globalInfo.getTlrno(), "", actors,
					workFlowStartBean.getFlowName(), workFlowStartBean.getVersion(), workFlowStartBean.getStatus(),
					workFlowStartBean.getAttribute(), workFlowStartBean.getKey());
			// 请求任务分配服务，记录操作员工作量统计表
			/*
			 * modify by shen_antonio 20080814 现在任务分配由统一模块处理 TaskAssignInfoBean
			 * taskAssignInfoBean = new TaskAssignInfoBean(
			 * wiFlowInstance.getFID(), wiFlowInstance.getNextTaskId(),
			 * wiFlowInstance.getCurrentUserName(), wiFlowInstance
			 * .getNextTaskId(), workFlowStartBean.getAssignMode(),
			 * DEFAULT_ASSIGN, workFlowStartBean.getWorkType(),
			 * workFlowStartBean .getBrcode());
			...
			ExceptionUtil.throwCommonException(wex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_START_ERROR);
		} catch (Exception ex) {
	...
	/**
	 * 执行任务 处理流程： 请求TaskAssignService服务，获取分配操作员号 请求TaskAssignService服务，完成上部任务
	 * 调用工作流处理任务，执行该任务 请求TaskAssignService服务，记录操作员工作量统计表信息
	 * 
	...
	public WITask doTask1(WorkFlowDoTaskBean workFlowDoTaskBean) throws CommonException {
		try {
			...
			/**
			 * add by jornezhang 20100519 BMS-2755 所有审批岗不填写审批结果，直接点击提交，系统报错
			 * begin
			 */
			...
			/** 最后的节点使用. */
			/*
			 * modify by shen_antonio 20080814 现在任务分配由统一模块处理 if(
			 * workFlowDoTaskBean.getTlrnoList() == null ||
			 * workFlowDoTaskBean.getTlrnoList().size() == 0){ tlrno = "";
			 * }else{ tlrno = taskAssignService.getAssignToTlrno(
			...
			rightNow.set(globalData.getTxdate().getYear(), globalData.getTxdate().getMonth(),
					globalData.getTxdate().getDate(), globalData.getTxtime().getHours(),
					globalData.getTxtime().getMinutes(), globalData.getTxtime().getSeconds());
			// 完成任务，记录操作员工作量统计表
			...
			WITask nextTask = workFlowDAO.doTask(String.valueOf(globalData.getTlrno()), "password",
					workFlowDoTaskBean.getTaskId(), actors, workFlowDoTaskBean.getStatus(),
					workFlowDoTaskBean.getAttribute(), workFlowDoTaskBean.getComment());
			...
			taskAssignService.insertOrUpdateTlrWorkload(workFlowDoTaskBean.getTlrno(), null, null,
					SystemConstant.TASK_ASSIGN_MODE_1, true, false);
			// 记录工作流任务信息表，更新当前任务信息 Added by UU_Wu 2008-6-24
			WorkflowTaskInfoDAO workflowTaskInfoDAO = BaseDAOUtils.getWorkflowTaskInfoDAO();
			List undoWorkflowTaskInfoList = workflowTaskInfoDAO
					.queryByCondition("po.procInsId = '" + workFlowDoTaskBean.getProcInsId() + "' and po.taskEndFlag ='"
							+ SystemConstant.FLAG_OFF + "' order by id");
			...
			// 更新WorkFlowAppInfo
			// WorkFlowAppInfoDAO workflowAppInfoDAO =
			// DAOUtils.getWorkFlowAppInfoDAO();
			// WorkflowAppInfo workflowAppInfo =
			// workflowAppInfoDAO.queryByProcInsId(workFlowDoTaskBean.getProcInsId());
			...
			if (nextTask != null) {
				/*
				 * modify by shen_antonio 20080814 现在任务分配由统一模块处理
				 * TaskAssignInfoBean taskAssignInfoBean = new
				 * TaskAssignInfoBean( nextTask.getProcInsId(),
				 * nextTask.getTaskId(), tlrno, nextTask.getTaskId(),
				 * workFlowDoTaskBean.getAssignMode(), DEFAULT_ASSIGN,
				 * workFlowDoTaskBean.getWorkType(),
				 * workFlowDoTaskBean.getBrcode());
				 * taskAssignService.assignTask(taskAssignInfoBean);
				 */
				return nextTask;
			} else {
				...
				List workflowInsRouteList = workflowInsRouteDAO.queryByCondition(" po.piid = ?",
						new Object[] { workFlowDoTaskBean.getProcInsId() }, new Type[] { Hibernate.STRING });
			...
			ExceptionUtil.throwCommonException(ex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_DOFINISH_ERROR, ex);
		}
		return null;
	}
	...
	public WITask doTask(WorkFlowDoTaskBean workFlowDoTaskBean) throws CommonException {
		try {
			...
			// 完成任务，记录操作员工作量统计表
			/*
			 * TaskFinishBean taskFinishBean = new
			 * TaskFinishBean(workFlowDoTaskBean.getProcInsId(),
			 * workFlowDoTaskBean.getTaskId(),workFlowDoTaskBean.getAssignMode()
			 * , workFlowDoTaskBean.getTlrno(),workFlowDoTaskBean.getWorkType(),
			 * workFlowDoTaskBean.getBrcode());
			 * taskAssignService.doFinishTask(taskFinishBean);
			 */
			// 更新流程参数中的上个任务
			...
			this.setTaskValue(taskId, map);
			// GlobalInfo.getCurrentInstance().setTaskId(Long.parseLong(taskId));
			WITask nextTask = workFlowDAO.doTask(globalInfo.getTlrno(), "password", workFlowDoTaskBean.getTaskId(),
					actors, workFlowDoTaskBean.getStatus(), workFlowDoTaskBean.getAttribute(),
					workFlowDoTaskBean.getComment());

			// 记录工作流任务信息表，更新当前任务信息 Added by UU_Wu 2008-6-24
			WorkflowTaskInfoDAO workflowTaskInfoDAO = BaseDAOUtils.getWorkflowTaskInfoDAO();
			// WorkflowTaskInfo workflowTaskInfo =
			// BaseDAOUtils.getWorkflowTaskInfoDAO().queryById(Long.valueOf(workFlowDoTaskBean.getTaskId()));
			WorkflowTaskInfo workflowTaskInfo = null;
			List undoWorkflowTaskInfoList = workflowTaskInfoDAO.queryByCondition("po.procInsId = '"
					+ workFlowDoTaskBean.getProcInsId() + "' and po.taskEndFlag ='" + SystemConstant.FLAG_OFF + "' ");
			...
			// workflowTaskInfo.setTaskEndFlag(SystemConstant.VALID_FLAG_VALID);
			// workflowTaskInfo.setTaskEndTime(new
			...
			ExceptionUtil.throwCommonException(wex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_DOFINISH_ERROR);
		} catch (Exception ex) {
			ExceptionUtil.throwCommonException(ex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_DOFINISH_ERROR);
		}
		return null;
	}

	/**
	 * 任务移交(支持移交指定工作和全部工作) 处理过程： 请求TaskAssignService服务，移交工作 调用工作流处理任务，执行该任务移交
	 * 
	...
	public void forwardTask(WorkFlowForwardTaskBean workFlowForwardTaskBean, boolean allFlag) throws CommonException {
		try {
			...
			// 移交指定工作
			if (!allFlag) {
				taskAssignService.forwardTask(workFlowForwardTaskBean.getProcInsId(),
						workFlowForwardTaskBean.getTaskId(), workFlowForwardTaskBean.getTlrno(),
						workFlowForwardTaskBean.getBrcode());
				workFlowDAO.forwardTask(workFlowForwardTaskBean.getTaskId(), workFlowForwardTaskBean.getTlrno());
				// 移交全部工作
			} else {
				List forwoadTaskList = taskAssignService.forwardTask(workFlowForwardTaskBean.getOTlrno(),
						workFlowForwardTaskBean.getTlrno(), workFlowForwardTaskBean.getBrcode());
				workFlowDAO.forwardAllTask(workFlowForwardTaskBean.getOTlrno(), "password",
						workFlowForwardTaskBean.getTlrno());
			}
		} catch (CommonException cex) {
			throw cex;
		} catch (WIException wex) {
			ExceptionUtil.throwCommonException(wex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_FORWARD_ERROR);
		} catch (Exception ex) {
	 ...
	 *            操作员号或者岗位号
	 * @param taskState
	 *            任务状态 允许为空，默认为所有潜在任务和私有任务
	 *            WorkFlowServiceHelper.TASK_STATUS_CLAIMED 已经声明的任务
	 *            WorkFlowServiceHelper.TASK_STATUS_READY 未声明的潜在任务
	 * @return
	 * @throws CommonException
	 */
	public WIPageQueryResult getTaskList(String tlrno, String taskState, Integer pageIndex, Integer pageSize,
			Map transMap) throws CommonException {
		try {
			// 请求工作流DAO
			WorkFlowDAO workFlowDAO = BaseDAOUtils.getWorkFlowDAO();
			return workFlowDAO.getTaskList(tlrno, "password", taskState, pageIndex, pageSize, transMap);
		} catch (WIException wex) {
			ExceptionUtil.throwCommonException(wex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_GETTASKLIST_ERROR);
		} catch (Exception ex) {
			ExceptionUtil.throwCommonException(ex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_GETTASKLIST_ERROR);
		}
		return null;
	}
	 ...
	 *            流程名
	 * @param version
	 *            流程版本号（允许为空，默认获取所有的）,或者FLOW_ALL_VERSION，FLOW_LAST_VERSION
	 * @param taskState
	 *            任务状态 允许为空，默认为所有潜在任务和私有任务
	 *            WorkFlowServiceHelper.TASK_STATUS_CLAIMED 已经声明的任务
	 *            WorkFlowServiceHelper.TASK_STATUS_READY 未声明的潜在任务
	 ...
	 * @throws CommonException
	 */
	public WIPageQueryResult getTaskList(String tlrno, String flowName, String version, String taskState,
			Integer pageIndex, Integer pageSize, Map transMap) throws CommonException {
		try {
			// 请求工作流DAO
			WorkFlowDAO workFlowDAO = BaseDAOUtils.getWorkFlowDAO();
			return workFlowDAO.getTaskList(tlrno, "password", flowName, version, taskState, pageIndex, pageSize,
					transMap);
		} catch (WIException wex) {
			ExceptionUtil.throwCommonException(wex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_GETTASKLIST_ERROR);
		} catch (Exception ex) {
	 ...
	 * @throws CommonException
	 */
	public WIPageQueryResult getTaskList(String tlrno, String flowName, String version, String taskName,
			String taskState, Integer pageIndex, Integer pageSize, Map transMap) throws CommonException {
		try {
			// 请求工作流DAO
			WorkFlowDAO workFlowDAO = BaseDAOUtils.getWorkFlowDAO();
			return workFlowDAO.getTaskList(tlrno, "password", flowName, version, taskName, taskState, pageIndex,
					pageSize, transMap);
		} catch (WIException wex) {
			ExceptionUtil.throwCommonException(wex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_GETTASKLIST_ERROR);
		} catch (Exception ex) {
	...
	public List getFlowInstanceList(String tlrno, String flowName, String version, String key) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException(wex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_GETFLOWINS_ERROR);
		} catch (Exception ex) {
			ExceptionUtil.throwCommonException(ex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_GETFLOWINS_ERROR);
		}
		return null;
	}
	...
	/**
	 * 结束指定流程实例 处理流程如下： 结束任务分配 结束工作流任务
	 * 
	 * @param flowInsId
	 *            流程实例号
	 * @throws CommonException
	...
	public Map getTaskValue(String taskId) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException(wex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_GETTASKVALUE_ERROR);
		} catch (Exception ex) {
			ExceptionUtil.throwCommonException(ex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_GETTASKVALUE_ERROR);
		}
		return null;
	}
	...
	public void setTaskValue(String taskId, Map valueMap) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException(wex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_SETTASKVALUE_ERROR);
		} catch (Exception ex) {
			ExceptionUtil.throwCommonException(ex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_SETTASKVALUE_ERROR);
		}
	}
	...
	public void lockTask(String taskId) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException(wex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_LOCKTASK_ERROR);
		} catch (Exception ex) {
			ExceptionUtil.throwCommonException(ex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_LOCKTASK_ERROR);
		}
	}
	...
	public void releaseTask(String taskId) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException(wex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_RELEASETASK_ERROR);
		} catch (Exception ex) {
			ExceptionUtil.throwCommonException(ex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_RELEASETASK_ERROR);
		}
	}
	...
	public void keepTask(String taskId) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException(wex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_KEEPTASK_ERROR);
		} catch (Exception ex) {
			ExceptionUtil.throwCommonException(ex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_KEEPTASK_ERROR);
		}
	}
	...
	public PageQueryResult getTaskListByTlrno(GetTaskRequestBean request) throws CommonException {
		...
		try {
			...
			// 返回taskList
			WIPageQueryResult result = workFlowDAO.getTaskList(userName, password, taskState, pageIndex, pageSize,
					transMap);
	...
	// * @version v1.0,2008-7-16
	// */
	// public StartFlowResponseBean startFlow(StartFlowRequestBean
	// startFlowRequest)
	// throws CommonException {
	...
	// info.setTransData(startFlowRequest.getContractno(),
	// startFlowRequest.getCustcd(), startFlowRequest.getAppno(),
	// startFlowRequest.getTxnamt());
	// // 调用工作流启动流程接口
	 ...
	 * @version v1.0,2008-7-16
	 */
	public TaskBean doTask(DoTaskRequestBean doTaskRequestBean) throws CommonException {
		TaskBean taskBean = new TaskBean();
		...
		try {
			...
			if (doTaskRequestBean.getStatus().equals(WorkFlowHelper.BP_TYPE_ApproveCancelProcess)
					|| doTaskRequestBean.getStatus().equals(WorkFlowHelper.BP_TYPE_ApproveRefuse)) {// 取消任务
				String flowInsId = curWITask.getProcInsId();
				workFlowDAO.closeFlowInstance(flowInsId);

			} else {
				// 调用工作流执行任务接口
				WITask wiTask = workFlowDAO.doTask(userName, password, taskId, actors, statusName, map, comment);
			...
			taskBean.setTaskStartTime(DateUtil.dateToString(curWITask.getTaskStartTime()));
			taskBean.setTaskEndTime(DateUtil.dateToString(curWITask.getTaskEndTime()));

			// edit by richmond_liu 在业务数据库中记录任务已完成
			// WorkflowTaskInfoService.getInstance().markTaskDone(
			// doTaskRequestBean, taskBean);
			// 调用BusinessProcess
			// ILoanSysWebInterfaceService interfaceService =
			// (ILoanSysWebInterfaceService)ApplicationContextUtils.getBean("Interface.LoanSysWebInterfaceService");
	...
	public PageQueryResult getTaskList(GetTaskRequestBean request, String taskState) throws CommonException {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				WorkflowTaskInfo info = WorkflowTaskInfoService.getInstance().findTaskInfo((String) map.get("APPNO"),
						(String) map.get("APPTYPE"), (String) wITask.getTaskName(), (String) flowName,
				...
				if (info != null) {
					...
					taskBean.setUserName(wITask.getUserName());
					/*
					 * author:yangyong start data:20100722 工作流拥有者
					 */
	...
	public PageQueryResult getUploadTaskList(GetTaskRequestBean request, String taskName) throws CommonException {
		List taskList = new ArrayList();
		PageQueryResult returnPageQueryResult = new PageQueryResult();
		try {
			// 请求工作流DAO
			...
			// List list = workFlowDAO.getTaskList(userName, password,
			// taskState);
			WIPageQueryResult result = workFlowDAO.getTaskList(userName, password, flowName, version, null, pageIndex,
					pageSize, transMap);
			List list = result.getQueryResult();
			for (int i = 0; i < list.size(); i++) {
				TOPBPMWITask wITask = (TOPBPMWITask) list.get(i);
	...
	public String getTOPBPMStatus(String status, String isApprove) {
		String statusName = "";
		/*
		 * if (status.equals(WorkFlowHelper.STATUS_Agree)) { if (isApprove !=
		 * null && isApprove.equals("1")) statusName = "ToEnd"; else
		 * //statusName = "AgreeToNext"; statusName = "Agree"; } else if
		 * (status.equals(WorkFlowHelper.STATUS_Rollback)) statusName =
		 * "GoBack"; else if
		 * (status.equals(WorkFlowHelper.STATUS_Refuse)||status.equals(
		 * WorkFlowHelper.STATUS_StopProcess)) statusName = "RefuseToEnd"; else
		 * if(status.equals(WorkFlowHelper.STATUS_AgreeUpload))
		 * statusName="AgreeToNext"; else
		 * if(status.equals(WorkFlowHelper.STATUS_RefuseUpload)) statusName =
		 * "AgreeToNext";
		 */
	 ...
	 * @version v1.0,2008-7-16
	 */
	public TaskBean claimTask(String tlrno, String taskId) throws CommonException {
		TaskBean taskBean = new TaskBean();
		try {
			// 请求工作流DAO
			WorkFlowDAO workFlowDAO = BaseDAOUtils.getWorkFlowDAO();
			...
			WorkflowTaskInfoService workflowTaskInfoService = WorkflowTaskInfoService.getInstance();
			Map map = workFlowDAO.getValue(taskId);
			...
			ExceptionUtil.throwCommonException(ex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_LOCKTASK_ERROR, ex);
		}
	 ...
	 * @version v1.0,2008-7-16
	 */
	public TaskBean cancelClaimTask(String tlrno, String taskId) throws CommonException {
		TaskBean taskBean = new TaskBean();
		try {
			// 请求工作流DAO
			WorkFlowDAO workFlowDAO = BaseDAOUtils.getWorkFlowDAO();
			...
			String appType = (String) map.get("APPTYPE");
			String flowName = (String) map.get("FLOWNAME");
			workflowTaskInfo = workflowTaskInfoService.findTaskInfo(appno, appType, taskName, flowName, piid);
			...
			taskBean.setTaskStartTime(workflowTaskInfo.getTaskStartTime().toString());
			taskBean.setUserName("");

			WorkflowTaskInfoService.getInstance().markTaskUnclaimed(tlrno, taskBean);
			GlobalInfo info = GlobalInfo.getCurrentInstance();
			info.setTransData(null, null, appno, null);
		} catch (Exception ex) {
			ExceptionUtil.throwCommonException(ex.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_RELEASETASK_ERROR, ex);
		}
	 ...
	 * @version v1.0,2008-7-17
	 */
	public PageQueryResult getFinishedTaskList(GetTaskRequestBean request) throws CommonException {
