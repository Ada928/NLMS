...
/**
 * Title: BankBaseWorkflowTaskRule Description: 工作流任务引擎 Copyright: Copyright (c)
...
public abstract class BankBaseWorkflowTaskRule extends BaseWorkflowTaskRule {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.huateng.business.workflow.rule.BaseWorkflowTaskRule#doTask(com.
	 * huateng.business.workflow.rule.base.IWorkflowContext)
	 */
	...
	public void doTask(IWorkflowContext context) throws AppException {
		...
		WorkFlowDoTaskBean workFlowDoTaskBean = new WorkFlowDoTaskBean(context.getTlrList(), brcode, getWorkType(),
				getAssignMode(), isAsnWorkType(), isLv(), tlrno, context.getProcId(), context.getTaskId(), param,
				context.getStatus(), context.getComment());
	...
	/**
	 * 检查执行任务前的WorkflowContext信息 TaskId 不能为空 ProcId 不能为空 TlrList 不能为空 Status
	 * 不能为空
	 * 
	 * @param context
	 * @throws CommonException
	...
	private void checkContextInfo(IWorkflowContext context) throws CommonException {
		if ((StringUtils.isEmpty(context.getTaskId())) || (StringUtils.isEmpty(context.getProcId()))
