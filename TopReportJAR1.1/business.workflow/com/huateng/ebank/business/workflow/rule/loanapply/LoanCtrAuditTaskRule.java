...
/**
 * Title: LoanCtrAuditTaskRule Description: Copyright: Copyright (c) 2008
...
public class LoanCtrAuditTaskRule extends BankBaseWorkflowTaskRule {
	...
	/**
	 * 执行任务前操作 准备待选操作员列表 流转控制,不推荐进行业务处理
	 * 
	 * @param context
	...
	public void preDoTask(IWorkflowContext context) throws CommonException {
		...
		Map parammap = (Map) context.getTxnInParam();

		String nextRoleId = (String) parammap.get("nextRoleId");
		// Added by UU_Wu 2008-05-30
		// String nextRoleId =
		// (String)((Map)context.getTxnInParam()).get("nextRoleId");
		// String taskId = (String)((Map)context.getTxnInParam()).get("taskId");
		// Map<String,Object> map =
		// WorkFlowService.getInstance().getTaskValue(taskId);
		// String nextRoleId = (String)map.get("nextRoleId");
		list.add(nextRoleId);
		// 组装数据
