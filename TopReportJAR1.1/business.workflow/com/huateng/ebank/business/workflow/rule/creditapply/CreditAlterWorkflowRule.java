 ...
 *
 */
package com.huateng.ebank.business.workflow.rule.creditapply;
...
/**
 * Title: LoanApplyWorkflowRule Description:额度变更流程规则 Copyright: Copyright (c)
 * 2008 Company: Shanghai Huateng Software Systems Co., Ltd.
 * 
 * @author shen_antonio
 * @version 1.1, 2008-4-2
 */
public class CreditAlterWorkflowRule extends BankBaseWorkflowRule {
	/**
	 * 流程启动前处理, 进行待选操作员信息列表 流程判断,不推荐进行业务处理
	 * 
	...
	public void preStart(BankWorkflowContext workflowContext) throws CommonException {
		...
		list.add(String.valueOf(WorkFlowConstants.ROLE_ID_LOAN_APPROVE));
		/** 设置流程key . */
	...
	/**
	 * 流程启动后处理 后处理中将可获取流程实例WIFlowInstance 进行更新申请表信息,将启动的流程实例号记录在申请表中 同样不推荐进行业务处理
	 * 
	 * @param workflowContext
	 * @return
	 * @throws CommonException
	...
	public void postStart(BankWorkflowContext workflowContext) throws CommonException {
		...
		/*
		 * LoanGrantApplyDAO loanGrantApplyDAO =
		 * DAOUtils.getLoanGrantApplyDAO(); LoanGrantApply loanGrantApply =
		 * loanGrantApplyDAO.query((String)wiFlowInstance.getValueMap().get(
		 * "appno")); loanGrantApply.setProcInsId(wiFlowInstance.getFID());
