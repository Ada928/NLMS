...
public class WorkFlowParamService {
	 ...
	 *        ????合同号(可能为借据号、项目编号) AMT ????贷款金额 APPTYPE ????申请类型（apptype）
	 *        PROC_NAME ????工作流模板（PROCESS_TEMPLATE） TASK_NAME
	 *        ????下一任务名（TaskName） LAST_TLRNO ????上一操作员（TLRNO） BRCLASS ????机构级别
	 *        BRCODE ????归属操作机构 BIZCLASS 审批类型 BIZ_SUBCLASS ????贷款品种类型
	 *
	 * @return
	...
	public Map getTlrnoRoleTaskOperate(Map hashMap) throws CommonException {
		...
		String processTemplate = DataFormat.trim((String) hashMap.get("PROC_NAME"));
		...
		// 审批类型
		String bizSubClass = DataFormat.trim((String) hashMap.get("BIZSUBCLASS"));
		...
		listwfp = BaseDAOUtils.getWorkflowParamDAO()
				.queryByCondition("po.processTemplate='" + processTemplate.trim() + "' and po.taskName='" + taskName
						+ "' " + " and po.apptype='" + apptype + "' and po.brclass='" + brclass + "' and po.bizClass='"
						+ bizClass + "' and po.bizSubclass='" + bizSubClass + "' ");
		// LIST SIZE= 0 000表示其他 贷款品种类型
		if (listwfp.size() == 0 || listwfp == null) {
			listwfp = BaseDAOUtils.getWorkflowParamDAO()
					.queryByCondition("po.processTemplate='" + processTemplate.trim() + "' and po.taskName='" + taskName
							+ "' " + " and po.apptype='" + apptype + "' and po.brclass='" + brclass
							+ "' and po.bizClass='" + bizClass + "' and po.bizSubclass='" + BIZ_SUB_CLASS_TYPE + "' ");
		}
		...
		if (brcode_type.equals(BRCODE_TYPE_OFF)) {// 0-
			 ...
			 * 
			 * if(WorkFlowConstants.ROLE_ID_LOAN_AUDIT==Integer.parseInt(
			 * role_type)){ brcodeList = brcode; }else{ brcodeList =
			 * bctlService.getUpBrcode(brclass, brcode); }
			 */
		...
		if (assign_type.equals(ASSIGN_TYPE_OFF)) { // 0- 指定到操作员列表
			...
			if (assign_type.equals(ASSIGN_TYPE_ON)) { // 1- 指定到岗位
				...
				listRoleInfo = BaseDAOUtils.getRoleInfoDAO().queryByCondition("po.id='" + role_type + "'");
				...
				for (int m = 0; m < strRoleId.length; m++) {
					...
					hql.append(
							"select tlr.tlrno,tlr.brcode from TlrInfo as tlr,TlrRoleRel as rela where tlr.tlrno=rela.tlrno and rela.roleId=")
							.append(roleStrid);
		...
		if (amt_type.equals(AMT_TYPE_ON)) {// 1-不按金额权限
			...
			if (amt_type.equals(AMT_TYPE_OFF)) {// 0-按金额权限
				...
				for (int i = 0; i < tlrnoListArray.length; i++) {
					...
					tlrList = BaseDAOUtils.getLimitParamDAO()
							.queryByCondition("po.tlrno='" + tlrnoListArray[i] + "' and po.status='1' and po.bizClass='"
									+ bizClass + "' and po.bizSubclass='" + bizSubClass + "'  and (po.limitMaxamount>="
									+ amt + " and po.limitMinamount<=" + amt + ")");
					...
					if (tlrList.size() > 0) {
						for (int j = 0; j < tlrList.size(); j++) {
							...
							if (i == 0 && j == 0) {
								...
								listTlrno = listTlrno + "," + DataFormat.trim(lmtp.getTlrno());
							}
						}
					}
				}
				...
				if (listTlrno.length() == 0) {
					for (int i = 0; i < tlrnoListArray.length; i++) {
						List tlrList = new ArrayList();
						tlrList = BaseDAOUtils.getLimitParamDAO()
								.queryByCondition("po.tlrno='" + tlrnoListArray[i]
										+ "' and po.status='1' and po.bizClass='" + bizClass
				...
				if (listTlrno.length() == 0) {// 3、
												// 查询结果TLRNO_LIST（上处理结果为空）为空,上述没有查询结果时
					...
					if (brList == null || brList.size() == 0) {
						...
						tlrnoList = this.getOnlyTlrno(this.doNotlrnoAmtList(tlrnoListTemp, bizClass, bizSubClass, amt));
					...
					// TlrRoleRelation trlRoleRelation = (TlrRoleRelation)
					// tlrRoleList1
					// .get(m);
	...
	public String getAttitude(Map hashMap) throws CommonException {
		...
		String bizSubClass = DataFormat.trim((String) hashMap.get("BIZ_SUBCLASS"));// 贷款大类
																					// 合作项目类型
																					// 000表示不限

		double amt = Double.valueOf(DataFormat.trim((String) hashMap.get("AMT"))).doubleValue();

		List wfaList = new ArrayList();
		wfaList = BaseDAOUtils.getWorkflowAttitudeDAO().queryByCondition("po.processTemplate='" + processTemplate
				+ "' and po.taskName='" + taskName + "' and po.apptype='" + appType + "'");
		// if (wfaList.size() == 1) {
		...
		// + "' and po.status='1' and po.bizClass='"
		// + bizClass
		// + "' and ( po.bizSubclass='000' or po.bizSubclass='0' ) and
		// (po.limitAmount>="
	 ...
	 *        结果返回："BB,CC,DD,EE"
	 */
	public static String getNotIncludeLastTlrno(String alltlrnoList, String lastTlrno) throws CommonException {
	...
	public void initInsertWorkFlowTaskInfoForRoute(Map map) throws CommonException {
		...
		// String brclass = DataFormat.trim((String) map.get("BRCLASS"));//
		// BRCLASS
		String roletype = DataFormat.trim((String) map.get("ROLETYPE"));
		// String bizClass = DataFormat.trim((String) map.get("BIZCLASS"));//
		// BIZCLASS
		// // 审批类型
		// String bizSubClass = DataFormat.trim((String)
		// map.get("BIZSUBCLASS"));
		...
		wftInfo.setProcessTemplate(DataFormat.trim((String) map.get("PROC_NAME")));
	...
	public void initInsertWorkFlowTaskInfo(Map map) throws CommonException {
		...
		List listwkfp = new ArrayList();
		listwkfp = BaseDAOUtils.getWorkflowParamDAO().queryByCondition("po.processTemplate='" + processTemplate.trim()
				+ "' and po.taskName='" + taskName + "' " + " and po.apptype='" + apptype
		...
		if (listwkfp.size() == 0 || listwkfp == null) {
			listwkfp = BaseDAOUtils.getWorkflowParamDAO().queryByCondition("po.processTemplate='"
					+ processTemplate.trim() + "' and po.taskName='" + taskName + "' " + " and po.apptype='" + apptype
					// + "' and po.brclass='" + brclass
					+ "' and po.bizClass='" + bizClass + "' and po.bizSubclass='" + BIZ_SUB_CLASS_TYPE + "' ");
		}
		...
		if (procInsList.size() == 0) {
			...
			// BaseDAOUtils.getWorkflowTaskInfoDAO().queryById(preTaskIdStr);
			List undoWorkflowTaskInfoList = workflowTaskInfoDAO.queryByCondition(
					"po.procInsId = '" + procInsId + "' and po.taskEndFlag ='" + SystemConstant.FLAG_OFF + "' ");
		...
		wftInfo.setProcInsId(DataFormat.trim(procInsId));
		wftInfo.setId((Long) map.get("TASKID"));
		// wftInfo.setTaskId(DataFormat.trim((String) map.get("TASKID")));
		wftInfo.setTaskName(DataFormat.trim((String) map.get("TASK_NAME")));
		wftInfo.setProcessTemplate(DataFormat.trim((String) map.get("PROC_NAME")));
	 ...
	 */
	public String doNotlrnoAmtList(String tlrnoListTemp, String bizClass, String bizSubClass, double amt)
			throws CommonException {
		...
		amtlist = BaseDAOUtils.getLimitParamDAO()
				.queryByCondition("po.bizClass='" + bizClass + "' and po.status='1' " + " and (po.bizSubclass='"
						+ bizSubClass + "' or po.bizSubclass='000' or  po.bizSubclass='0' ) " + " and po.tlrno in ("
						+ tlrnoListTemp + ") order by po.limitMinamount desc ");
		...
		if (amtlist.size() == 1) {
			...
			if (amtlist.size() > 1) {// 如果有相同的最高金额，所对应的操作员都获取到
				...
				amtlist2 = BaseDAOUtils.getLimitParamDAO()
						.queryByCondition("po.bizClass='" + bizClass + "' and po.status='1' " + " and (po.bizSubclass='"
								+ bizSubClass + "' or po.bizSubclass='000' or po.bizSubclass='0' ) and po.tlrno in ("
								+ tlrnoListTemp + ") and po.limitMinamount=" + limitAmt);
	...
	public List getAllProcName() throws CommonException {
		...
		String[] template = DataFormat.trim(WorkFlowParamConfigReader.getProperty("PROCESS.TEMPLATGE.LIST")).split(",");
		for (int i = 0; i < template.length; i++) {
			WorkFlowParamSelectBean wfpSelectBean = new WorkFlowParamSelectBean();
			wfpSelectBean.setProcName(template[i]);
			String templateName = DataFormat.trim(WorkFlowParamConfigReader.getProperty(template[i] + "_NAME"));
	...
	//
	// BrhWorkFlowDefDAO brhWorkFlowDefDAO =
	// BaseDAOUtils.getBrhWorkFlowDefDAO();
	//
	 ...
	 * @throws CommonException
	 */
	public BrhWorkflowDef getProcNameByLnid(String brcode, String apptype, String lnid) throws CommonException {

		BrhWorkFlowDefDAO brhWorkFlowDefDAO = BaseDAOUtils.getBrhWorkFlowDefDAO();

		BrhWorkflowDef brhWorkflowDef = brhWorkFlowDefDAO.queryByUk(brcode, apptype, lnid);
		...
		if (brhWorkflowDef == null) {
			ExceptionUtil.throwCommonException("贷款品种流程配置表查询错误.", ErrorCode.ERROR_CODE_BRH_WORKFLOW_DEF_SELECT);
		}
		return brhWorkflowDef;
	}
	...
	public WorkflowTaskInfo getUnEndTaskInfoByAppno(String appno) throws CommonException {
		...
		workflowTaskInfolist = BaseDAOUtils.getWorkflowTaskInfoDAO()
				.queryByCondition(" po.appno ='" + appno + "' order by id");
	 ...
	 */
	public String taskAssign(String appno, String appType, String taskName, String brcode) throws CommonException {

		WorkflowParamDAO dao = BaseDAOUtils.getWorkflowParamDAO();
		List list = dao.queryByCondition("po.taskName = ? and po.apptype = ?", new Object[] { taskName, appType });
		if (list.size() == 0) {
			ExceptionUtil.throwCommonException("没有找到任务分配参数！ taskName: " + taskName,
		...
		if (param.getAssignType().equals(ASSIGN_TYPE_TOROLE)) {
			...
			result = code + role;
			WorkflowTaskInfoService.getInstance().markTaskAssigned(appno, appType, taskName, role);
	...
	public List queryByCondition(String procName) throws CommonException {
		...
		WorkflowParamDAO dao = BaseDAOUtils.getWorkflowParamDAO();
		List list = dao.queryByCondition("po.processTemplate = ?", new Object[] { procName });
		return list;
	}
	 ...
	 * @throws CommonException
	 */
	public void updateParam(List insertList, List delList) throws CommonException {
		WorkflowParamDAO dao = BaseDAOUtils.getWorkflowParamDAO();
		// 删除操作.
		...
		for (Iterator it = insertList.iterator(); it.hasNext();) {
			...
			if (list.size() > 0) {
				ExceptionUtil.throwCommonException("已经存在任务：" + taskName + "，不能新增此任务",
						ErrorCode.ERROR_CODE_CANNOT_SUBMIT);
			} else
				dao.insert(param);
		}
	...
	public WorkflowParam queryWorkflowParam(String procName, String taskName) throws CommonException {
		...
		WorkflowParamDAO dao = BaseDAOUtils.getWorkflowParamDAO();
		// List list = dao.queryByCondition("po.processTemplate = ? and
		// po.taskName = ?",
		// new Object[] { procName, taskName });
		// String sql = "where po.processTemplate = '"+procName+"' and
		// po.taskName = '"+taskName+"'";
	...
	public List getTlrnolistByBrcodeRole(String brcode, Integer roleId) throws CommonException {
		...
		buffer.append("select t.tlrno from TlrInfo t,TlrRoleRel tr where t.tlrno = tr.tlrno ")
				.append(" and t.brcode = '").append(brcode).append("'").append(" and tr.roleId = ").append(roleId);
		List list = new ArrayList();
	...
	public Integer getWorkflowRouteInfoNew(String bussType, BigDecimal amount, String startbrcode)
			throws CommonException {
		Integer routeId = 0;
		...
		workflowRouteBindinglist = workflowRouteBindingDAO.queryByCondition(
				" po.bussType = ? and po.maxAmt >= ? and po.startBrhid = ? and po.routeId <> ? order by po.maxAmt",
				new Object[] { SystemConstant.DEFAULT_BUSSTYPE + SystemConstant.DEFAULT_BIZTYPE, amount, startbrcode,
						0 },
				null);
		if (workflowRouteBindinglist != null && workflowRouteBindinglist.size() > 0) {
			WorkflowRouteBinding workflowRouteBinding = (WorkflowRouteBinding) workflowRouteBindinglist.get(0);
		...
		workflowRouteBindinglist = workflowRouteBindingDAO.queryByCondition(
				" po.bussType = ? and po.maxAmt >= ? and po.startBrhid = ? and po.routeId <> ? order by po.maxAmt",
				new Object[] { SystemConstant.DEFAULT_BUSSTYPE + SystemConstant.DEFAULT_BIZTYPE, amount, "-" + brhClass,
						0 },
				null);
		if (workflowRouteBindinglist != null && workflowRouteBindinglist.size() > 0) {
			WorkflowRouteBinding workflowRouteBinding = (WorkflowRouteBinding) workflowRouteBindinglist.get(0);
	...
	public Integer getWorkflowRouteInfo(String bussType, BigDecimal amount, String startbrcode) throws CommonException {
		...
		Bctl bctl = BctlService.getInstance().queryByBrcode(startbrcode);
		/**
		 * 查询原则：按照下面原则先后次序查询 1. 查询指定机构、指定业务品种的审批路线 2. 查询指定机构、不指定业务品种的审批路线 3.
		 * 查询制定机构级别、指定业务品种的审批路线 4. 查询指定机构级别、不指定业务品种的审批路线
		 */
		/** modify by shen_antonio 20091214 jira: BMS-2334 end . */
		// 1. 查询指定机构、指定业务品种的审批路线
		List workflowRouteBindinglist1 = workflowRouteBindingDAO.queryByCondition(
				" po.bussType = ? and po.maxAmt >= ? and po.startBrhid = ? and po.routeId <> ? order by po.maxAmt",
				new Object[] { bussType, amount, Integer.valueOf(startbrcode), 0 }, null);
		// 2. 查询指定机构、不指定业务品种的审批路线
		if (workflowRouteBindinglist1.size() == 0) {
			List workflowRouteBindinglist2 = workflowRouteBindingDAO.queryByCondition(
					" po.bussType = ? and po.maxAmt >= ? and po.startBrhid = ?  and po.routeId <> ? order by po.maxAmt",
					new Object[] { SystemConstant.DEFAULT_BUSSTYPE, amount, Integer.valueOf(startbrcode), 0 }, null);
			if (workflowRouteBindinglist2.size() > 0) {
				...
				String brhClass = bctl.getBrclass();
				List workflowRouteBindinglist3 = workflowRouteBindingDAO.queryByCondition(
						" po.bussType = ? and po.startBrhid = ? and po.maxAmt >= ?  and po.routeId <> ? order by po.maxAmt",
						new Object[] { bussType, Integer.parseInt(brhClass) * -1, amount, 0 }, null);
				...
				// 4. 查询指定机构级别、不指定业务品种的审批路线
					...
					List workflowRouteBindinglist4 = workflowRouteBindingDAO.queryByCondition(
							" po.bussType = ?  and po.startBrhid = ? and po.maxAmt >= ? and po.routeId <> ? order by po.maxAmt",
							new Object[] { SystemConstant.DEFAULT_BUSSTYPE, Integer.parseInt(brhClass) * -1, amount,
									0 },
							null);
					if (workflowRouteBindinglist4.size() > 0) {
						WorkflowRouteBinding workflowRouteBinding = (WorkflowRouteBinding) workflowRouteBindinglist4
								.get(0);
						routeId = workflowRouteBinding.getRouteId();
					}
				}
	...
	public void insertWorkflowInsRoute(String piid, Integer routeId) throws CommonException {
		...
		try {
			routeParamlist = BaseDAOUtils.getWorkflowRouteParamDAO().queryByCondition(
					" po.routeId = ? order by po.stopId", new Object[] { routeId }, new Type[] { Hibernate.INTEGER });
		} catch (CommonException e) {
	 ...
	 * @auth bussType 目前是8位的。BIZ_CLASS(4位)+BIZ_SUBCLASS(4位)
	 */
	...
	public List getOprlistByAmt(List oprnolist1, String bussType, BigDecimal amount) throws CommonException {
		...
		for (int k = 0; k < oprnolist1.size(); k++) {
			...
			List tempLimitParamlist = BaseDAOUtils.getLimitParamDAO().queryByCondition(
					" po.tlrno =? and po.status=? and po.bizClass= ? and po.bizSubclass= ?",
			...
			if (tempLimitParamlist.size() >= 1) {
				...
				// 大类匹配，小类不匹配的情况下 比较金额
				List tempLimitParamlist2 = BaseDAOUtils.getLimitParamDAO().queryByCondition(
						" po.tlrno =? and po.status=? and po.bizClass=? and po.bizSubclass=? and po.limitMaxamount >= ?",
						new Object[] { tlrno, SystemConstant.VALID_FLAG_VALID, bizClass,
								SystemConstant.DEFAULT_BUSSTYPE, amount },
						new Type[] { Hibernate.STRING, Hibernate.STRING, Hibernate.STRING, Hibernate.STRING,
				...
				if (tempLimitParamlist2.size() >= 1) {
					...
					// 大类不匹配，小类匹配的情况下 比较金额
					List tempLimitParamlist3 = BaseDAOUtils.getLimitParamDAO().queryByCondition(
							" po.tlrno =? and po.status=? and po.bizClass=? and po.bizSubclass=? and po.limitMaxamount >= ?",
							new Object[] { tlrno, SystemConstant.VALID_FLAG_VALID, SystemConstant.DEFAULT_BUSSTYPE,
									bizSubclass, amount },
							new Type[] { Hibernate.STRING, Hibernate.STRING, Hibernate.STRING, Hibernate.STRING,
									Hibernate.BIG_DECIMAL });
					...
					// 大类不匹配，小类不匹配的情况下
						...
						// 如果有大类不限制，有小类不限制的话
						List tempLimitParamlist4 = BaseDAOUtils.getLimitParamDAO().queryByCondition(
								" po.tlrno =? and po.status=? and po.bizClass=? and po.bizSubclass=? and po.limitMaxamount >= ?",
								new Object[] { tlrno, SystemConstant.VALID_FLAG_VALID, SystemConstant.DEFAULT_BUSSTYPE,
										SystemConstant.DEFAULT_BUSSTYPE, amount },
								new Type[] { Hibernate.STRING, Hibernate.STRING, Hibernate.STRING, Hibernate.STRING,
	...
	public Map getAssignedOprnoList(Integer routeId, Integer stopId, String startbrcode, String bussType,
			String manualAssigned, String piid, BigDecimal amount) throws CommonException {
		...
		if (stopId == null || stopId.equals(0)) {
			InsRouteList = BaseDAOUtils.getWorkflowInsRouteDAO().queryByCondition(
					" po.routeId = ? and po.piid = ? order by po.stopId", new Object[] { routeId, piid },
					new Type[] { Hibernate.INTEGER, Hibernate.STRING });
		} else {
			InsRouteList = BaseDAOUtils.getWorkflowInsRouteDAO().queryByCondition(
					" po.routeId = ? and po.stopId > ? and po.piid = ? order by po.stopId",
	...
	public Map getCommonTaskAssign(Map map) throws CommonException {
		...
		String tlrno = taskAssignService.getAssignToTlrnoGZ(tlrnolist, null, SystemConstant.TASK_ASSIGN_MODE_1, false,
				true, map);
			...
			if (map != null) {
				...
				for (int i = 0; i < routelist.size(); i++) {
					...
					if (workflowRouteParam.getNeed().equals(SystemConstant.FLAG_ON)) {
						String currBrhid = BctlService.getInstance().getUpBrcode(workflowRouteParam.getBrhClass(),
								startbrcode);
	...
	public boolean hasNeedStop(Integer routeId, Integer stopId) throws CommonException {
		List routeParamlist = BaseDAOUtils.getWorkflowRouteParamDAO().queryByCondition(
				" po.routeId = ? and po.stopId > ? order by po.stopId", new Object[] { routeId, stopId },
	...
	public PageQueryResult queryWorkflowParamAll(WorkflowParamBean inbean) throws CommonException {
		...
		for (int i = 0; i < nodelist.size(); i++) {
			...
			if (nodeName.equals("Start") || nodeName.equals("End")) {
				...
				List workflowparamList = DAOUtils.getWorkflowParamDAO().queryByCondition(
						" po.processTemplate = ? and po.taskName = ?", new Object[] { procName, nodeName },
	 ...
	 *
	 */
	public void saveWorkflowParamEntry(List delList, List insertList, List updateList)
		...
		for (Iterator it = updateList.iterator(); it.hasNext();) {
			...
			if (list.size() == 1) {
				...
				// try {
				// BeanUtils.copyProperties(workflowParam,
				// updatebean.getWorkflowParam());
				...
				ExceptionUtil.throwCommonException(
						"已存在一条流程为" + updatebean.getProcName() + "任务为" + updatebean.getNodeName() + "业务种类为"
								+ updatebean.getWorkflowParam().getBizClass() + "的记录",
		...
		for (Iterator it = delList.iterator(); it.hasNext();) {
			...
			;
			WorkflowParam workflowParam = new WorkflowParam();
			List list = workflowParamDAO.queryByCondition(
					" po.processTemplate = ? and po.taskName = ? and bussType = ?",
					new Object[] { delbean.getProcName(), delbean.getNodeName(),
							delbean.getWorkflowParam().getBizClass() },
					new Type[] { Hibernate.STRING, Hibernate.STRING, Hibernate.STRING });
			if (list.size() > 0) {
				workflowParam = (WorkflowParam) list.get(0);
				try {
	...
	public List getAllApproveTaskAssignOprList(Map map, String approvePosition) throws CommonException {
		...
		String piid = (String) map.get("PROC_INS_ID");
		// 得到当前流程实例的审批路线实例信息
		List currRouteList = DAOUtils.getWorkflowInsRouteDAO()
				.queryByCondition(" po.piid = '" + piid + "' order by po.stopId");

		WorkflowInsRoute workflowInsRoute = null;
		for (int i = 0; i < currRouteList.size(); i++) {
			workflowInsRoute = new WorkflowInsRoute();
			workflowInsRoute = (WorkflowInsRoute) currRouteList.get(i);
			Map resultMap = getAssignedOprnoList(workflowInsRoute.getRouteId(), workflowInsRoute.getStopId(),
					startBrhId.toString(), bussType, manualAssigned, piid, amount);
	 ...
	 * @auth UU_Wu
	 */
	public List getAllCommonTaskAssignOprList(Map map, int currTaskIndex, List hashedWorkflowParamList)
			throws CommonException {
		List resultlist = new ArrayList();
	...
	private List getSortedWorkflowParam(String procName) throws CommonException {
		...
		for (int i = 0; i < taskNames.length; i++) {
			...
			List workflowparamList = BaseDAOUtils.getWorkflowParamDAO()
					.queryByCondition("po.processTemplate ='" + procName + "' and taskName = '" + taskName + "'");
			if (workflowparamList != null && workflowparamList.size() > 0) {
