...
public class ApproveService {
	...
	public List queryHistoryAppInfo(String appno) throws CommonException {
		...
		List applydtlList = applydtlDAO
				.queryByCondition(" tpci.appno = '" + appno + "' order by tpci.txnDate, tpci.timestamps, tpci.id");
		...
		for (int i = 0; i < applydtlList.size(); i++) {
			...
			// if(SystemConstant.ROLE_CUST_ZONGJINGLI == applydtl.getRoleId() &&
			// SystemConstant.APP_ATTITUDE_AGREE_TO_NEXT.equals(applydtl.getAttitude()))
			// {
			//
			// historyAppInfo.setAttitude(ds.getNameByTypeNo(SystemConstant.DATADIC_TYPE_ATTITUDE_JZ_ALL,
			// SystemConstant.APP_ATTITUDE_AGREE_TO_END));
			// }
			/* END */
	...
	// + SystemConstant.APPLY_TYPE_PROJECT_APPLY + "' order by po.txdate,
	// po.txtime,po.id");
	//
	// for (int i = 0; i < applydtlList.size(); i++) {
	// Applydtl applydtl = (Applydtl) applydtlList.get(i);
	...
	// + SystemConstant.APPLY_TYPE_LOAN_CARD_APPLY + "') order by po.txdate,
	// po.txtime, po.timestamps");
	//
	// for (int i = 0; i < applydtlList.size(); i++) {
	// Applydtl applydtl = (Applydtl) applydtlList.get(i);
	// TlrInfo tlrInfo = tlrInfoDAO.query(applydtl.getTlrno());
	...
	// */
	// //退回标志 不存在了
	// /*public void updateHistoryWhenUntread(String appno) throws
	// CommonException {
