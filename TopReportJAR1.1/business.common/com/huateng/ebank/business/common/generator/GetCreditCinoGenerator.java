...
/**
 * Title: GetCreditCinoGenerator Description:授信贷款借据号生成器 Copyright: Copyright (c)
...
public class GetCreditCinoGenerator extends BaseGenerator {
	...
	public String gen(Object paramMap) throws CommonException {
		...
		if (seqno > 99) {
			ExceptionUtil.throwCommonException("同一笔合同下借据数不能超过99笔", ErrorCode.ERROR_CODE_GEN_CINO_ERROR);
		}
		Double objLocker = 0.00; // 20090327 加锁保护
		synchronized (objLocker) {
			String tempContractno = contractno.substring(0, SystemConstant.CONTRACTNO_LENGTH);

			Iterator it = DAOUtils.getHQLDAO()
					.queryByQL("select po.cino from Loancino po where po.cino like '" + tempContractno
							+ "%' order by substring(po.cino, " + (SystemConstant.CONTRACTNO_LENGTH + 1) + ", 4) desc");
			if (it.hasNext()) {
				int maxSeqno = Integer
						.parseInt(((String) it.next()).substring(SystemConstant.CONTRACTNO_LENGTH).trim());
