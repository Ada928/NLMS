...
public class DataDicUtils {
	...
	public static void initDataDic(Locale locale) throws CommonException {
		try {
			...
			MessageResources mess = MessageResources.getMessageResources("resources.DataDicResources");
			...
			while (it.hasNext()) {
				...
				if (locale != null) {
					String dataNameInter = mess.getMessage(locale,
							dataDic.getDataTypeNo() + "." + dataDic.getDataNo().trim());
					if (dataNameInter != null) {
						dataName = dataNameInter;
					}
					tblNm = TBL_NM + locale;
				}
