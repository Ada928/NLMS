...
public class DepartmentInfoService {
	...
	public List getAllEnableDepartment() throws CommonException {
		...
		List list = departmentInfoDAO.queryByCondition(
				"po.status='" + SystemConstant.VALID_FLAG_VALID + "' order by po.departmentCode desc");
		return list;
	...
	public List getAllEnableDepartment(String currentCode) throws CommonException {
		DepartmentInfoDAO departmentInfoDAO = BaseDAOUtils.getDepartmentInfoDAO();
		List list = departmentInfoDAO.queryByCondition("po.brcode='" + currentCode + "' and po.status='"
				+ SystemConstant.VALID_FLAG_VALID + "' order by po.departmentCode desc");
		return list;
	...
	public void departmentInfo(List insertList, List updateList, List delList) throws CommonException {
		DepartmentInfoDAO departmentInfoDAO = BaseDAOUtils.getDepartmentInfoDAO();
		...
		for (Iterator it = insertList.iterator(); it.hasNext();) {
			DepartmentInfo departmentInfo = (DepartmentInfo) it.next();
			List list = departmentInfoDAO.queryByCondition(
					"po.departmentName = '" + departmentInfo.getDepartmentName() + "'" + " and po.brcode = '"
							+ departmentInfo.getBrcode() + "'" + " and po.status = " + SystemConstant.VALID_FLAG_VALID);
