...
public class BrhWorkFlowDefDAO extends HibernateDaoSupport {
	...
	public List queryByCondition(String whereString) throws CommonException {
		try {
			List list = this.getHibernateTemplate().find("from BrhWorkflowDef po where " + whereString);
			return list;
		} catch (Exception e) {
			ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_BRH_WORKFLOW_DEF_SELECT, e);
		}
		return null;
	}
	...
	public List queryByCondition(String whereString, Object[] objArray, Type[] typeArray) throws CommonException {
		try {
			List list = this.getHibernateTemplate().find("from BrhWorkflowDef po where " + whereString, objArray);
			return list;
		} catch (Exception e) {
			ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_BRH_WORKFLOW_DEF_SELECT, e);
		}
	...
	public void update(BrhWorkflowDef po) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_BRH_WORKFLOW_DEF_UPDATE, e);
		}
	}
	...
	public void insert(BrhWorkflowDef po) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_BRH_WORKFLOW_DEF_INSERT, e);
		}
	}
	...
	public void delete(BrhWorkflowDef po) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_BRH_WORKFLOW_DEF_DELETE, e);
		}
	}
	...
	public BrhWorkflowDef query(Long id) throws CommonException {
		try {
			return (BrhWorkflowDef) this.getHibernateTemplate().get(BrhWorkflowDef.class, new Long(id));
		} catch (Exception e) {
			ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_BRH_WORKFLOW_DEF_SELECT, e);
		}
		return null;
	...
	/**
	 * 根据输入的条件查询所有符合条件的记录 机构号、申请类型、贷款种类
	 * 
	 * @param whereString
	 * @return 包含WorkflowAttitude对象的List
	 * @throws CommonException
	...
	public BrhWorkflowDef queryByUk(String brcode, String apptype, String bizClass) throws CommonException {
		try {
			...
			List list = this.getHibernateTemplate().find("from BrhWorkflowDef po where po.brcode = '" + brcode
					+ "' and po.apptype = '" + apptype + "' and po.bizClass = '" + bizClass + "'");
	...
	/**
	 * 根据输入的条件查询所有符合条件的记录 机构号、申请类型、贷款种类、贷款性质
	 * 
	 * @param whereString
	 * @return 包含WorkflowAttitude对象的List
	 * @throws CommonException
	 */
	public BrhWorkflowDef queryByUk(String brcode, String apptype, String bizClass, String bizSubclass)
			throws CommonException {
		try {
			BrhWorkflowDef brhWorkflowDef = null;
			List list = this.getHibernateTemplate()
					.find("from BrhWorkflowDef po where po.brcode = '" + brcode + "' and po.apptype = '" + apptype
							+ "' and po.bizClass = '" + bizClass + "'" + " and po.bizSubclass = '" + bizSubclass + "'");
