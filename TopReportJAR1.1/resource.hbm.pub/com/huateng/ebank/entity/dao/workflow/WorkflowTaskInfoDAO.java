...
public class WorkflowTaskInfoDAO extends HibernateDaoSupport {
	...
	public List queryByCondition(String whereString) throws CommonException {
		try {
			List list = this.getHibernateTemplate().find("from WorkflowTaskInfo po where " + whereString);
			return list;
		} catch (Exception e) {
			ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_LNTYPE_INFO_SELECT, e);
		}
		return null;
	}
	...
	public List queryByProcInsId(String procInsId) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException("工作流任务信息表查询错误-" + e.getMessage(), ErrorCode.ERROR_CODE_DAO, e);
		}
		return null;
	}
	...
	public WorkflowTaskInfo queryByTask(String taskName, String procInsId) throws CommonException {
		try {
			List list = queryByCondition(
					"po.procInsId = '" + procInsId + "' and po.taskName = '" + taskName + "' order by po.id desc");
			if (list.size() > 0) {
		...
		ExceptionUtil.throwCommonException(
				"工作流任务信息表查询错误-该[proc_ins_id = " + procInsId + " , task_name = " + taskName + "]任务记录失败",
	...
	public WorkflowTaskInfo queryByTaskId(String taskId) throws CommonException {
		try {
			...
			List list = this.getHibernateTemplate().find("from WorkflowTaskInfo po where po.taskId = '" + taskId + "'");
			if (list.size() == 1)
				return (WorkflowTaskInfo) list.get(0);
		} catch (Exception e) {
			ExceptionUtil.throwCommonException("工作流任务信息表查询错误-" + e.getMessage(), ErrorCode.ERROR_CODE_DAO, e);
