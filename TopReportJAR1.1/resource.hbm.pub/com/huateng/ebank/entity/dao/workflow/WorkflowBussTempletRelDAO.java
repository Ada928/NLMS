...
public class WorkflowBussTempletRelDAO extends HibernateDaoSupport {
	...
	public WorkflowBussTempletRel query(long id) throws CommonException {
		try {
			return (WorkflowBussTempletRel) this.getHibernateTemplate().load(WorkflowBussTempletRel.class,
					new Long(id));
		} catch (Exception e) {
			ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_BUSS_TEMPLET_REL_SELECT,
					e);
		}
		return null;
	}
	...
	public List queryByCondition(String whereString, Object[] objArray, Type[] typeArray) throws CommonException {
		try {
			List list = this.getHibernateTemplate().find("from WorkflowBussTempletRel po where " + whereString,
					objArray);
			return list;
		} catch (Exception e) {
			ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_BUSS_TEMPLET_REL_SELECT,
	...
	public void update(WorkflowBussTempletRel po) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_BUSS_TEMPLET_REL_UPDATE,
					e);
		}
	}
	...
	public void insert(WorkflowBussTempletRel po) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_BUSS_TEMPLET_REL_INSERT,
					e);
		}
	}
	...
	public void delete(WorkflowBussTempletRel po) throws CommonException {
		try {
			...
			ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_WORKFLOW_BUSS_TEMPLET_REL_DELETE,
					e);
		}
		// try {
		// delete(po.getId());
