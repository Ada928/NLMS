...
public class WorkflowTaskPoolDAO extends HibernateDaoSupport {
	...
	public List queryByCondition(String whereString) throws CommonException {
		try {
			List list = this.getHibernateTemplate().find("from WorkflowTaskPool po where " + whereString);
			return list;
		} catch (Exception e) {
	...
	public WorkflowTaskPool queryByTaskId(String taskId) throws CommonException {
		try {
			...
			List list = this.getHibernateTemplate().find("from WorkflowTaskPool po where po.taskId = '" + taskId + "'");
			if (list.size() == 1)
				return (WorkflowTaskPool) list.get(0);
		} catch (Exception e) {
			ExceptionUtil.throwCommonException("工作流任务信息表查询错误-" + e.getMessage(), ErrorCode.ERROR_CODE_DAO, e);
